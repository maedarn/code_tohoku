RECURSIVE subroutine SELFGRAVWAVE(dt,mode)
  ...
  if(mode==50) then
     ...
     do k = -1, Ncellz+2
     do j = -1, Ncelly+2
       read(98) (Phiwv(i,j,k,1),Phigrdwv(i,j,k,1),dummy, i=-1,Ncellx+2 )
     end do
     end do
     close(98)
  Phi(:,:,:) = Phiwv(:,:,:,1)
...
U(i,j,k,1) = dble(dmy(28))
 ...
 !    call movesph(dt)
     tdm=dx1/cg*tratio
     ntdiv=ntdiv*int(1.d0/tratio)
     do nt=1,ntdiv
     iwx=1;iwy=1;iwz=1
     call BCgrv(BCphi,1,1)
     call slvmuscle(tdm)
     enddo
  end if
