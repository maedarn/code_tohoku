module comvar
  implicit none
  integer, parameter :: ndx=130,ndy=130,laststep=5000,istx=1,ienx=2,isty=1,ieny=2,svnum=50 !preiodic:ist=1,ien=2 , kotei:ist=2,ien=3 : ndx=130
  !double precision, parameter :: Lbox=1.0d2 , h=10.0d0 , hcen=50.0d0 , dinit1=1.29988444d0,w1=2.0d0
  integer :: iwx,iwy,iwz,bndx=3,bndy=4 !odd:x, even:y, 1,2:periodic, 3,4:exact, 5,6:exact+free
  DOUBLE PRECISION :: cg = 1.0d0 , dx,dy != Lbox/dble(ndx-2) !, bcphi1 , bcphi2
double precision :: Lbox=1.0d2 , h=10.0d0 , hcen=50.0d0 , dinit1=1.29988444d1,w1=2.0d0 ,rsph=5.d0
  !double precision :: G=1.11142d-4, G4pi=12.56637d0*G , coeff=0.90d0 ,  kappa=1.0d0/3.0d0
  double precision ::  G4pi=12.56637d0*1.11142d-4 , coeff=0.25d0 ,meanrho!,  kappa=1.0d0/3.0d0
...
module grvvar
  implicit none
  integer, parameter :: ndx2=130 , ndy2=130 !パラメータ属性必要
  DOUBLE PRECISION , dimension(-1:ndx2) :: x
  DOUBLE PRECISION , dimension(-1:ndy2) :: y
  DOUBLE PRECISION , dimension(-1:ndx2,-1:ndy2) :: Phidtn , rho , fx , fy ,wp1 ,wp2, wp2p
...
program muscl1D
  ...
  do i=1,laststep
    ...
    call BC(bndy,0)
    call muslcslv1D(wp2,rho,dt*0.5d0,1,2)
    call muslcslv1D(wp1,rho,dt*0.5d0,1,2)
    call BC(0,bndy)
    call BC(0,bndx)
    wp2p(:,:)=wp2(:,:)
    do k=1,ndy-2
      do j=1,ndx-2
    wp2(j,k) = wp2(j,k)+cg*dt*wp1(j,k)*0.5d0
      enddo
    enddo
    iwx=1;iwy=1
    do k=1,ndy-2
      do j=1,ndx-2
        wp1(j,k) = wp1(j,k)-cg*G4pi*rho(j,k)*dt &
                   -0.5d0*dt*cg*(wp2p(j+1,k+1)-wp2p(j+1,k-1)-wp2p(j-1,k+1)+wp2p(j-1,k-1))/dx/dy
      enddo
    enddo
    call BC(0,bndx)
    call BC(0,bndy)
...
subroutine INITIAL()
  ...
  do i= -1,ndx
     if( dsqrt((x(i) - hcen)**2+(y(j) - hcen)**2) .le. rsph) then
        Phiexa(i,j) = G4pi/4.0d0 * dinit1 * ((x(i) - hcen)**2+(y(j) - hcen)**2) !pi*G*rho*r^2
        write(142,*) sngl(x(i)),sngl(y(j)) ,  sngl(Phiexa(i,j))
        meanphi=meanphi+G4pi/2.0d0 * dinit1 * (x(i) - hcen )**2
     else
        Phiexa(i,j) = G4pi/4.0d0 * dinit1 * rsph **2 / dlog(rsph) *dlog(dsqrt((x(i) - hcen)**2+(y(j) - hcen)**2))
        write(142,*) sngl(x(i)),sngl(y(j)) , sngl(Phiexa(i,j))
        meanphi=meanphi+G4pi * dinit1 * h * dabs(x(i) - hcen)  - G4pi/2.0d0 * dinit1 * h**2
     end if
  ...
  do j=0,ndy-1
  do i=0,ndx-1
     Phigrd(i,j)=(-Phiexa(i-1,j)+Phiexa(i+1,j))*0.5d0/dx+(-Phiexa(i,j-1)+Phiexa(i,j+1))*0.5d0/dy
     !write(144,*) sngl(x(i)) , Phigrd(i) , Phiexa(i-1),Phiexa(i+1)
  end do
  Phigrd(-1,j)=(-Phiexa(0,j)+Phiexa(1,j))/dx
  Phigrd(ndx,j)=(Phiexa(ndx-1,j)-Phiexa(ndx-2,j))/dx
  ...
  do j=0,ndy-1
  Phigrd(-1,j)=(-Phiexa(0,j)+Phiexa(1,j))/dx+(-Phiexa(i,j-1)+Phiexa(i,j+1))*0.5d0/dy
  Phigrd(ndx,j)=(-Phiexa(ndx-1,j)+Phiexa(ndx-2,j))/dx+(-Phiexa(i,j-1)+Phiexa(i,j+1))*0.5d0/dy
  enddo
  do i=0,ndx-1
  Phigrd(i,-1)=(-Phiexa(i-1,j)+Phiexa(i+1,j))*0.5d0/dx+(-Phiexa(i,0)+Phiexa(i,1))/dy
  Phigrd(i,ndy)=(-Phiexa(i-1,j)+Phiexa(i+1,j))*0.5d0/dx+(-Phiexa(i,ndy-1)+Phiexa(i,ndy-2))/dy
  enddo
  Phigrd(-1,-1)=(-Phiexa(0,j)+Phiexa(1,j))/dx +(-Phiexa(i,0)+Phiexa(i,1))/dy
  Phigrd(ndx,-1)=(-Phiexa(ndx-1,j)+Phiexa(ndx-2,j))/dx +(-Phiexa(i,0)+Phiexa(i,1))/dy
  Phigrd(-1,ndy)=(-Phiexa(0,j)+Phiexa(1,j))/dx+(-Phiexa(i,ndy-1)+Phiexa(i,ndy-2))/dy
  Phigrd(ndx,ndy)=(-Phiexa(ndx-1,j)+Phiexa(ndx-2,j))/dx+(-Phiexa(i,ndy-1)+Phiexa(i,ndy-2))/dy
  close(142)
  close(143)
  close(144)
  !200 continue
...
subroutine BC(modewp1,modewp2)
  use comvar
  use grvvar
  integer :: i,modewp1,modewp2
...
subroutine saveu(in1)
  ...
  do j=1,ndy-2
     do i=1,ndx-2
        write(21,*) x(i),',',y(j),',',wp1(i,j),',',wp2(i,j),',',rho(i,j),',',Phiexa(i,j)&
,',',Phigrd(i,j),',',wp2(i,j)-Phiexa(i,j),',',wp1(i,j)-Phigrd(i,j)
     end do
     write(21,*)
  end do
  close(21)
