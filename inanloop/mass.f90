...
!                    vy(n)=dble(U(i,j,k,3))+vy(n)
     ...
     !Massr(:,:)=Massr(:,:)*Msun
        ...
        open(650,file=dir//'Clmpanly/Massr'//timech//num//'.DAT',FORM='FORMATTED')
        do n=1,rgn
           if(cas(n)==1)then
              write(650,*) n,Massr(n,1),Massr(n,2),Massr(n,3),Massr(n,4),Massr(n,5)
           end if
           !write(510,*) tag(i,j,k)
        end do
        close(650)
