...
DOUBLE PRECISION  :: Mmassive=10.d0,cs0=0.19d0/0.953d0,rsphBE1,rdmod
INTEGER, parameter :: numstar=5000,valstar=14,nlpmass=100000
integer :: idum=1,nid=0,nidnw=0,isphloop=5000
...
ALLOCATE(Ustar(1:valstar,0:numstar))
...
IF(NRANK.EQ.0) THEN
open(11,FILE='/work/maedarn/3DMHD/samplecnv2/ST'//filenm//NPENUM//'.dat',FORM='FORMATTED') !,CONVERT='LITTLE_ENDIAN')
!100 format(D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3,D10.3)
    do j=1,numstar
...
!********************************  BC for the leftsides of domains  *****
  DO K = 1, N_MPI(20)
    ...
    IF((BCx1.eq.1).and.(IST.eq.0)) THEN
      DO KZ = -1, Ncellz+2; DO JY = -1, Ncelly+2; DO IX = 1-N_ol, 0
        U(IX,JY,KZ,N_MPI(K)) = U(1,JY,KZ,N_MPI(K))
      END DO;END DO;END DO
    END IF
...
!********************************  BC for the rightsides of domains  ****
  DO K = 1, N_MPI(20)
    ...
    IF((BCx2.eq.1).and.(IST.eq.NSPLTx-1)) THEN
      DO KZ = -1, Ncellz+2; DO JY = -1, Ncelly+2; DO IX = Ncellx+1, Ncellx+N_ol
        U(IX,JY,KZ,N_MPI(K)) = U(Ncellx,JY,KZ,N_MPI(K))
      END DO;END DO;END DO
    END IF
...
!*************************************  BC for the downsides of domains  ****
  ...
  IF((BCy1.eq.1).and.(JST.eq.0)) THEN
    DO KZ = -1, Ncellz+2; DO JY = 1-N_ol, 0; DO IX = -1, Ncellx+2
      U(IX,JY,KZ,N_MPI(K)) = U(IX,1,KZ,N_MPI(K))
    END DO;END DO;END DO
  END IF
...
!**************************************  BC for the upsides of domains  ****
  DO K = 1, N_MPI(20)
    ...
    IF((BCy2.eq.1).and.(JST.eq.NSPLTy-1)) THEN
      DO KZ = -1, Ncellz+2; DO JY = Ncelly+1, Ncelly+N_ol; DO IX = -1, Ncellx+2
        U(IX,JY,KZ,N_MPI(K)) = U(IX,Ncelly,KZ,N_MPI(K))
      END DO;END DO;END DO
    END IF
  ...
  DO K = 1, N_MPI(20)
    ...
    IF((BCz1.eq.1).and.(KST.eq.0)) THEN
      DO KZ = 1-N_ol, 0; DO JY = -1, Ncelly+2; DO IX = -1, Ncellx+2
        U(IX,JY,KZ,N_MPI(K)) = U(IX,JY,1,N_MPI(K))
      END DO;END DO;END DO
    END IF
...
!***********************************  BC for Lagrange coordinate  **********
  IF(N_MPI(10).eq.1) THEN
    ...
    IF((BCx1.eq.1).and.(IST.eq.0)) THEN
      DO KZ = -1, Ncellz+2; DO JY = -1, Ncelly+2; DO IX = 1-N_ol, 0
        dxlagM(IX,JY,KZ) = dxlagM(1,JY,KZ)
      END DO;END DO;END DO
    END IF
    ...
    IF((BCx2.eq.1).and.(IST.eq.NSPLTx-1)) THEN
      DO KZ = -1, Ncellz+2; DO JY = -1, Ncelly+2; DO IX = Ncellx+1, Ncellx+N_ol
        dxlagM(IX,JY,KZ) = dxlagM(Ncellx,JY,KZ)
      END DO;END DO;END DO
    END IF
  END IF
!********************************************************  BC for Bcc ******
...
!********************************************************  BC for Blg ******
  IF(N_MPI(12).eq.1) THEN
    DO K = 1, 2
      ...
      IF((BCx2.eq.1).and.(IST.eq.NSPLTx-1)) THEN
        DO KZ = -1, Ncellz+2; DO JY = -1, Ncelly+2; DO IX = Ncellx+1, Ncellx+N_ol
          Blg(IX,JY,KZ,K) = Blg(Ncellx,JY,KZ,K)
        END DO;END DO;END DO
      END IF
...
!        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8) 
    ...
    !        ndH(IX,JY,KZ)   = BBRV_cm(1); ndp(IX,JY,KZ) = BBRV_cm(2); ndH2(IX,JY,KZ) = BBRV_cm(3); ndHe(IX,JY,KZ) = BBRV_cm(4)
    !        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8)
            ndH(IX,JY,KZ)   = ndH(1,JY,KZ)   ; ndp(IX,JY,KZ)  = ndp(1,JY,KZ)
            ndH2(IX,JY,KZ)  = ndH2(1,JY,KZ)  ; ndHe(IX,JY,KZ) = ndHe(1,JY,KZ)
            ndHep(IX,JY,KZ) = ndHep(1,JY,KZ) ; ndC(IX,JY,KZ)  = ndC(1,JY,KZ)
            ndCO(IX,JY,KZ)  = ndCO(1,JY,KZ)  ; ndCp(IX,JY,KZ) = ndCp(1,JY,KZ)
          END DO;END DO;END DO
    END IF
...
!        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8)
    ...
    !        ndH(IX,JY,KZ)   = BBRV_cm(1); ndp(IX,JY,KZ) = BBRV_cm(2); ndH2(IX,JY,KZ) = BBRV_cm(3); ndHe(IX,JY,KZ) = BBRV_cm(4)
    !        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8)
            ndH(IX,JY,KZ)   = ndH(Ncellx,JY,KZ)   ; ndp(IX,JY,KZ)  = ndp(Ncellx,JY,KZ)
            ndH2(IX,JY,KZ)  = ndH2(Ncellx,JY,KZ)  ; ndHe(IX,JY,KZ) = ndHe(Ncellx,JY,KZ)
            ndHep(IX,JY,KZ) = ndHep(Ncellx,JY,KZ) ; ndC(IX,JY,KZ)  = ndC(Ncellx,JY,KZ)
            ndCO(IX,JY,KZ)  = ndCO(Ncellx,JY,KZ)  ; ndCp(IX,JY,KZ) = ndCp(Ncellx,JY,KZ)
          END DO; END DO; END DO
    END IF
  END IF
!***************************************************************************
...
!***********************************  BC for Lagrange coordinate  **********
  IF(N_MPI(10).eq.1) THEN
    ...
    IF((BCy1.eq.1).and.(JST.eq.0)) THEN
      DO KZ = -1, Ncellz+2; DO JY = 1-N_ol, 0; DO IX = -1, Ncellx+2
        dxlagM(IX,JY,KZ) = dxlagM(IX,1,KZ)
      END DO;END DO;END DO
    END IF

    IF((BCy2.eq.1).and.(JST.eq.NSPLTy-1)) THEN
      DO KZ = -1, Ncellz+2; DO JY = Ncelly+1, Ncelly+N_ol; DO IX = -1, Ncellx+2
        dxlagM(IX,JY,KZ) = dxlagM(IX,Ncelly,KZ)
      END DO;END DO;END DO
    END IF
...
!********************************************************  BC for Bcc ******
  IF(N_MPI(11).eq.1) THEN
    ...
    IF((BCy1.eq.1).and.(JST.eq.0)) THEN
        DO KZ = -1, Ncellz+2; DO JY = 1-N_ol, 0; DO IX = -1, Ncellx+2
          Bcc(IX,JY,KZ,K) = Bcc(IX,1,KZ,K)  !!! divfree
        END DO;END DO;END DO
    END IF

    IF((BCy2.eq.1).and.(JST.eq.NSPLTy-1)) THEN
        DO KZ = -1, Ncellz+2; DO JY = Ncelly+1, Ncelly+N_ol; DO IX = -1, Ncellx+2
          Bcc(IX,JY,KZ,K) = Bcc(IX,Ncelly,KZ,K)  !!! divfree
        END DO;END DO;END DO
    END IF
...
!********************************************************  BC for Vfc ******
  IF(N_MPI(13).eq.1) THEN
    ...
    IF((BCy1.eq.1).and.(JST.eq.0)) THEN
      DO KZ = -1, Ncellz+2; DO JY = 1-N_ol, 0; DO IX = -1, Ncellx+2
        Vfc(IX,JY,KZ,NV(K)) = Vfc(IX,1,KZ,NV(K))
      END DO;END DO;END DO
    END IF
...
!***********************************  BC for chemical species  **********
  ...
  IF((BCy1.eq.1).and.(JST.eq.0)) THEN
        DO KZ = 1, Ncellz; DO JY = 1-N_ol, 0; DO IX = 1, Ncellx
  !        ndH(IX,JY,KZ)   = BBRV_cm(1); ndp(IX,JY,KZ) = BBRV_cm(2); ndH2(IX,JY,KZ) = BBRV_cm(3); ndHe(IX,JY,KZ) = BBRV_cm(4)
  !        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8)
          ndH(IX,JY,KZ)   = ndH(IX,1,KZ)   ; ndp(IX,JY,KZ)  = ndp(IX,1,KZ)
          ndH2(IX,JY,KZ)  = ndH2(IX,1,KZ)  ; ndHe(IX,JY,KZ) = ndHe(IX,1,KZ)
          ndHep(IX,JY,KZ) = ndHep(IX,1,KZ) ; ndC(IX,JY,KZ)  = ndC(IX,1,KZ)
          ndCO(IX,JY,KZ)  = ndCO(IX,1,KZ)  ; ndCp(IX,JY,KZ) = ndCp(IX,1,KZ)
        END DO;END DO;END DO
  END IF
  IF((BCy2.eq.1).and.(JST.eq.NSPLTy-1)) THEN
        DO KZ=1,Ncellz; DO JY=Ncelly+1,Ncelly+N_ol; DO IX=1,Ncellx
  !        ndH(IX,JY,KZ)   = BBRV_cm(1); ndp(IX,JY,KZ) = BBRV_cm(2); ndH2(IX,JY,KZ) = BBRV_cm(3); ndHe(IX,JY,KZ) = BBRV_cm(4)
  !        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8)
          ndH(IX,JY,KZ)   = ndH(IX,Ncelly,KZ)   ; ndp(IX,JY,KZ)  = ndp(IX,Ncelly,KZ)
          ndH2(IX,JY,KZ)  = ndH2(IX,Ncelly,KZ)  ; ndHe(IX,JY,KZ) = ndHe(IX,Ncelly,KZ)
          ndHep(IX,JY,KZ) = ndHep(IX,Ncelly,KZ) ; ndC(IX,JY,KZ)  = ndC(IX,Ncelly,KZ)
          ndCO(IX,JY,KZ)  = ndCO(IX,Ncelly,KZ)  ; ndCp(IX,JY,KZ) = ndCp(IX,Ncelly,KZ)
        END DO; END DO; END DO
  END IF
...
!***********************************  BC for Lagrange coordinate  **********
  IF(N_MPI(10).eq.1) THEN
    ...
    IF((BCz1.eq.1).and.(KST.eq.0)) THEN
      DO KZ = 1-N_ol, 0; DO JY = -1, Ncelly+2; DO IX = -1, Ncellx+2
        dxlagM(IX,JY,KZ) = dxlagM(IX,JY,1)
      END DO;END DO;END DO
    END IF

    IF((BCz2.eq.1).and.(KST.eq.NSPLTz-1)) THEN
      DO KZ = Ncellz+1, Ncellz+N_ol; DO JY = -1, Ncelly+2; DO IX = -1, Ncellx+2
        dxlagM(IX,JY,KZ) = dxlagM(IX,JY,Ncellz)
      END DO;END DO;END DO
    END IF
  END IF
!********************************************************  BC for Bcc ******
  IF(N_MPI(11).eq.1) THEN
    ...
    IF((BCz1.eq.1).and.(KST.eq.0)) THEN
        DO KZ = 1-N_ol, 0; DO JY = -1, Ncelly+2; DO IX = -1, Ncellx+2
          Bcc(IX,JY,KZ,K) = Bcc(IX,JY,1,K)  !!! divfree
        END DO;END DO;END DO
    END IF
...
!********************************************************  BC for Vfc ******
  IF(N_MPI(13).eq.1) THEN
    ...
    IF((BCz1.eq.1).and.(KST.eq.0)) THEN
      DO KZ = 1-N_ol, 0; DO JY = -1, Ncelly+2; DO IX = -1, Ncellx+2
        Vfc(IX,JY,KZ,NV(K)) = Vfc(IX,JY,1,NV(K))
      END DO;END DO;END DO
    END IF
...
!***********************************  BC for chemical species  **********
  IF(N_MPI(10).eq.10) THEN
    ...
    !        ndH(IX,JY,KZ)   = BBRV_cm(1); ndp(IX,JY,KZ) = BBRV_cm(2); ndH2(IX,JY,KZ) = BBRV_cm(3); ndHe(IX,JY,KZ) = BBRV_cm(4)
    !        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8)
            ndH(IX,JY,KZ)   = ndH(IX,JY,1)   ; ndp(IX,JY,KZ)  = ndp(IX,JY,1)
            ndH2(IX,JY,KZ)  = ndH2(IX,JY,1)  ; ndHe(IX,JY,KZ) = ndHe(IX,JY,1)
            ndHep(IX,JY,KZ) = ndHep(IX,JY,1) ; ndC(IX,JY,KZ)  = ndC(IX,JY,1)
            ndCO(IX,JY,KZ)  = ndCO(IX,JY,1)  ; ndCp(IX,JY,KZ) = ndCp(IX,JY,1)
          END DO;END DO;END DO
    END IF
    IF((BCz2.eq.1).and.(KST.eq.NSPLTz-1)) THEN
          DO KZ = Ncellz+1, Ncellz+N_ol; DO JY = -1, Ncelly+2; DO IX = -1, Ncellx+2
    !        ndH(IX,JY,KZ)   = BBRV_cm(1); ndp(IX,JY,KZ) = BBRV_cm(2); ndH2(IX,JY,KZ) = BBRV_cm(3); ndHe(IX,JY,KZ) = BBRV_cm(4)
    !        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8)
            ndH(IX,JY,KZ)   = ndH(IX,JY,Ncellz)   ; ndp(IX,JY,KZ)  = ndp(IX,JY,Ncellz)
            ndH2(IX,JY,KZ)  = ndH2(IX,JY,Ncellz)  ; ndHe(IX,JY,KZ) = ndHe(IX,JY,Ncellz)
            ndHep(IX,JY,KZ) = ndHep(IX,JY,Ncellz) ; ndC(IX,JY,KZ)  = ndC(IX,JY,Ncellz)
            ndCO(IX,JY,KZ)  = ndCO(IX,JY,Ncellz)  ; ndCp(IX,JY,KZ) = ndCp(IX,JY,Ncellz)
          END DO; END DO; END DO
    END IF
  END IF
!***************************************************************************
