...
integer :: countin = 0,ooo=0,oo=2,jMPI=5,jnoMPI=5
double precision :: msrs=1.0d5

!Pre-BC for rho is necessary
msrs=1.0d5
write(*,*) NRANK,U(0,0,0,1),U(1,1,1,1),U(Ncellx+1,Ncelly+1,Ncellz+1,1),'POSI1'
      ...
      IF(jj.lt.NGcr) THEN !*** generate residual from jj to jj-1 ****
         do jpre=1,NPRE+jnoMPI
            if(jpre==NPRE+jnoMPI) then
               write(*,*)
               write(*,*)
               write(*,*) '**********************itrmax*******************',NRANK,jpre
               write(*,*)
               write(*,*)
               msrs=0.0d0
               call relax(cphi1(point1(jj)),crhs1(point1(jj)),nfx,nfy,nfz,mode,msrs,jpre)
               goto 2999
            end if
            mode=2; if((jj.ne.j).and.(jpre.eq.1)) mode=1
            msrs=0.0d0
          call relax(cphi1(point1(jj)),crhs1(point1(jj)),nfx,nfy,nfz,mode,msrs,jpre)
          write(*,*) msrs , NRANK ,'????????????????????msrs1??????????????'
          if(msrs .lt. 1.0d-2) then
             !call relax(cphi1(point1(jj)),crhs1(point1(jj)),nfx,nfy,nfz,mode,4,1,NRANK,msrs,jpre)
             write(*,*) msrs,jpre,NRANK,'=========1==========='
             goto 2999
          end if
       end do
2999   continue
      ...
      ELSE
        ...
        do jpre=1,NPRE1+jMPI
           ...
           if(jpre==NPRE1+jMPI) then
              ...
              write(*,*)
              msrs=0.0d0
              call relaxMPI(cphi2(point2(jj)),crhs2(point2(jj)),nfx,nfy,nfz,mode,msrs,jpre)
              goto 2799
           endif
           msrs=0.0d0
           call relaxMPI(cphi2(point2(jj)),crhs2(point2(jj)),nfx,nfy,nfz,mode,msrs,jpre)
           write(*,*) msrs ,jpre,j,NGL,NPRE1, NRANK ,'????????????????????msrs2??????????????'
           if(msrs .lt. 1.0d-2) then
              !           call relaxMPI(cphi2(point2(jj)),crhs2(point2(jj)),nfx,nfy,nfz,mode,4,1,msrs,jpre)
              write(*,*)  msrs,jpre,NRANK,'=========2==========='
              goto 2799
           end if
        end do
2799    continue
        ...
        if(jj.eq.NGcr) then
          ...
          do jpost=1,NPOST+jMPI
             if(jpost==NPOST+jMPI) then
                ...
                write(*,*)
                msrs=0.0d0
                call relaxMPI(cphi2(point2(jj)),crhs2(point2(jj)),nfx,nfy,nfz,2,msrs,jpost)
                goto 2699
             endif
             msrs=0.0d0
             call relaxMPI(cphi2(point2(jj)),crhs2(point2(jj)),nfx,nfy,nfz,2,msrs,jpost)
...
2699      continue
          ...
          do jpost=1,NPOST+jnoMPI
             if(jpost==NPOST+jnoMPI) then
                ...
                write(*,*)
                msrs=0.0d0
                call relax(cphi1(point1(jj)),crhs1(point1(jj)),nfx,nfy,nfz,2,msrs,jpost)
                goto 2899
             end if
             msrs=0.0d0
             call relax(cphi1(point1(jj)),crhs1(point1(jj)),nfx,nfy,nfz,2,msrs,jpost)
...
SUBROUTINE relaxMPI(u,rhs,nx,ny,nz,mode,msrs,itrr)
USE mpivar
USE slfgrv
double precision, parameter :: w=1.d0/6.d0
double precision u(0:nx,0:ny,0:nz),rhs(0:nx,0:ny,0:nz),uold(0:nx,0:ny,0:nz)
double precision h,h2,srs,msrs
integer :: itrr,itrmax=0,itrnew=0
    ...
    do k=1,nz-1
      do j=1,ny-1
        do i=isw,nx-1,2
          ...
          *dble(1-ifl) + dble(0.5d0+dsign(0.5d0,dble(ifl)-0.5d0))*u(i,j,k)
...
do k=1,nz-1
   do j=1,ny-1
      do i=1,nx-1
         !if(NRANK==40) then
         !   write(*,*)i,j,k, u(i,j,k),uold(i,j,k),u(i,j,k)-uold(i,j,k)
         !end if
         if(uold(i,j,k).ne.0.0d0) then
            srs=dabs((u(i,j,k)-uold(i,j,k))/(uold(i,j,k))+0.0d-30)
         !   if (NRANK==40) then
               write(*,*) NRANK,srs
         !  end if
            msrs=dmax1(srs,msrs)
...
if(NRANK==40) then
   write(*,*) NRANK,itrr,itrmax,msrs,'========itrMPIrlx40=========='
end if

if(itrmax .ne. itrnew) then
   write(*,*) NRANK,itrr,itrmax,msrs,'========itrMPIrlx=========='
   itrnew=itrmax
end if
END SUBROUTINE relaxMPI


SUBROUTINE relax(u,rhs,nx,ny,nz,mode,msrs,itrr)
USE slfgrv
double precision, parameter :: w=1.d0/6.d0
