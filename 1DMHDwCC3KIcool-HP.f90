...
double precision  :: ndpmin,ndHmin,ndH2min,ndHemin,ndHepmin,ndCmin,ndCpmin,ndCOmin,aaaa
...
double precision :: pi,kwave,xp,kz,Bz,vzinp , haba ,kwave2
integer :: bzstrt , bzfin

open(8,file=dir//'INPUT3.DAT')
  read(8,*)  Np1x,Np2x
  read(8,*)  ql1x,ql2x
...
goto 99
do i = -1 , Np1x
   U(i,2) = vinitx1 * dexp(-(x(i)-250.d0)**2.0d0 / 400.d0 )
enddo
do i = Np1x+1 , Ncell+2
   U(i,2) = vinitx2 * dexp(-(x(i)-350.d0)**2.0d0 / 400.d0 )
enddo
99 continue
!**************hennebelle2**************!
...
haba = 1.0d0 / 1.0d0
do i = -1 , Ncell+2
  U(i,2) = - vinitx1 * dtanh(haba * (x(i) - 10.d0))
enddo
!88 continue
!**************hennebelle2**************!
...
!**************purtub sin**************!
goto 77
kwave2  =  (2.0d0*3.1415d0) / 1.9530d0
aaaa = 2.0d0
do i = 462 , 562
   U(i,1) =dinit1* 0.05d0*( dsin(kwave2*( x(i)-9.02d0)) ) + dinit1
   ndH(i) = Hini1 * 0.05d0*( dsin(kwave2*( i-400)) ) + Hini1
   ! aaaa = dmin1(aaaa,U(i,1))
end do
77 continue
!**************purtub sin**************!
...
integer :: nunit,msig,i
double precision  :: dt,t(10000) , a, priv(1:1024)
character(4) filenm
a=0.0d0
priv(:) = 0.0d0
!do i = 505 , 519
...
write(filenm,'(I4.4)') nunit
100 format(E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3, &
         E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3,E19.10e3) !再利用のため100をつけておく
...
open(10,file=dir//title//filenm//'.dat')
   write(10,100) ( 0.5d0*(x(i)+x(i-1)),U(i,1),U(i,2),U(i,3),U(i,4),U(i,5),U(i,6),U(i,7),U(i,8), &
                   ndH(i),ndp(i),ndH2(i),ndHe(i),ndHep(i),ndC(i),ndCO(i),ndCp(i),a,i=1,Ncell )
close(10)

if(priv(512) > U(512,2)) write(*,*) nunit , '*******err*******'
...
t(nunit) = time
open(3,file=dir//title//'time.DAT')
  do i = 1, nunit
    write(3,'(d25.17)') t(i)
  end do
close(3)
...
USE comvar
double precision :: tCFL,c2 ,aaa,bbb,ccc
...
double precision :: rtTx,tcd,CooL,aaaaa
...
do i = 0, Ncell+1
   !N(i) = ndH(i) + ndHe(i)
   N(i) = U(i,1) * 0.7874d0 !個数密度
   Tn(i) = U(i,5)/( kb*N(i)) !温度
   Pn(i) = U(i,5) !圧力
end do
do i = 0, Ncell
...
!----- Cooling ---------------------------------------------------------
  ...
  gammi1 =   3.d0*(ndH(i)+ndp(i)+ndHe(i)+ndHep(i))+5.d0*ndH2(i)
  gammi1 = ( 2.d0*(ndH(i)+ndp(i)+ndHe(i)+ndHep(i))+2.d0*ndH2(i) )/gammi1
  if (i == 500) write(*,*) U(i,5) , CooL*dt*gammi1
 ! if(U(i,5) .le. 6.0d0) CooL =0.0d0
  if( dt .le. 0.2d0*Pn(i)/(gammi1*dabs(CooL)) ) then
