...
open(10,file=dir//filenm//'.dat')
   ...
   !write(*,*) '***************************'
!do i=1,Ncell
 !  write(*,*)  0.5d0*(x(i)+x(i-1)),U(i,1),U(i,2),U(i,3),U(i,4),U(i,5),U(i,6),U(i,7),U(i,8)
!enddo
 write(*,*) '***************************'
t(nunit) = time
open(3,file=dir//'time.DAT')
...
!from riemann solver
!===============================================================
real(8)  g1 , g2 , g3 , g4 , g5 , g6 , g7 , g8 , cl , cr
 real(8) cup , gel , ger  , pmaxx , pminn , ppv , pq 
 real(8) ptl , ptr , qmax , quser , um2
 real(8) changee , fl , fld , fr , frd  , pold , pstart
 real(8) diaph , domlen , ds  , pm , mpa , ps , s , timeout , um , us
  real(8) xpos
...
!from riemann solver
!===============================================================
cl = sqrt(gamma * P1i / D1i)
cr = sqrt(gamma * P2i / D2i)

quser = 2.0d0
nriter = 100

  cup = 0.25d0 * (D1i + D2i) * (cl + cr)
  ppv = 0.5d0 * (P1i + P2i) + 0.5d0 * (V1i - V2i) * cup
  ppv = DMAX1(0.0d0 , ppv)   !https://jp.xlsoft.com/documents/intel/cvf/vf-html/az/az08_07.htm
  pminn = DMIN1(P1i , P2i) !小さい方 D : real(8)
  pmaxx = DMAX1(P1i , P2i)
  qmax = pmaxx/pminn

  if(qmax <= quser .AND. (pminn <= ppv .AND. ppv <= pmaxx)) then
     ...
     if(ppv <= pminn) then
        ...
        pq = (P1i/P21) ** g1
        um = (pq*V1i/cl + V2i/cr + g4*(pq - 1.0d0))/(pq/cl + 1.0d0/cr)
        ptl = 1.0d0 + g7*(V1i - um)/cl
        ptr = 1.0d0 + g7*(um - V2i)/cr
        pstart = 0.5d0 * (P1i*ptl**g3 + P2i*ptr**g3)

     else

        gel = sqrt((g5/D1i)/(g6*P1i + ppv))
        ger = sqrt((g5/D2i)/(g6*P2i + ppv))
        pstart = (gel*P1i + ger*P2i - (V2i - V1i))/(gel + ger)
  ...
  tolpre = 1.0d-6
   ...
   do  i = 1 , nriter
     ...
     if(pold <= P1i) then
        ...
        qrt = sqrt(ak/(bk + pold))
        fl = (pold - P1i)*qrt
        fld = (1.0d0 - 0.5d0*(pold - P1i)/(bk + pold))
     ...
     if(pold <= P2i) then
        ...
        qrt = sqrt(ak/(bk + pold))
        fr = (pold - P2i)*qrt
        frd = (1.0d0 - 0.5d0*(pold - P2i)/(bk + pold))

     end if


     pm = pold - (fl + fr + udiff)/(fld + frd)
     changee = 2.0d0 * abs((pm - pold)/(pm + pold))
     !write(*,*) i , changee

     if(changee <= tolpre) exit
     if(pm <= 0.0) pm = tolpre
     pold = pm

  end do
  Pai = pm
  Vai = 0.5d0 * (V1i + V2i + fr - fl)
