...
subroutine slvmuscle(dt)
  ...
  double precision :: rho(-1:ndx,-1:ndy,-1:ndz),Phipre1(-1:ndx,-1:ndy,-1:ndz)
  ...
  double precision  grdxt1,grdyt1,grdzt1,grdkt1!,grdxt1,grdyt1,grdzt1
...
!   call muslcslv1D(Phiwv(-1,-1,-1,2),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,3),dt*0.5d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,4),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,5),dt*0.5d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,6),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,7),dt*0.5d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,8),dt*0.5d0,2)
   !call BCgrv(100,1,8)
   iwx=0;iwy=1;iwz=0
   call BCgrv(100,1,8)
   call muslcslv1D(Phiwv(-1,-1,-1,1),dt*0.25d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,2),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,3),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,4),dt*0.5d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,5),dt*0.5d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,6),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,7),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,8),dt*0.5d0,1)
   !call BCgrv(100,1,8)
   iwx=0;iwy=0;iwz=1
   call BCgrv(100,1,8)
   call muslcslv1D(Phiwv(-1,-1,-1,1),dt*0.25d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,2),dt*0.5d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,3),dt*0.5d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,4),dt*0.5d0,1)
!   call muslcslv1D(Phiwv(-1,-1,-1,5),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,6),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,7),dt*0.5d0,2)
!   call muslcslv1D(Phiwv(-1,-1,-1,8),dt*0.5d0,2)
   !call BCgrv(100,1,8)
...
!grdxy1=Phiwv(i,j+1,k,1)-Phiwv(i,j,k,1)-Phiwv(i-1,j+1,k,1)+Phiwv(i-1,j,k,1)
!+Phiwv(i+1,j,k,1)-Phiwv(i,j,k,1)-Phiwv(i+1,j-1,k,1)+Phiwv(i,j-1,k,1)


!grdxy1=(9.d0*Phiwv(i,j,k,1)-12.d0*Phiwv(i,j-1,k,1)+3.d0*Phiwv(i,j-2,k,1)-12.d0*Phiwv(i-1,j,k,1) &
!+16.d0*Phiwv(i-1,j-1,k,1)-4.d0*Phiwv(i-1,j-2,k,1)+3.d0*Phiwv(i-2,j,k,1)&
!-4.d0*Phiwv(i-2,j-1,k,1)+Phiwv(i-2,j-2,k,1))*0.25d0!Backward
!grdxy1=Phiwv(i,j,k,1)-Phiwv(i,j-1,k,1)-Phiwv(i-1,j,k,1)+Phiwv(i-1,j-1,k,1) !Backward

!grdxy1=(9.d0*Phiwv(i,j,k,1)-12.d0*Phiwv(i,j+1,k,1)+3.d0*Phiwv(i,j+2,k,1)-12.d0*Phiwv(i+1,j,k,1) &
!+16.d0*Phiwv(i+1,j+1,k,1)-4.d0*Phiwv(i+1,j+2,k,1)+3.d0*Phiwv(i+2,j,k,1)&
!-4.d0*Phiwv(i+2,j+1,k,1)+Phiwv(i+2,j+2,k,1))*0.25d0 !forward

!grdxy1mn=grdxy1

!grdxy1zp=(9.d0*Phiwv(i,j,k+1,1)-12.d0*Phiwv(i,j-1,k+1,1)+3.d0*Phiwv(i,j-2,k+1,1)-12.d0*Phiwv(i-1,j,k+1,1) &
!+16.d0*Phiwv(i-1,j-1,k+1,1)-4.d0*Phiwv(i-1,j-2,k+1,1)+3.d0*Phiwv(i-2,j,k+1,1)&
!-4.d0*Phiwv(i-2,j-1,k+1,1)+Phiwv(i-2,j-2,k+1,1))*0.25d0 !Backward
!grdxy1zm=(9.d0*Phiwv(i,j,k+1,1)-12.d0*Phiwv(i,j-1,k+1,1)+3.d0*Phiwv(i,j-2,k+1,1)-12.d0*Phiwv(i-1,j,k+1,1) &
!+16.d0*Phiwv(i-1,j-1,k+1,1)-4.d0*Phiwv(i-1,j-2,k+1,1)+3.d0*Phiwv(i-2,j,k+1,1)&
!-4.d0*Phiwv(i-2,j-1,k+1,1)+Phiwv(i-2,j-2,k+1,1))*0.25d0 !Backward
!grdxy1mn=(grdxy1+grdxy1zp+grdxy1zm)/3.d0
...
!grdyz1=(9.d0*Phiwv(i,j,k,1)-12.d0*Phiwv(i,j,k-1,1)+3.d0*Phiwv(i,j,k-2,1)-12.d0*Phiwv(i,j-1,k,1) &
!+16.d0*Phiwv(i,j-1,k-1,1)-4.d0*Phiwv(i,j-1,k-2,1)+3.d0*Phiwv(i,j-2,k,1)&
!-4.d0*Phiwv(i,j-2,k-1,1)+Phiwv(i,j-2,k-2,1))*0.25d0 !Backward
!grdyz1=Phiwv(i,j,k,1)-Phiwv(i,j,k-1,1)-Phiwv(i,j-1,k,1)+Phiwv(i,j-1,k-1,1) !Backward

!grdyz1=(9.d0*Phiwv(i,j,k,1)-12.d0*Phiwv(i,j,k+1,1)+3.d0*Phiwv(i,j,k+2,1)-12.d0*Phiwv(i,j+1,k,1) &
!+16.d0*Phiwv(i,j+1,k+1,1)-4.d0*Phiwv(i,j+1,k+2,1)+3.d0*Phiwv(i,j+2,k,1)&
!-4.d0*Phiwv(i,j+2,k+1,1)+Phiwv(i,j+2,k+2,1))*0.25d0 !forward

!grdyz1mn=grdyz1

!grdzx1=(9.d0*Phiwv(i,j,k,1)-12.d0*Phiwv(i-1,j,k,1)+3.d0*Phiwv(i-2,j,k,1)-12.d0*Phiwv(i,j,k-1,1) &
!+16.d0*Phiwv(i-1,j,k-1,1)-4.d0*Phiwv(i-2,j,k-1,1)+3.d0*Phiwv(i,j,k-2,1)&
!-4.d0*Phiwv(i-1,j,k-2,1)+Phiwv(i-2,j,k-2,1))*0.25d0 !Backward
!grdzx1=Phiwv(i,j,k,1)-Phiwv(i-1,j,k,1)-Phiwv(i,j,k-1,1)+Phiwv(i-1,j,k-1,1) !Backward

!grdzx1=(9.d0*Phiwv(i,j,k,1)-12.d0*Phiwv(i+1,j,k,1)+3.d0*Phiwv(i+2,j,k,1)-12.d0*Phiwv(i,j,k+1,1) &
!+16.d0*Phiwv(i+1,j,k+1,1)-4.d0*Phiwv(i+2,j,k+1,1)+3.d0*Phiwv(i,j,k+2,1)&
!-4.d0*Phiwv(i+1,j,k+2,1)+Phiwv(i+2,j,k+2,1))*0.25d0 !forward
...
!grdxy1zp=(-(-Phiwv(i+2,j+2,k+1,1)+8.d0*Phiwv(i+2,j+1,k+1,1)-8.d0*Phiwv(i+2,j-1,k+1,1)+Phiwv(i+2,j-2,k+1,1))/12.d0 &
!     +8.d0*(-Phiwv(i+1,j+2,k+1,1)+8.d0*Phiwv(i+1,j+1,k+1,1)-8.d0*Phiwv(i+1,j-1,k+1,1)+Phiwv(i+1,j-2,k+1,1))/12.d0 &
!     -8.d0*(-Phiwv(i-1,j+2,k+1,1)+8.d0*Phiwv(i-1,j+1,k+1,1)-8.d0*Phiwv(i-1,j-1,k+1,1)+Phiwv(i-1,j-2,k+1,1))/12.d0 &
!          +(-Phiwv(i-2,j+2,k+1,1)+8.d0*Phiwv(i-2,j+1,k+1,1)-8.d0*Phiwv(i-2,j-1,k+1,1)+Phiwv(i-2,j-2,k+1,1))/12.d0 &
!          )/12.d0*2.d0/3.d0
!grdxy1zm=(-(-Phiwv(i+2,j+2,k-1,1)+8.d0*Phiwv(i+2,j+1,k-1,1)-8.d0*Phiwv(i+2,j-1,k-1,1)+Phiwv(i+2,j-2,k-1,1))/12.d0 &
!     +8.d0*(-Phiwv(i+1,j+2,k-1,1)+8.d0*Phiwv(i+1,j+1,k-1,1)-8.d0*Phiwv(i+1,j-1,k-1,1)+Phiwv(i+1,j-2,k-1,1))/12.d0 &
!     -8.d0*(-Phiwv(i-1,j+2,k-1,1)+8.d0*Phiwv(i-1,j+1,k-1,1)-8.d0*Phiwv(i-1,j-1,k-1,1)+Phiwv(i-1,j-2,k-1,1))/12.d0 &
!          +(-Phiwv(i-2,j+2,k-1,1)+8.d0*Phiwv(i-2,j+1,k-1,1)-8.d0*Phiwv(i-2,j-1,k-1,1)+Phiwv(i-2,j-2,k-1,1))/12.d0 &
!          )/12.d0*2.d0/3.d0

!grdxy1zp=(-(-Phiwv(i+2,j+2,k+2,1)+8.d0*Phiwv(i+2,j+1,k+2,1)-8.d0*Phiwv(i+2,j-1,k+2,1)+Phiwv(i+2,j-2,k+2,1))/12.d0 &
!     +8.d0*(-Phiwv(i+1,j+2,k+2,1)+8.d0*Phiwv(i+1,j+1,k+2,1)-8.d0*Phiwv(i+1,j-1,k+2,1)+Phiwv(i+1,j-2,k+2,1))/12.d0 &
!     -8.d0*(-Phiwv(i-1,j+2,k+2,1)+8.d0*Phiwv(i-1,j+1,k+2,1)-8.d0*Phiwv(i-1,j-1,k+2,1)+Phiwv(i-1,j-2,k+2,1))/12.d0 &
!          +(-Phiwv(i-2,j+2,k+2,1)+8.d0*Phiwv(i-2,j+1,k+2,1)-8.d0*Phiwv(i-2,j-1,k+2,1)+Phiwv(i-2,j-2,k+2,1))/12.d0 &
!          )/12.d0*(-1.d0/6.d0)+grdxy1zp
!grdxy1zm=(-(-Phiwv(i+2,j+2,k-2,1)+8.d0*Phiwv(i+2,j+1,k-2,1)-8.d0*Phiwv(i+2,j-1,k-2,1)+Phiwv(i+2,j-2,k-2,1))/12.d0 &
!     +8.d0*(-Phiwv(i+1,j+2,k-2,1)+8.d0*Phiwv(i+1,j+1,k-2,1)-8.d0*Phiwv(i+1,j-1,k-2,1)+Phiwv(i+1,j-2,k-2,1))/12.d0 &
!     -8.d0*(-Phiwv(i-1,j+2,k-2,1)+8.d0*Phiwv(i-1,j+1,k-2,1)-8.d0*Phiwv(i-1,j-1,k-2,1)+Phiwv(i-1,j-2,k-2,1))/12.d0 &
!          +(-Phiwv(i-2,j+2,k-2,1)+8.d0*Phiwv(i-2,j+1,k-2,1)-8.d0*Phiwv(i-2,j-1,k-2,1)+Phiwv(i-2,j-2,k-2,1))/12.d0 &
!          )/12.d0*(-1.d0/6.d0)+grdxy1zm
!grdxy1mn=(grdxy1)*2.d0/3.d0+(grdxy1zp+grdxy1zm)/3.d0!/3.d0
!grdxy1mn=(grdxy1)

!grdyz1xp=(-(-Phiwv(i+1,j+2,k+2,1)+8.d0*Phiwv(i+1,j+2,k+1,1)-8.d0*Phiwv(i+1,j+2,k-1,1)+Phiwv(i+1,j+2,k-2,1))/12.d0 &
!     +8.d0*(-Phiwv(i+1,j+1,k+2,1)+8.d0*Phiwv(i+1,j+1,k+1,1)-8.d0*Phiwv(i+1,j+1,k-1,1)+Phiwv(i+1,j+1,k-2,1))/12.d0 &
!     -8.d0*(-Phiwv(i+1,j-1,k+2,1)+8.d0*Phiwv(i+1,j-1,k+1,1)-8.d0*Phiwv(i+1,j-1,k-1,1)+Phiwv(i+1,j-1,k-2,1))/12.d0 &
!          +(-Phiwv(i+1,j-2,k+2,1)+8.d0*Phiwv(i+1,j-2,k+1,1)-8.d0*Phiwv(i+1,j-2,k-1,1)+Phiwv(i+1,j-2,k-2,1))/12.d0 &
!          )/12.d0*2.d0/3.d0
!grdyz1xm=(-(-Phiwv(i-1,j+2,k+2,1)+8.d0*Phiwv(i-1,j+2,k+1,1)-8.d0*Phiwv(i-1,j+2,k-1,1)+Phiwv(i-1,j+2,k-2,1))/12.d0 &
!     +8.d0*(-Phiwv(i-1,j+1,k+2,1)+8.d0*Phiwv(i-1,j+1,k+1,1)-8.d0*Phiwv(i-1,j+1,k-1,1)+Phiwv(i-1,j+1,k-2,1))/12.d0 &
!     -8.d0*(-Phiwv(i-1,j-1,k+2,1)+8.d0*Phiwv(i-1,j-1,k+1,1)-8.d0*Phiwv(i-1,j-1,k-1,1)+Phiwv(i-1,j-1,k-2,1))/12.d0 &
!          +(-Phiwv(i-1,j-2,k+2,1)+8.d0*Phiwv(i-1,j-2,k+1,1)-8.d0*Phiwv(i-1,j-2,k-1,1)+Phiwv(i-1,j-2,k-2,1))/12.d0 &
!          )/12.d0*2.d0/3.d0

!grdyz1xp=(-(-Phiwv(i+2,j+2,k+2,1)+8.d0*Phiwv(i+2,j+2,k+1,1)-8.d0*Phiwv(i+2,j+2,k-1,1)+Phiwv(i+2,j+2,k-2,1))/12.d0 &
!     +8.d0*(-Phiwv(i+2,j+1,k+2,1)+8.d0*Phiwv(i+2,j+1,k+1,1)-8.d0*Phiwv(i+2,j+1,k-1,1)+Phiwv(i+2,j+1,k-2,1))/12.d0 &
!     -8.d0*(-Phiwv(i+2,j-1,k+2,1)+8.d0*Phiwv(i+2,j-1,k+1,1)-8.d0*Phiwv(i+2,j-1,k-1,1)+Phiwv(i+2,j-1,k-2,1))/12.d0 &
!          +(-Phiwv(i+2,j-2,k+2,1)+8.d0*Phiwv(i+2,j-2,k+1,1)-8.d0*Phiwv(i+2,j-2,k-1,1)+Phiwv(i+2,j-2,k-2,1))/12.d0 &
!          )/12.d0*(-1.d0/6.d0)+grdyz1xp
!grdyz1xm=(-(-Phiwv(i-2,j+2,k+2,1)+8.d0*Phiwv(i-2,j+2,k+1,1)-8.d0*Phiwv(i-2,j+2,k-1,1)+Phiwv(i-2,j+2,k-2,1))/12.d0 &
!     +8.d0*(-Phiwv(i-2,j+1,k+2,1)+8.d0*Phiwv(i-2,j+1,k+1,1)-8.d0*Phiwv(i-2,j+1,k-1,1)+Phiwv(i-2,j+1,k-2,1))/12.d0 &
!     -8.d0*(-Phiwv(i-2,j-1,k+2,1)+8.d0*Phiwv(i-2,j-1,k+1,1)-8.d0*Phiwv(i-2,j-1,k-1,1)+Phiwv(i-2,j-1,k-2,1))/12.d0 &
!          +(-Phiwv(i-2,j-2,k+2,1)+8.d0*Phiwv(i-2,j-2,k+1,1)-8.d0*Phiwv(i-2,j-2,k-1,1)+Phiwv(i-2,j-2,k-2,1))/12.d0 &
!          )/12.d0*(-1.d0/6.d0)+grdyz1xm
!grdyz1mn=(grdyz1)*2.d0/3.d0+(grdyz1xp+grdyz1xm)/3.d0!/3.d0
!grdyz1mn=(grdyz1)

!grdzx1yp=(-(-Phiwv(i+2,j+1,k+2,1)+8.d0*Phiwv(i+1,j+1,k+2,1)-8.d0*Phiwv(i-1,j+1,k+2,1)+Phiwv(i-2,j+1,k+2,1))/12.d0 &
!     +8.d0*(-Phiwv(i+2,j+1,k+1,1)+8.d0*Phiwv(i+1,j+1,k+1,1)-8.d0*Phiwv(i-1,j+1,k+1,1)+Phiwv(i-2,j+1,k+1,1))/12.d0 &
!     -8.d0*(-Phiwv(i+2,j+1,k-1,1)+8.d0*Phiwv(i+1,j+1,k-1,1)-8.d0*Phiwv(i-1,j+1,k-1,1)+Phiwv(i-2,j+1,k-1,1))/12.d0 &
!          +(-Phiwv(i+2,j+1,k-2,1)+8.d0*Phiwv(i+1,j+1,k-2,1)-8.d0*Phiwv(i-1,j+1,k-2,1)+Phiwv(i-2,j+1,k-2,1))/12.d0 &
!          )/12.d0*2.d0/3.d0
!grdzx1ym=(-(-Phiwv(i+2,j-1,k+2,1)+8.d0*Phiwv(i+1,j-1,k+2,1)-8.d0*Phiwv(i-1,j-1,k+2,1)+Phiwv(i-2,j-1,k+2,1))/12.d0 &
!     +8.d0*(-Phiwv(i+2,j-1,k+1,1)+8.d0*Phiwv(i+1,j-1,k+1,1)-8.d0*Phiwv(i-1,j-1,k+1,1)+Phiwv(i-2,j-1,k+1,1))/12.d0 &
!     -8.d0*(-Phiwv(i+2,j-1,k-1,1)+8.d0*Phiwv(i+1,j-1,k-1,1)-8.d0*Phiwv(i-1,j-1,k-1,1)+Phiwv(i-2,j-1,k-1,1))/12.d0 &
!          +(-Phiwv(i+2,j-1,k-2,1)+8.d0*Phiwv(i+1,j-1,k-2,1)-8.d0*Phiwv(i-1,j-1,k-2,1)+Phiwv(i-2,j-1,k-2,1))/12.d0 &
!          )/12.d0*2.d0/3.d0

!grdzx1yp=(-(-Phiwv(i+2,j+2,k+2,1)+8.d0*Phiwv(i+1,j+2,k+2,1)-8.d0*Phiwv(i-1,j+2,k+2,1)+Phiwv(i-2,j+2,k+2,1))/12.d0 &
!     +8.d0*(-Phiwv(i+2,j+2,k+1,1)+8.d0*Phiwv(i+1,j+2,k+1,1)-8.d0*Phiwv(i-1,j+2,k+1,1)+Phiwv(i-2,j+2,k+1,1))/12.d0 &
!     -8.d0*(-Phiwv(i+2,j+2,k-1,1)+8.d0*Phiwv(i+1,j+2,k-1,1)-8.d0*Phiwv(i-1,j+2,k-1,1)+Phiwv(i-2,j+2,k-1,1))/12.d0 &
!          +(-Phiwv(i+2,j+2,k-2,1)+8.d0*Phiwv(i+1,j+2,k-2,1)-8.d0*Phiwv(i-1,j+2,k-2,1)+Phiwv(i-2,j+2,k-2,1))/12.d0 &
!          )/12.d0*(-1.d0/6.d0)+grdzx1yp
!grdzx1ym=(-(-Phiwv(i+2,j-2,k+2,1)+8.d0*Phiwv(i+1,j-2,k+2,1)-8.d0*Phiwv(i-1,j-2,k+2,1)+Phiwv(i-2,j-2,k+2,1))/12.d0 &
!     +8.d0*(-Phiwv(i+2,j-2,k+1,1)+8.d0*Phiwv(i+1,j-2,k+1,1)-8.d0*Phiwv(i-1,j-2,k+1,1)+Phiwv(i-2,j-2,k+1,1))/12.d0 &
!     -8.d0*(-Phiwv(i+2,j-2,k-1,1)+8.d0*Phiwv(i+1,j-2,k-1,1)-8.d0*Phiwv(i-1,j-2,k-1,1)+Phiwv(i-2,j-2,k-1,1))/12.d0 &
!          +(-Phiwv(i+2,j-2,k-2,1)+8.d0*Phiwv(i+1,j-2,k-2,1)-8.d0*Phiwv(i-1,j-2,k-2,1)+Phiwv(i-2,j-2,k-2,1))/12.d0 &
!          )/12.d0*(-1.d0/6.d0)+grdzx1ym
!grdzx1mn=(grdzx1)*2.d0/3.d0+(grdzx1yp+grdzx1ym)/3.d0!/3.d0
!grdzx1mn=(grdzx1)
...
!grdxt1=-(Phiwv(i+1,j,k)-Phiwv(i,j,k)-Phipre1(i+1,j,k)+Phipre1(i,j,k) &
!-Phiwv(i,j,k)+Phiwv(i-1,j,k)+Phipre1(i,j,k)-Phipre1(i-1,j,k))

!grdyt1=-(Phiwv(i,j+1,k)-Phiwv(i,j,k)-Phipre1(i,j+1,k)+Phipre1(i,j,k) &
!-Phiwv(i,j,k)+Phiwv(i,j-1,k)+Phipre1(i,j,k)-Phipre1(i,j-1,k))

!grdzt1=-(Phiwv(i,j,k+1)-Phiwv(i,j,k)-Phipre1(i,j,k+1)+Phipre1(i,j,k) &
!-Phiwv(i,j,k)+Phiwv(i,j,k-1)+Phipre1(i,j,k)-Phipre1(i,j,k-1))

!grdkt1=(Phipre1(i+1,j,k)-2.d0*Phipre1(i,j,k)+Phipre1(i-1,j,k)+&
!        Phipre1(i,j+1,k)-2.d0*Phipre1(i,j,k)+Phipre1(i,j-1,k)+&
!        Phipre1(i,j,k+1)-2.d0*Phipre1(i,j,k)+Phipre1(i,j,k-1)  )
...
!+2.d0*Tdiff*2.d0*Tdiff*cg*grdxt1/dx1/dt &
!+2.d0*Tdiff*2.d0*Tdiff*cg*grdyt1/dy1/dt &
!+2.d0*Tdiff*2.d0*Tdiff*cg*grdzt1/dz1/dt &
!+2.d0*Tdiff*2.d0*Tdiff*cg*grdkt1/dt/dx1
-cg*cg*4.d0*Tdiff*Tdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-0.5d0/Tdiff * dt))
...
if(mode==100) then
  ...
  !   END DO;END DO;END DO
  !END IF
  !IF((IST.eq.NSPLTx-1)) then!).and.(mod(idm,2)==1)) THEN
...
!**************************************  BC for the upsides of domains  ****
   ...
   !   Phiwv(IX,JY,KZ,idm)=Phiwv(IX,JY-1,KZ,idm)
   !   END DO;END DO;END DO
   !END IF
   enddo
!***************************************************************************
...
!**************************************  BC for the upsides of domains  ****
  ...
  !   Phiwv(IX,JY,KZ,idm)=Phiwv(IX,JY,KZ-1,idm)
  !   END DO;END DO;END DO
  !END IF
   enddo
!***************************************************************************
...
if(mode==110) then
  ...
  !   END DO;END DO;END DO
  !END IF
  !IF((IST.eq.0)) then!.and.(mod(idm,2)==1)) THEN
  !   DO KZ = -1, Ncellz+2; DO JY = -1, Ncelly+2; DO IX = 1-N_ol, 0
...
!*************************************  BC for the downsides of domains  ****
   ...
   IF(JST.eq.0) THEN
        ...
        END DO;END DO;END DO
     END IF
   !IF(JST.eq.0) THEN!.and.(mod(idm,2)==1)) THEN
   !   DO KZ = -1, Ncellz+2; DO JY = 1-N_ol, 0; DO IX = -1, Ncellx+2
   !   Phigrdwv(IX,JY,KZ,idm)= Phigrdwv(IX,JY+1,KZ,idm)
   !   END DO;END DO;END DO
   !END IF
   enddo
!**************************************  BC for the upsides of domains  ****
...
!*************************************  BC for the downsides of domains  ****
  ...
  Phigrdwv(-1,-1,       1-N_ol,idm),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
   ...
   !IF(KST.eq.0) THEN
   !   DO KZ = 1-N_ol, 0; DO JY = -1, Ncelly+2; DO IX = -1, Ncellx+2
   !   Phigrdwv(IX,JY,KZ,idm)= Phigrdwv(IX,JY,KZ+1,idm)
   !   END DO;END DO;END DO
   !END IF
   enddo
!**************************************  BC for the upsides of domains  ****
