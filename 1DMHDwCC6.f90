...
INTEGER :: iflag,ifchem,ifthrm,ifrad , iflimit
INTEGER :: BCx1,BCx2
...
double precision :: pi,kwave,xp

open(8,file=dir//'INPUT2.DAT')
  read(8,*)  Np1x,Np2x
  read(8,*)  ql1x,ql2x
  ...
  read(8,*)  BCx1,BCx2
  read(8,*)  iflag,ifchem,ifthrm,ifrad,iflimit
close(8)
...
100 format(E19.10,E19.10,E19.10,E19.10,E19.10,E19.10,E19.10,E19.10,E19.10, &
           E19.10,E19.10,E19.10,E19.10,E19.10,E19.10,E19.10,E19.10) !再利用のため100をつけておく
...
CALL BC(Ncell,U(-1,1),BCx1,BCx2); call VLIMIT(Ncell,U(-1,1),grdQ(-1,1),dx,0,1,iflimit)
CALL BC(Ncell,U(-1,2),BCx1,BCx2); call VLIMIT(Ncell,U(-1,2),grdQ(-1,2),dx,0,1,iflimit)
CALL BC(Ncell,U(-1,5),BCx1,BCx2); call VLIMIT(Ncell,U(-1,5),grdQ(-1,5),dx,0,1,iflimit)
CALL BC(Ncell,U(-1,7),BCx1,BCx2); call VLIMIT(Ncell,U(-1,7),grdQ(-1,7),dx,0,1,iflimit)
CALL BC(Ncell,U(-1,8),BCx1,BCx2); call VLIMIT(Ncell,U(-1,8),grdQ(-1,8),dx,0,1,iflimit)
...
CALL BC(Ncell,U(-1,1),BCx1,BCx2); call VLIMIT(Ncell,U(-1,1),grdU(-1,1),dxlag,0,1,iflimit)
CALL BC(Ncell,U(-1,2),BCx1,BCx2); call VLIMIT(Ncell,U(-1,2),grdU(-1,2),dxlag,0,1,iflimit)
CALL BC(Ncell,U(-1,3),BCx1,BCx2); call VLIMIT(Ncell,U(-1,3),grdU(-1,3),dxlag,0,1,iflimit)
CALL BC(Ncell,U(-1,4),BCx1,BCx2); call VLIMIT(Ncell,U(-1,4),grdU(-1,4),dxlag,0,1,iflimit)
CALL BC(Ncell,U(-1,5),BCx1,BCx2); call VLIMIT(Ncell,U(-1,5),grdU(-1,5),dxlag,0,1,iflimit)
CALL BC(Ncell,U(-1,7),BCx1,BCx2); call VLIMIT(Ncell,U(-1,7),grdU(-1,7),dxlag,0,1,iflimit)
CALL BC(Ncell,U(-1,8),BCx1,BCx2); call VLIMIT(Ncell,U(-1,8),grdU(-1,8),dxlag,0,1,iflimit)
  
CALL BC(Ncell,  ndH(-1),BCx1,BCx2); call VLIMIT(Ncell,  ndH(-1),grdC(-1,1),dxlag,0,1,iflimit)
CALL BC(Ncell,  ndp(-1),BCx1,BCx2); call VLIMIT(Ncell,  ndp(-1),grdC(-1,2),dxlag,0,1,iflimit)
CALL BC(Ncell, ndH2(-1),BCx1,BCx2); call VLIMIT(Ncell, ndH2(-1),grdC(-1,3),dxlag,0,1,iflimit)
CALL BC(Ncell, ndHe(-1),BCx1,BCx2); call VLIMIT(Ncell, ndHe(-1),grdC(-1,4),dxlag,0,1,iflimit)
CALL BC(Ncell,ndHep(-1),BCx1,BCx2); call VLIMIT(Ncell,ndHep(-1),grdC(-1,5),dxlag,0,1,iflimit)
CALL BC(Ncell,  ndC(-1),BCx1,BCx2); call VLIMIT(Ncell,  ndC(-1),grdC(-1,6),dxlag,0,1,iflimit)
CALL BC(Ncell, ndCp(-1),BCx1,BCx2); call VLIMIT(Ncell, ndCp(-1),grdC(-1,7),dxlag,0,1,iflimit)
CALL BC(Ncell, ndCO(-1),BCx1,BCx2); call VLIMIT(Ncell, ndCO(-1),grdC(-1,8),dxlag,0,1,iflimit)
...
!======================================================================*
SUBROUTINE VLIMIT(Ncell,U,grdU,dx,i_sta,i_end,iflimit)

  
integer :: Ncell
double precision, parameter :: eps = 1.d-10
double precision  :: U(-1:Ncell+2), grdU(-1:Ncell+2), dx(-1:Ncell+2)
double precision  :: delp,delm,flmt , r
...
do i = i_sta, Ncell+i_end
  ...
  if(iflimit .eq. 0) then
     flmt = dmax1( 0.d0,(2.d0*delp*delm+eps)/(delp**2.0d0+delm**2.0d0+eps) )
     !write(*,*) 'van albada'
  end if
  if(iflimit .eq. 1) then
  flmt = dmax1( 0.d0,(r**2.0d0+r+eps)/(r**2.0d0+1.0d0+eps)) !van albada1
  end if
  if(iflimit .eq. 2) then
  flmt = dmax1(0.0d0 , dmin1(1.0d0 , r)) !minmod
  end if
  if(iflimit .eq. 3) then
  flmt = dmax1(0.0d0 , dmin1(dmax1(1.0d0,r),2.0d0,2*r)) !superbee
  end if
  if(iflimit .eq. 4) then
     flmt =dmax1(0.0d0 , 0.5d0*(r+1.0d0)*dmin1(dmin1(1.0d0, 4.0d0*r/(r+1.0d0)),dmin1(1.0d0,4.0d0/(r+1.0d0)))) !barth-jespersen
