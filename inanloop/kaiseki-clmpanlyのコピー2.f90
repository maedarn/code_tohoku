program main
  ...
  integer nx,ny,nz,rgn,zero,sloop,smesh,initime,iii,im
  ...
  character(1) num1
  double precision Lbox,Msun,dl,Mshd,ganmim1,Mtot,mu,Mh2tot
...
enddo
  ...
  do time1=initime,timeloop,timestep
     ...
     do m=1,loop
        ...
        open(150,file=dir//'tag/tag'//timech//num//'.DAT',access='stream',FORM='UNFORMATTED')
        write(*,*) num
        !m=1
        do k=1,nz
           do j=1,ny
           ...
           do j=1,ny
              do i=1,nz
                 ...
                 if(n.ne.0) then
                    ...
                    Mh(n)=Mh(n)+dble(U(i,j,k,9))*(dl**3)
                    Mh2(n)=Mh2(n)+dble(U(i,j,k,11))*(dl**3)
...
!443     continue
        ...
        !do n=2,6
           if(cas(n)==1)then
               Mh2tot=2.0d0*Mh2(n)*Msun+Mh2tot
              write(610,*) n,MM(n),div(n),emag(n),ekin(n),ethm(n),-egrv(n),&
                   ...
                   vdsp(n),Tmean(n)*1.d3,csmean(n),csmean1(n),MM(n)*2.d0*Mh2(n)/Mh(n),2.0d0*Mh2(n)*Msun,Mh(n)*Msun,&
                   Mh2tot
           end if
           !write(510,*) tag(i,j,k)
        end do
...
write(*,*)'kokoha?'
    ...
    !               (real(tag(i,j,k,ijk)),ijk=1,loop),(real(tagr(i,j,k,ijkl)),ijkl=1,5)!,(real(tagr(i,j,k,5,ijkl)),ijkl=1,5)
              ...
              write(100) U(i,j,k,1), (real(tag(i,j,k,ijk)),ijk=1,loop),real(tagr(i,j,k,1 )),real(tagr(i,j,k,2 ))&
                   ,real(tagr(i,j,k,3 )),real(tagr(i,j,k,4 )),real(tagr(i,j,k,5 ))
           end do
        end do
        write(*,*) k
     end do
