...
!DOUBLE PRECISION, dimension(:,:),allocatable :: prt
DOUBLE PRECISION, dimension(:,:),allocatable :: prt
integer :: prtnum=1000
...
subroutine mvprt
  ...
  if(mode==1) then
  CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
  CALL MPI_ALLREDUCE(prt(1,1),prtinf(1,1),prtnum,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,IERR)
  CALL MPI_BCAST(prtinf(1,1),6*prtnum,MPI_REAL8,0,MPI_COMM_WORLD,IERR) !6 is x,y,z,vx,vy,vz

  do iii = 1,prtnum
     if(prtinf(1,iii) > Lbox) then
        prtinf(1,iii)=prtinf(1,iii)-Lbox
     end if
     if(prtinf(1,iii) > 0) then
        prtinf(1,iii)=prtinf(1,iii)+Lbox
     end if
     if(prtinf(2,iii) > Lbox) then
        prtinf(2,iii)=prtinf(2,iii)-Lbox
     end if
     if(prtinf(2,iii) > 0) then
        prtinf(2,iii)=prtinf(2,iii)+Lbox
     end if
     if(prtinf(3,iii) > Lbox) then
        prtinf(3,iii)=prtinf(3,iii)-Lbox
     end if
     if(prtinf(3,iii) > 0) then
        prtinf(3,iii)=prtinf(3,iii)+Lbox
     end if
     Iprt=int(prtinf(1,iii)/(Lbox/dble(NSPLTx)))
     Jprt=int(prtinf(2,iii)/(Lbox/dble(NSPLTy)))
     Kprt=int(prtinf(3,iii)/(Lbox/dble(NSPLTz)))
     imprt=int(prtinf(1,iii)/dx)
     jmprt=int(prtinf(2,iii)/dy)
     kmprt=int(prtinf(3,iii)/dz)
!     imprt=int(prtinf(1,iii)/dx)-Iprt*Ncellx
!     jmprt=int(prtinf(2,iii)/dy)-Jprt*Ncelly
!     kmprt=int(prtinf(3,iii)/dz)-Kprt*Ncellz
     if((Iprt==IST).and.(Jprt==JST).and.(Kprt==KST)) then
        prt(:,iii)=prtinf(:,iii)
     else if
        prt(:,iii)=0.d0
     end if
  ...
  if(mode==2) then
     do iii = 1,prtnum
        ...
        jmprt=int(prtinf(2,iii)/dy)-Jprt*Ncelly
        kmprt=int(prtinf(3,iii)/dz)-Kprt*Ncellz
        vxp = U(imprt,jmprt,kmprt,2)*(prtinf(1,iii)-x(imprt))/dx+U(imprt+1,jmprt,kmprt,2)*(-prtinf(1,iii)+x(imprt+1))/dx&
             +U(imprt,jmprt,kmprt,2)*(prtinf(2,iii)-y(jmprt))/dx+U(imprt,jmprt+1,kmprt,2)*(-prtinf(2,iii)+y(jmprt+1))/dy&
             +U(imprt,jmprt,kmprt,2)*(prtinf(3,iii)-z(kmprt))/dx+U(imprt,jmprt,kmprt+1,2)*(-prtinf(3,iii)+z(kmprt+1))/dz
        vyp = U(imprt,jmprt,kmprt,3)*(prtinf(1,iii)-x(imprt))/dx+U(imprt+1,jmprt,kmprt,3)*(-prtinf(1,iii)+x(imprt+1))/dx&
             +U(imprt,jmprt,kmprt,3)*(prtinf(2,iii)-y(jmprt))/dx+U(imprt,jmprt+1,kmprt,3)*(-prtinf(2,iii)+y(jmprt+1))/dy&
             +U(imprt,jmprt,kmprt,3)*(prtinf(3,iii)-z(kmprt))/dx+U(imprt,jmprt,kmprt+1,3)*(-prtinf(3,iii)+z(kmprt+1))/dz
        vzp = U(imprt,jmprt,kmprt,4)*(prtinf(1,iii)-x(imprt))/dx+U(imprt+1,jmprt,kmprt,3)*(-prtinf(1,iii)+x(imprt+1))/dx&
             +U(imprt,jmprt,kmprt,4)*(prtinf(2,iii)-y(jmprt))/dx+U(imprt,jmprt+1,kmprt,3)*(-prtinf(2,iii)+y(jmprt+1))/dy&
             +U(imprt,jmprt,kmprt,4)*(prtinf(3,iii)-z(kmprt))/dx+U(imprt,jmprt,kmprt+1,3)*(-prtinf(3,iii)+z(kmprt+1))/dz

        prtinf(1,iii)=prtinf(1,iii)+dt*vxp !improve 2nd order
        prtinf(2,iii)=prtinf(2,iii)+dt*vyp
        prtinf(3,iii)=prtinf(3,iii)+dt*vzp
     end do
  end if
  ...
  if(mode==4) then
     !save with 3D data save
  end if
end subroutine mvprt
