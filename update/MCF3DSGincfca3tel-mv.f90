...
DOUBLE PRECISION, parameter :: cg=1.0d0,sourratio=0.5d0,Tdiff=20.d0,adiff=0.375d0,rratio=0.2d0,rmove=35.d0
double precision :: dx1,dy1,dz1,ddd,rrsph3,rrsph3x,rrsph3y,rrsph3z,vmove=0.02d0*cg/rmove
!integer ifevogrv,ifevogrv2
!character(25) :: dir='/work/maedarn/3DMHD/test/' !samplecnv2
character(48) :: dir='/work/maedarn/3DMHD/tel/cg1-T20-mash128-L100-mv/'
END MODULE comvar
...
do k = -1, Ncellz+2; do j = -1, Ncelly+2; do i = -1, Ncellx+2
   ...
   Phiwv(i,j,k,2)=Phiexa(i,j,k)
   Phiwv(i,j,k,3)=Phiexa(i,j,k)
   Phiwv(i,j,k,4)=Phiexa(i,j,k)
   Phiwv(i,j,k,5)=Phiexa(i,j,k)
   Phiwv(i,j,k,6)=Phiexa(i,j,k)
   Phiwv(i,j,k,7)=Phiexa(i,j,k)
   Phiwv(i,j,k,8)=Phiexa(i,j,k)
end do
end do
end do
...
do k = -1, Ncellz+2; do j = -1, Ncelly+2; do i = -1, Ncellx+2
   ...
   Phigrdwv(i,j,k,2)=cg*Phigrd(i,j,k,2)*2.d0*Tdiff+Phiwv(i,j,k,2)
   Phigrdwv(i,j,k,3)=cg*Phigrd(i,j,k,3)*2.d0*Tdiff+Phiwv(i,j,k,3)
   Phigrdwv(i,j,k,4)=cg*Phigrd(i,j,k,4)*2.d0*Tdiff+Phiwv(i,j,k,4)
   Phigrdwv(i,j,k,5)=cg*Phigrd(i,j,k,5)*2.d0*Tdiff+Phiwv(i,j,k,5)
   Phigrdwv(i,j,k,6)=cg*Phigrd(i,j,k,6)*2.d0*Tdiff+Phiwv(i,j,k,6)
   Phigrdwv(i,j,k,7)=cg*Phigrd(i,j,k,7)*2.d0*Tdiff+Phiwv(i,j,k,7)
   Phigrdwv(i,j,k,8)=cg*Phigrd(i,j,k,8)*2.d0*Tdiff+Phiwv(i,j,k,8)
end do
end do
end do
