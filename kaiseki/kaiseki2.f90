program main
  ...
  double precision , allocatable :: v(:,:,:,:),x(:),y(:),z(:),inputlength(:)
      ...
      do k=corestart,corestart+corexy-1,1
         ...
         do j=1,mesh
            do i=1,mesh
               ...
               v(i+IST*mesh,j+KST*mesh,kz,1)=dble(u(i,j,kz,1))
               v(i+IST*mesh,j+KST*mesh,kz,2)=dble(u(i,j,kz,2))
               v(i+IST*mesh,j+KST*mesh,kz,3)=dble(u(i,j,kz,3))
               v(i+IST*mesh,j+KST*mesh,kz,4)=dble(u(i,j,kz,4))
               v(i+IST*mesh,j+KST*mesh,kz,5)=dble(u(i,j,kz,5))
               v(i+IST*mesh,j+KST*mesh,kz,6)=dble(u(i,j,kz,6))
               v(i+IST*mesh,j+KST*mesh,kz,7)=dble(u(i,j,kz,7))
               v(i+IST*mesh,j+KST*mesh,kz,8)=dble(u(i,j,kz,8))
               v(i+IST*mesh,j+KST*mesh,kz,9)=dble(u(i,j,kz,9))
               v(i+IST*mesh,j+KST*mesh,kz,10)=dble(u(i,j,kz,10))
               v(i+IST*mesh,j+KST*mesh,kz,11)=dble(u(i,j,kz,11))
               v(i+IST*mesh,j+KST*mesh,kz,12)=dble(u(i,j,kz,12))
               v(i+IST*mesh,j+KST*mesh,kz,13)=dble(u(i,j,kz,13))
               v(i+IST*mesh,j+KST*mesh,kz,14)=dble(u(i,j,kz,14))
               v(i+IST*mesh,j+KST*mesh,kz,15)=dble(u(i,j,kz,15))
               v(i+IST*mesh,j+KST*mesh,kz,16)=dble(u(i,j,kz,16))
               v(i+IST*mesh,j+KST*mesh,kz,17)=dble(u(i,j,kz,17))
