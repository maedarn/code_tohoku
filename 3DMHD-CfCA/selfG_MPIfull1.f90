...
call collect( crho1(point1(NGcr)),crho2(point2(NGcr)),ncx,ncy,ncz,ncxcr,ncycr,nczcr ) !補完
...
call slvsmlb(cphi1(point1(1)),crho1(point1(1))) !BC set is necessary 行列反転
...
do j=2,ngrid
  ...
  IF(j.le.NGcr) THEN !*** generate candidate sol. from j-1 to j (upward) *** 
    ...
    if(j.eq.NGcr) then
      ...
      call divide( cphi2(point2(NGcr)),cphi1(point1(NGcr)),ncx,ncy,ncz,2**NGcr+1,2**NGcr+1,2**NGcr+1 ) !MPI用に分ける
...
Phi(i+2,j,k)deviEND SUBROUTINE interp
