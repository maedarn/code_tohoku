...
DOUBLE PRECISION  :: Tth=1.d10, tstep_min=2.5d-6
END MODULE comvar
...
double precision  :: time_CPU_insub(5)
END MODULE mpivar
...
write(*,*) 'sub_Time1',time_CPU_insub(2)-time_CPU_insub(1),time_CPU_insub(3)-time_CPU_insub(2),time_CPU_insub(4)-time_CPU_insub(3)&
,time_CPU_insub(5)-time_CPU_insub(4)
...
IF(NRANK.EQ.0) THEN
  ...
  close(3)
  !write(*,'(1p1d25.17,a,i8,a,1p1e11.3,a)') time,'  Step =',(nunit-1)*nitera,'  dt =', dt,stb(st)
END IF

nunit = nunit + 1

time_CPU_insub(3) = MPI_WTIME()
...
!Lamd = ndfgr(i,j,k)*nde(i,j,k)*ndtot(i,j,k)*6.06236d0 * (T**0.94d0) * ( pha**( 0.462628d0/(T**6.8d-2) ) )
...
!Lamdg = ndfgr(i,j,k)*ndtot(i,j,k)*ndtot(i,j,k)*dsqrt(T*1.d3)*3.8d-33*1.9733d27*(T*1.d3-Td)*(1.d0-0.8d0*dexp(-75.d0/(T*1.d3)))
...
!Gampe = ndfgr(i,j,k)*ndtot(i,j,k) * 2.56526d3 * G0*ATN1 * &
!       ( 7.382d-3*(T**0.7d0)/(1.d0+2.d-4*pha) + 4.9d-2/(1.d0+4.d-3*(pha**0.73d0)) )
