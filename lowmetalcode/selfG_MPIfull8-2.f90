...
!  open(10,FILE=dir//'ephi'//chnum//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
!  do k = -1, Ncellz+2
!     do j = -1, Ncelly+2
!        do i = -1, Ncellx+2
!           write(10,*) Phi(i,j,k)
...
! CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
 ...
 !-----------ckbc--------------
 if(NRANK==292) then
    open(10,FILE=dir//'xphi292.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   Phi(-1,1,1),Phi(0,1,1),Phi(1,1,1),Phi(2,1,1)
    close(10)
 end if
 if(NRANK==291) then
    open(10,FILE=dir//'xphi291.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   Phi(64-1,1,1),Phi(64+0,1,1),Phi(64+1,1,1),Phi(64+2,1,1)
    close(10)
 end if
 !-----------ckbc--------------
 !-----------ckbc--------------
 if(NRANK==292) then
    open(10,FILE=dir//'yphi292.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   Phi(1,-1,1),Phi(1,0,1),Phi(1,1,1),Phi(1,2,1)
    close(10)
 end if
 if(NRANK==284) then
    open(10,FILE=dir//'yphi284.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   Phi(1,64-1,1),Phi(1,64+0,1),Phi(1,64+1,1),Phi(1,64+2,1)
    close(10)
 end if
 !-----------ckbc--------------
 !-----------ckbc--------------
 if(NRANK==292) then
    open(10,FILE=dir//'zphi292.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   Phi(1,1,-1),Phi(1,1,0),Phi(1,1,1),Phi(1,1,2)
    close(10)
 end if
 if(NRANK==228) then
    open(10,FILE=dir//'zphi228.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   Phi(1,1,64-1),Phi(1,1,64+0),Phi(1,1,64+1),Phi(1,1,64+2)
    close(10)
 end if
  !-----------ckbc--------------
...
!WRITE(chnum,'(I4.4)') cntnum
!WRITE(NPENUM,'(I3.3)') NRANK
!open(10,FILE=dir//'phibc'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
!do k=0,ncz; kk= (ncy+1)*k
!do j=0,ncy; n = j+kk
!  jb  = JST*Ncelly + j
!  kbb = KST*Ncellz + k
...
!   write(10) dble(data(jb,kbb,1)),dble(data(jb,kbb,2))
  !bphi2(pointb2(NGL)+n,1) = dble(data(jb,kbb,1))
  !bphi2(pointb2(NGL)+n,2) = dble(data(jb,kbb,2))
!end do; end do
!cntnum=cntnum+1
