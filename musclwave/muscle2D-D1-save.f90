module comvar
  implicit none
  integer, parameter :: ndx=130,ndy=130,laststep=100000,istx=1,ienx=2,isty=1,ieny=2,svnum=100 !preiodic:ist=1,ien=2 , kotei:ist=2,ien=3 : ndx=130
  !double precision, parameter :: Lbox=1.0d2 , h=10.0d0 , hcen=50.0d0 , dinit1=1.29988444d0,w1=2.0d0
  integer :: iwx,iwy,iwz,bndx=5,bndy=6
...
program muscl1D
  ...
  do i=1,laststep
    ...
    !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    iwx=1;iwy=0
    call muslcslv1D(wp2,rho*0.25d0,dt,1,2)
    call BC(bndx)
    iwx=0;iwy=1
    call muslcslv1D(wp2,rho*0.25d0,dt,1,2)
    call BC(bndy)
    ...
    call BC(bndy)
    iwx=0;iwy=1
    call muslcslv1D(wp2,rho,dt*0.25d0,1,2)
    call BC(bndy)
    iwx=1;iwy=0
    call muslcslv1D(wp2,rho,dt*0.25d0,1,2)
    call BC(bndx)
...
subroutine saveu(in1)
  ...
  do j=1,ndy-2
     do i=1,ndx-2
        write(21,*) x(i),y(j),wp1(i,j),wp2(i,j),rho(i,j),Phiexa(i,j),Phigrd(i,j)
     end do
     write(21,*)
  end do
  close(21)
