program main
  implicit none
  integer i,j,k,val,m,loop,n,n1,n2,n3,dm,time1,timeloop,jump,timestep,cnt10,ijk
...
!  end do
  ...
  open(610,file='mdivhighden.DAT',FORM='FORMATTED')
  do n=1,rgn
     if(cas(n)==1)then
  ...
  open(910,file='masfunhighden.DAT',FORM='FORMATTED')
  do i=1,sloop
...
open(100,file='Allphymanytaghiden'//timech//'.DAT',FORM='FORMATTED')
!do k=1,nz,jump
!   do j=1,ny,jump
!      do i=1,nx,jump
...
U(i,j,k,8),U(i,j,k,9),sngl(Tn(i,j,k)*1.d3),vdotf(i,j,k),vdotfb(i,j,k),vdotfg(i,j,k),vdotfp(i,j,k),(real(tag(i,j,k,ijk)),ijk=1,loop)
