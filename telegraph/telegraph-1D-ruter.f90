module comvar
  implicit none
integer, parameter :: ndx=130,laststep=100,ist=1,ien=2,svnum=1,eta=2.d0 !preiodic:ist=1,ien=2 , kotei:ist=2,ien=3 : ndx=130
...
program muscl1D
  ...
  do i=1,laststep
     ...
     do ii=1,ndx-2
     Phicgp(ii)=Phicgp(ii)*dexp(-eta*dt*0.5d0)-1.d0/eta * Phi1step(ii)*(1.d0-dexp(-eta*dt*0.5d0))
     !write (*,*) Phicgp(ii)*dexp(-eta*dt*0.5d0)-1.d0/eta * Phi1step(ii)*(1.d0-dexp(-eta*dt*0.5d0))
     enddo

     call BC(156)
     do ii=1,ndx-2
     Phi1step(ii)=Phi1step(ii)+(Phicgp(ii+1)-2.d0*Phicgp(ii)+Phicgp(ii-1))/dx/dx * dt -G4pi*rho(ii)*dt
     write (*,*) (Phicgp(ii+1)-2.d0*Phicgp(ii)+Phicgp(ii-1))/dx/dx
     enddo
