...
DOUBLE PRECISION, parameter :: sourratio=0.5d0,adiff=0.25d0,rratio=0.20d0,rmove=0.25d0!rmove=0.35d0!,adiff=0.375d0,cg=1.0d0,
double precision :: dx1,dy1,dz1,ddd,rrsph3,rrsph3x,rrsph3y,rrsph3z,tratio=0.5d0,vmove=0.02d0,cg=1.d0,Tdiff=0.2d0,rncn=0.d0!vmove=0.02d0*cg/rmove
double precision :: kappa=1.d0/0.2d0,Msph1=0.d0,dntdiv
...
DOUBLE PRECISION  :: mass1,rhomean,alpharcm=3.d0*1.d-13
...
INTEGER :: MSTATUS(MPI_STATUS_SIZE),grv1
double precision, dimension(:,:), allocatable :: plane,rand
integer i3,i4,i2,i2y,i2z,rsph2,pls
double precision :: rrsph,rsph3,T0=1.d3
integer :: modegrv
...
open(8,file=dir//'INPUT3D.DAT')
  ...
  read(8,*)  rhoth,LSFE
  read(8,*)  nad
  read(8,*)  cg,Tdiff,rncn,ntdiv,svci
close(8)
write(*,*)'READ',NRANK
...
!      U(i,j,k,3) = 0.0d0
!      U(i,j,k,4) = 0.0d0
      U(i,j,k,5) = (1.d4/T0)*kb*U(j,j,k,1)/1.27d0
!      U(i,j,k,6) = 0.0d0
!      U(i,j,k,7) = 0.0d0
!      U(i,j,k,8) = 0.0d0
...
double precision  :: t(1000),dt, stt, tLMT, dt_mpi(0:1024), dt_gat(0:1024), time_CPU(3),dt_vfast_mpi(0:1024),dt_vfast_gat(0:1024),dt_vfast
double precision  :: tsave,dtsave,tsave2D,dtsave2D
integer :: nunit, st, st_mpi(0:1024), st_gat(0:2047), Time_signal,tcool_tv_ratio,itrcg
character*7 stb(3)
...
do in10 = 1, maxstp
  ...
  do in20 = 1, nitera
    ...
    dt_vfast_mpi(NRANK) = dmin1( dt_mpi(NRANK), CFL * tLMT )
    st_mpi(NRANK) = 1
    stt= dt_mpi(NRANK)
    ...
    CALL MPI_GATHER(st_mpi(NRANK),1,MPI_INTEGER, &
                    ...
                    0            ,MPI_COMM_WORLD,IERR)
    CALL MPI_GATHER(dt_vfast_mpi(NRANK),1,MPI_REAL8,   &
                    dt_vfast_gat       ,1,MPI_REAL8,   &
                    0            ,MPI_COMM_WORLD,IERR)
    ...
    IF(NRANK.EQ.0)  THEN
      ...
      do i_t = 0, NPE-1
        ...
        dt_vfast = dmin1( dt_vfast, dt_vfast_gat(i_t) )
        if(dt.lt.dtt) st = st_gat(i_t)
        dtt = dt
      end do
      write(*,*)'dt,dtvfast',dt,dt_vfast
    END IF
    CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
...
!***** Source parts 1*****
 ...
 !   if(ifgrv.eq.2) then; call SELFGRAVWAVE(dt*0.5d0,2); call SELFGRAVWAVE(dt*0.5d0,3) ; end if
    if(ifgrv.eq.2) then; call SELFGRAVWAVE(dt,2); end if
...
!end if
  ...
  open(98,file=dir//'PHI/PHIINI'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  do k = -1, Ncellz+2
  do j = -1, Ncelly+2
    write(98) (Phiwv(i,j,k,1),Phigrdwv(i,j,k,1),U(i,j,k,1), i=-1,Ncellx+2 )
  end do
  end do
  close(98)
