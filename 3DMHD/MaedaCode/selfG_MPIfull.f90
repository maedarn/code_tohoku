...
USE slfgrv
INCLUDE 'mpif.h'
REAL*8 :: cphi1(Need1),crho1(Need1),cres1(Need1),crhs1(Need1) !Unsplit
REAL*8 :: cphi2(Need2),crho2(Need2),cres2(Need2),crhs2(Need2) !MPI Split
REAL*8 :: tMPI(Need1,0:NPE-1)
...
INCLUDE 'mpif.h'
INTEGER :: MSTATUS(MPI_STATUS_SIZE)
real*8 :: u1(nx1,ny1,nz1),u2(0:nx2,0:ny2,0:nz2)
real*8 :: tMPI(0:nx2,0:ny2,0:nz2,0:NPE-1)
...
USE mpivar
real*8 :: u2(0:nx2,0:ny2,0:nz2),u1(nx1,ny1,nz1)
...
USE mpivar
real*8 :: uc(0:nx,0:ny,0:nz),uf(0:2*nx-1,0:2*ny-1,0:2*nz-1)
real*8, parameter :: w = 1.d0/12.d0
...
real*8 uc(nx,ny,nz),uf(2*nx-1,2*ny-1,2*nz-1)
real*8, parameter :: w = 1.d0/12.d0
...
USE mpivar
USE slfgrv
real*8 uf(0:nx,0:ny,0:nz),uc(0:nx/2+1,0:ny/2+1,0:nz/2+1)
...
USE slfgrv
real*8 uf(nx,ny,nz),uc(nx/2+1,ny/2+1,nz/2+1)
...
Real*8 res(0:nx,0:ny,0:nz),uc(0:nx/2+1,0:ny/2+1,0:nz/2+1),uf(0:nx,0:ny,0:nz)
...
Real*8 res(nx,ny,nz),uc(nx/2+1,ny/2+1,nz/2+1),uf(nx,ny,nz)
...
USE mpivar
USE slfgrv
real*8, parameter :: w=1.d0/6.d0
real*8 u(0:nx,0:ny,0:nz),rhs(0:nx,0:ny,0:nz)
real*8 h,h2
h=Lbox/((nx-1)*NSPLTx)
h2=h*h
...
USE mpivar
USE slfgrv
real*8 res(0:nx,0:ny,0:nz),rhs(0:nx,0:ny,0:nz),u(0:nx,0:ny,0:nz)
real*8 h,h2i
h=Lbox/((nx-1)*NSPLTx)
h2i=1.d0/(h*h)
...
USE slfgrv
real*8 res(nx,ny,nz),rhs(nx,ny,nz),u(nx,ny,nz)
real*8 h,h2i
h=Lbox/(nx-1)
h2i=1.d0/(h*h)
...
REAL*8, dimension(:,:),  allocatable :: dat1,dat2
complex*16, dimension(:), allocatable :: spe1,spe2
real*8 :: kap,temp1r,temp1i,temp2r,temp2i,facG,fac,dxx,dyy,dzz,zp1,zp2
real*8, dimension(:,:,:), allocatable :: fint0,fint1
