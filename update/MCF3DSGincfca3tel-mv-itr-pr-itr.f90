...
DOUBLE PRECISION, parameter :: sourratio=0.5d0,adiff=0.25d0,rratio=0.2d0,tratio=0.5d0,rmove=0.d0!rmove=0.35d0!,adiff=0.375d0,cg=1.0d0,
double precision :: dx1,dy1,dz1,ddd,rrsph3,rrsph3x,rrsph3y,rrsph3z,vmove=0.02d0,cg=1.d0,Tdiff=0.2d0!vmove=0.02d0*cg/rmove
INTEGER, parameter :: mvstp=2!cfratio=5
INTEGER ::ntdiv=5
!integer ifevogrv,ifevogrv2
!character(25) :: dir='/work/maedarn/3DMHD/test/' !samplecnv2
character(43) :: dir='/work/maedarn/3DMHD/tel/tel-mesh128-mv-1st/'
character(17)  :: svdir
END MODULE comvar
...
!rsph3 =dsqrt( (ql1x+0.5d0*dx1-x_i(i2))**2 + (ql1y+0.5d0*dy1-y_i(i2y))**2 + (ql1z+0.5d0*dz1-z_i(i2z))**2 )
   ...
   !   Phiexa(i,j,k)=G4pi/6.d0*dinit1*(rsph3)**2
      Phiexa(i,j,k)=dinit1*dexp(-rsph3**2.d0/rrsph3**2.d0)
      !U(i,j,k,1) = dinit1
      !write(*,*) 'in'
   else
