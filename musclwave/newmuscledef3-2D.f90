module comvar
  implicit none
  integer, parameter :: ndx=258,laststep=1000000,ist=1,ien=2,svnum=1000 !preiodic:ist=1,ien=2 , kotei:ist=2,ien=3 : ndx=130
...
module grvvar
  implicit none
  integer, parameter :: ndx2=258,ndy2=258 !パラメータ属性必要
  DOUBLE PRECISION , dimension(-1:ndx2,-1:ndy2) :: Phi,v!,w1,w2
...
!     end do
 ...
 !    call BC(1)
     call BC(2)
     do j=1,ndx-2
        Phidtn(j) = Phidtn(j) -  G4pi * cg * cg * dt * rho(j)
     end do
...
subroutine saveu(in1)
  ...
  do i=1,ndx-2
        write(21,*) x(i),Phidtn(i),fx(i),wp1(i),wp2(i)!Phi(i), v(i),wp1(i),wp2(i),xi1(i),xi2(i)!Phicgp(i) , Phicgm(i) ,rho(i)!,&
  end do
  close(21)
  write(*,*) 'save step : ',in1
  in1=in1+1
