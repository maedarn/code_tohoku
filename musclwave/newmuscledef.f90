module comvar
  implicit none
  integer, parameter :: ndx=130,laststep=10000,ist=1,ien=2,svnum=50 !preiodic:ist=1,ien=2 , kotei:ist=2,ien=3 : ndx=130
...
module grvvar
  ...
  DOUBLE PRECISION , dimension(-1:ndx2) :: Phi
...
program muscl1D
  ...
  integer :: i,sv=0,iws,ws=2,j
   DOUBLE PRECISION , dimension(-1:ndx2) :: Phidummy,Phidummy1,Phidummy2
...
!     call muslcslv1D(Phi,rho,dt,3)
     ...
     !call BC(1)
     ! write(*,*)'ok3'
      call muslcslv1D(Phidummy1,rho,dt,1)
      call muslcslv1D(Phidummy2,rho,dt,2)
      do j=1,ndx-2
         Phi(j)=0.5d0*(Phidummy1(j)+Phidummy2(j))+0.5d0*(Phidummy(j)-Phidt(j))
      end do
     call BC(1)
!     do j=1,ndx-2
...
subroutine saveu(in1)
  ...
  do i=1,ndx-2
        write(21,*) x(i),Phi(i), Phicgp(i) , Phicgm(i) ,rho(i)!,&
  end do
  close(21)
  write(*,*) 'save step : ',in1
  in1=in1+1
