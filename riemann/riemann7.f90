program main
  implicit none
  character filename*128 , initial*128
  ...
  timeout = 0.005d0 !はじめのout put time
  ...
  !open(unit=1 , file = 'riemann-ini.dat' , STATUS = 'unknown' )
  ...
  do ini = 1 , 5
     ...
     write (initial, '("riemann-ini", i1.1, ".dat")') ini
     open (1, file=initial, status='unknown')
     ...
     write (filename, '("riemanndata",i1.1, i3.3, ".dat")') ini,0
     ...
     if(qmax <= quser .AND. (pmin <= ppv .AND. ppv <= pmax)) then
        ...
        if(ppv <= pmin) then
           ...
           pq = (pl/pr) ** g1
           um2 = (pq*ul/cl + ur/cr + g4*(pq - 1.0d0))/(pq/cl + 1.0d0/cr)
     ...
     write(*,*) 'Divergence in Newton-Raphson iteration'
     ...
     do dt = 1 , tstep
        ...
        write (filename, '("riemanndata",i1.1, i3.3, ".dat")') ini , dt
        open (17, file=filename, status='replace')
