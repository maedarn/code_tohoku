...
double precision:: U1pre,ndHpre,ndppre,ndH2pre,ndHepre,ndHeppre,ndCpre,ndCOpre,ndCppre,ndepre,ndtotpre,Phistarpre
...
emag1 = 0.5d0 * ( U(i,j,k,6)**2.d0 + U(i,j,k,7)**2.d0 + U(i,j,k,8)**2.d0 )
ekin1 = 0.5d0 * ( U(i,j,k,2)**2.d0 + U(i,j,k,3)**2.d0 + U(i,j,k,4)**2.d0 ) * U(i,j,k,1)
...
SFE2=LSFE*dsqrt(U(i,j,k,1)/(4.04d0*1.d3)) * dt * 2.d0 * ndH2(i,j,k) / (2.d0 * ndH2(i,j,k)+ndH(i,j,k)+ndp(i,j,k)) * nlpnm2
...
Phistar(i,j,k,2) = U(i,j,k,2)
Phistar(i,j,k,3) = U(i,j,k,3)
Phistar(i,j,k,4) = U(i,j,k,4)
Phistar(i,j,k,5) = dble(IST*Ncellx+i)*dx1
Phistar(i,j,k,6) = dble(JST*Ncelly+j)*dy1
Phistar(i,j,k,7) = dble(KST*Ncellz+k)*dz1
...
!call ran0(ransfe,idum1)
nlpnm3=idint(0.5d0+dsign(0.5d0,1.d-15-SFE2))
nlpnm4=1
nmsv1=0.d0
rdenst1=0.d0
denst1=0.d0
...
Ustar(19,(nid+nidnw)*nlpnm)=Mstar1 !Mstarlp
...
!If massive star form, return low-mass star mass to gas.
!U(i,j,k,1)   = U1pre    * (1.d0 - dble(nlpnm1)) + U(i,j,k,1)   * dble(nlpnm1)
!ndH(i,j,k)   = ndHpre   * (1.d0 - dble(nlpnm1)) + ndH(i,j,k)   * dble(nlpnm1)
!ndp(i,j,k)   = ndppre   * (1.d0 - dble(nlpnm1)) + ndp(i,j,k)   * dble(nlpnm1)
!ndH2(i,j,k)  = ndH2pre  * (1.d0 - dble(nlpnm1)) + ndH2(i,j,k)  * dble(nlpnm1)
!ndHe(i,j,k)  = ndHepre  * (1.d0 - dble(nlpnm1)) + ndHe(i,j,k)  * dble(nlpnm1)
!ndHep(i,j,k) = ndHeppre * (1.d0 - dble(nlpnm1)) + ndHep(i,j,k) * dble(nlpnm1)
!ndC(i,j,k)   = ndCpre   * (1.d0 - dble(nlpnm1)) + ndC(i,j,k)   * dble(nlpnm1)
!ndCO(i,j,k)  = ndCOpre  * (1.d0 - dble(nlpnm1)) + ndCO(i,j,k)  * dble(nlpnm1)
!ndCp(i,j,k)  = ndCppre  * (1.d0 - dble(nlpnm1)) + ndCp(i,j,k)  * dble(nlpnm1)
!nde(i,j,k)   = ndepre   * (1.d0 - dble(nlpnm1)) + nde(i,j,k)   * dble(nlpnm1)
!ndtot(i,j,k) = ndtotpre * (1.d0 - dble(nlpnm1)) + ndtot(i,j,k) * dble(nlpnm1)
Phistar(i,j,k,1) = Phistarpre * (1.d0 - dble(nlpnm1)) + Phistar(i,j,k) * dble(nlpnm1)
enddo
!if(Mstar1-ranout<0) then
!endif
end do; end do; end do
call collectST()
