...
double precision, parameter :: Tmp1p=1.d-2,Tmp2p=1.d6
double precision :: Tmp1 = 1.0d-2,Tmp2 = 1.0d5,Tmin=1.d-2,Tmax=1.d2
double precision, parameter :: Rhomin=1.d-1,Rhomax=1.d4,CooLth=1.d-3,dRho1=1.d-5,dTmp1=1.d-4,Prsmin=9.d0,Prsmax=1.d3
...
double precision ::   N,AbsL,T0,Rho1h,Rho1l,Laml,Lamc,CooL,Prs1
...
dlogP = (-dlog10(Prsmin) + dlog10(Prsmax))/dble(ixp)
Tmp(0) = Tmin
Rho(0) = Rhomin
Prs(0) = Prsmin
dlogT1=dlog10(Tmin)
dlogRho1=dlog10(Rhomin)
dlogP1=dlog10(Prsmin)
...
!Tmpmid = Prs1/(Rho1/1.27d0*kb)       !Tmp(j)
!Tmp1   = Prs1/(Rho1/1.27d0*kb)-dTmp1 !Tmp(j)-dTmp1
!Tmp2   = Prs1/(Rho1/1.27d0*kb)+dTmp1 !Tmp(j)+dTmp1
!Tmpmid = Tmp(i)
!Tmp1 = Tmp(i)-dTmp1
!Tmp2 = Tmp(i)+dTmp1

!Rho1 = Prs1/(Tmpmid*kb/1.27d0)
!Rho1h= Prs1/(Tmp2*kb/1.27d0)
!Rho1l= Prs1/(Tmp1*kb/1.27d0)

!Rho1 = Rho(i)
!Rho1h= Rho(i)+dRho1
!Rho1l= Rho(i)-dRho1
!write(*,*) Rho1,Rho1h,Rho1l

!do j=0,ixt
!Tmpmid = Prs1/(Rho1/1.27d0*kb)  !Tmp(j)
!Tmp1 = Prs1/(Rho1l/1.27d0*kb) !Tmp(j)-dTmp1
!Tmp2 = Prs1/(Rho1h/1.27d0*kb) !Tmp(j)+dTmp1
!write(*,*) Tmpmid,Tmp1,Tmp2
 ...
 CooLmid = ( N * ( Laml + Lamc ) - 1.d0 ) * AbsL * N
 CooLmid = CooLmid!/N
 ...
 Laml = dmin1(Laml,5.d3)
 Lamc = 1.4d-2 * dsqrt(T*T0) * dexp( -9.2d1/(T*T0) )
 CooLmid1 = ( N * ( Laml + Lamc ) - 1.d0 ) * AbsL * N
 CooLmid1 = CooLmid!/N
 ...
 Laml = dmin1(Laml,5.d3)
 Lamc = 1.4d-2 * dsqrt(T*T0) * dexp( -9.2d1/(T*T0) )
 CooLmid2 = ( N* ( Laml + Lamc ) - 1.d0 ) * AbsL * N
 CooLmid2 = CooLmid2!/N
 ...
 CooLR(i,j,1)=CooLmid
 CooLR(i,j,2)=Rho1
 CooLR(i,j,3)=Tmpmid*1.d3*Rho1!Tmpmid
 !write(*,*)Rho1,Tmp(j)
 CooLR(i,j,4)=(CooLh-CooLl)/(Tmp2-Tmp1) * Tmpmid - (CooLmid1-CooLmid2)/(dRho1*2.d0) * Rho1 - CooLmid !(CooLh-CooLl)/(dRho1*2.d0)
 CooLR(i,j,5)=(CooLh-CooLl)/(Tmp2-Tmp1) * Tmpmid - CooLmid !(CooLh-CooLl)/(Tmp2-Tmp1)-Rho1/1.27/Tmpmid * (CooLmid1-CooLmid2)/(dRho1*2.d0)
 CooLR(i,j,6)  = 5.d0/3.d0 !( 5.d0*(ndHmd+ndpmd+ndHemd+ndHepmd+ndHmmd)+7.d0*ndH2md )/( 3.d0*(ndHmd+ndpmd+ndHemd+ndHepmd+ndHmmd)+5.d0*ndH2md )
...
open(120,file=dir//'Thermal_inst5_br_KI.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(130,file=dir//'Thermal_inst5_KI.dat' ,access='stream',FORM='FORMATTED')
do j=1,ixt
do i=1,ix
!write(120) CooLR(i,j,2),CooLR(i,j,3),&
write(120) Rho(i),Tmp(j),&
...
open(220,file=dir//'Thermal_inst5_br_Rho_KI.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
do i=1,ix
write(220) Rho(i)
!write(*,*) Rho(i)
enddo
...
open(230,file=dir//'Thermal_inst5_br_Tmp_KI.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
do j=1,ixt
write(230) Tmp(j)
!write(*,*) Tmp(j)
enddo
...
open(320,file=dir//'Thermal_inst5_br_dLdR_KI.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(330,file=dir//'Thermal_inst5_br_dLdT_KI.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(340,file=dir//'Thermal_inst5_br_R_KI.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(350,file=dir//'Thermal_inst5_br_T_KI.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(360,file=dir//'Thermal_inst5_br_gmm_KI.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
do i=1,ix
do j=1,ixt
write(320) CooLR(i,j,4)
write(330) CooLR(i,j,5)
write(340) CooLR(i,j,2)!CooLR(i,j,2)
write(350) CooLR(i,j,3)!CooLR(i,j,3)
