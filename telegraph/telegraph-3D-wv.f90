...
Phiwv(i,j,k,1) = Phiwv(i,j,k,1)*dexp(- kpdiff * 0.5d0 * dt)+Phigrdwv(i,j,k,1)/kpdiff*(1.d0-dexp(- kpdiff * 0.5d0 * dt))
!Phiwv(i,j,k,2) = Phiwv(i,j,k,2)*dexp(- kpdiff * 0.5d0 * dt)+Phigrdwv(i,j,k,2)/kpdiff*(1.d0-dexp(- kpdiff * 0.5d0 * dt))
!Phiwv(i,j,k,3) = Phiwv(i,j,k,3)*dexp(- kpdiff * 0.5d0 * dt)+Phigrdwv(i,j,k,3)/kpdiff*(1.d0-dexp(- kpdiff * 0.5d0 * dt))
!Phiwv(i,j,k,4) = Phiwv(i,j,k,4)*dexp(- kpdiff * 0.5d0 * dt)+Phigrdwv(i,j,k,4)/kpdiff*(1.d0-dexp(- kpdiff * 0.5d0 * dt))
!Phiwv(i,j,k,5) = Phiwv(i,j,k,5)*dexp(- kpdiff * 0.5d0 * dt)+Phigrdwv(i,j,k,5)/kpdiff*(1.d0-dexp(- kpdiff * 0.5d0 * dt))
!Phiwv(i,j,k,6) = Phiwv(i,j,k,6)*dexp(- kpdiff * 0.5d0 * dt)+Phigrdwv(i,j,k,6)/kpdiff*(1.d0-dexp(- kpdiff * 0.5d0 * dt))
!Phiwv(i,j,k,7) = Phiwv(i,j,k,7)*dexp(- kpdiff * 0.5d0 * dt)+Phigrdwv(i,j,k,7)/kpdiff*(1.d0-dexp(- kpdiff * 0.5d0 * dt))
!Phiwv(i,j,k,8) = Phiwv(i,j,k,8)*dexp(- kpdiff * 0.5d0 * dt)+Phigrdwv(i,j,k,8)/kpdiff*(1.d0-dexp(- kpdiff * 0.5d0 * dt))
...
Phigrdwv(i,j,k,1) = Phigrdwv(i,j,k,1)*dexp(-kpdiff * dt)+(kpdiff * Phiwv(i,j,k,1) &
-2.d0/kpdiff*cg*cg*grdxy1mn/dx1/dy1 &
-2.d0/kpdiff*cg*cg*grdyz1mn/dy1/dz1 &
-2.d0/kpdiff*cg*cg*grdzx1mn/dz1/dx1 &
-cg*cg/kpdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-kpdiff * dt))

!Phigrdwv(i,j,k,2) = Phigrdwv(i,j,k,2)*dexp(-kpdiff * dt)+(Phiwv(i,j,k,2) &
!-2.d0/kpdiff*cg*cg*grdxy2/dx1/dy1 &
!+2.d0/kpdiff*cg*cg*grdyz2/dy1/dz1 &
!+2.d0/kpdiff*cg*cg*grdzx2/dz1/dx1 &
!-cg*cg/kpdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-kpdiff * dt))

!Phigrdwv(i,j,k,3) = Phigrdwv(i,j,k,3)*dexp(-kpdiff * dt)+(Phiwv(i,j,k,3) &
!+2.d0/kpdiff*cg*cg*grdxy3/dx1/dy1 &
!+2.d0/kpdiff*cg*cg*grdyz3/dy1/dz1 &
!-2.d0/kpdiff*cg*cg*grdzx3/dz1/dx1 &
!-cg*cg/kpdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-kpdiff * dt))

!Phigrdwv(i,j,k,4) = Phigrdwv(i,j,k,4)*dexp(-kpdiff * dt)+(Phiwv(i,j,k,4) &
!+2.d0/kpdiff*cg*cg*grdxy4/dx1/dy1 &
!-2.d0/kpdiff*cg*cg*grdyz4/dy1/dz1 &
!+2.d0/kpdiff*cg*cg*grdzx4/dz1/dx1 &
!-cg*cg/kpdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-kpdiff * dt))

!Phigrdwv(i,j,k,5) = Phigrdwv(i,j,k,5)*dexp(-kpdiff * dt)+(Phiwv(i,j,k,5) &
!-2.d0/kpdiff*cg*cg*grdxy5/dx1/dy1 &
!+2.d0/kpdiff*cg*cg*grdyz5/dy1/dz1 &
!+2.d0/kpdiff*cg*cg*grdzx5/dz1/dx1 &
!-cg*cg/kpdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-kpdiff * dt))

!Phigrdwv(i,j,k,6) = Phigrdwv(i,j,k,6)*dexp(-kpdiff * dt)+(Phiwv(i,j,k,6) &
!-2.d0/kpdiff*cg*cg*grdxy6/dx1/dy1 &
!-2.d0/kpdiff*cg*cg*grdyz6/dy1/dz1 &
!-2.d0/kpdiff*cg*cg*grdzx6/dz1/dx1 &
!-cg*cg/kpdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-kpdiff * dt))

!Phigrdwv(i,j,k,7) = Phigrdwv(i,j,k,7)*dexp(-kpdiff * dt)+(Phiwv(i,j,k,7) &
!+2.d0/kpdiff*cg*cg*grdxy7/dx1/dy1 &
!-2.d0/kpdiff*cg*cg*grdyz7/dy1/dz1 &
!+2.d0/kpdiff*cg*cg*grdzx7/dz1/dx1 &
!-cg*cg/kpdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-kpdiff * dt))

!Phigrdwv(i,j,k,8) = Phigrdwv(i,j,k,8)*dexp(-kpdiff * dt)+(Phiwv(i,j,k,8) &
!+2.d0/kpdiff*cg*cg*grdxy8/dx1/dy1 &
!+2.d0/kpdiff*cg*cg*grdyz8/dy1/dz1 &
!-2.d0/kpdiff*cg*cg*grdzx8/dz1/dx1 &
!-cg*cg/kpdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-kpdiff * dt))
       enddo
     enddo
   enddo
...
if(mode==110) then
  ...
  IF(IST.eq.0) THEN
     ...
     !DO KZ = 1, Ncellz; DO JY = 1, Ncelly; DO IX = 1-N_ol, 1
     !Phigrdwv(IX,JY,KZ,idm)= bphigrdxl(JY,KZ,IX,idm)
     Phigrdwv(IX,JY,KZ,idm) = cg*Phigrd(IX,JY,KZ,idm)+Phiexa(IX,JY,KZ)*kpdiff
     END DO;END DO;END DO
  END IF
  enddo
  do idm=is,ie
...
do j=-1,ndy
do i=-1,0
   bphigrdxl(j,k,i,1)= cg*( bphigrdxl(j,k,i,1) &
                    +(-bphil(j-1,k,i)+bphil(j+1,k,i))*0.5d0/dy1+(-bphil(j,k-1,i)+bphil(j,k+1,i))*0.5d0/dz1)+bphil(j,k,i)*kpdiff
   bphigrdxl(j,k,i,2)= cg*(-bphigrdxl(j,k,i,2) &
                    -(-bphil(j-1,k,i)+bphil(j+1,k,i))*0.5d0/dy1+(-bphil(j,k-1,i)+bphil(j,k+1,i))*0.5d0/dz1)+bphil(j,k,i)*kpdiff
   bphigrdxl(j,k,i,3)= cg*( bphigrdxl(j,k,i,3) &
                    -(-bphil(j-1,k,i)+bphil(j+1,k,i))*0.5d0/dy1+(-bphil(j,k-1,i)+bphil(j,k+1,i))*0.5d0/dz1)+bphil(j,k,i)*kpdiff
   bphigrdxl(j,k,i,4)= cg*(-bphigrdxl(j,k,i,4) &
                    +(-bphil(j-1,k,i)+bphil(j+1,k,i))*0.5d0/dy1+(-bphil(j,k-1,i)+bphil(j,k+1,i))*0.5d0/dz1)+bphil(j,k,i)*kpdiff
   bphigrdxl(j,k,i,5)= cg*( bphigrdxl(j,k,i,5) &
                    +(-bphil(j-1,k,i)+bphil(j+1,k,i))*0.5d0/dy1-(-bphil(j,k-1,i)+bphil(j,k+1,i))*0.5d0/dz1)+bphil(j,k,i)*kpdiff
   bphigrdxl(j,k,i,6)= cg*(-bphigrdxl(j,k,i,6) &
                    -(-bphil(j-1,k,i)+bphil(j+1,k,i))*0.5d0/dy1-(-bphil(j,k-1,i)+bphil(j,k+1,i))*0.5d0/dz1)+bphil(j,k,i)*kpdiff
   bphigrdxl(j,k,i,7)= cg*( bphigrdxl(j,k,i,7) &
                    -(-bphil(j-1,k,i)+bphil(j+1,k,i))*0.5d0/dy1-(-bphil(j,k-1,i)+bphil(j,k+1,i))*0.5d0/dz1)+bphil(j,k,i)*kpdiff
   bphigrdxl(j,k,i,8)= cg*(-bphigrdxl(j,k,i,8) &
                    +(-bphil(j-1,k,i)+bphil(j+1,k,i))*0.5d0/dy1-(-bphil(j,k-1,i)+bphil(j,k+1,i))*0.5d0/dz1)+bphil(j,k,i)*kpdiff

   bphigrdxr(j,k,i+Ncellx+2,1)= cg*( bphigrdxr(j,k,i+Ncellx+2,1) &
                    +(-bphir(j-1,k,i+Ncellx+2)+bphir(j+1,k,i+Ncellx+2))*0.5d0/dy1+(-bphir(j,k-1,i+Ncellx+2)+bphir(j,k+1,i+Ncellx+2))*0.5d0/dz1)&
                    +bphir(j,k,i+Ncellx+2)*kpdiff
   bphigrdxr(j,k,i+Ncellx+2,2)= cg*(-bphigrdxr(j,k,i+Ncellx+2,2) &
                    -(-bphir(j-1,k,i+Ncellx+2)+bphir(j+1,k,i+Ncellx+2))*0.5d0/dy1+(-bphir(j,k-1,i+Ncellx+2)+bphir(j,k+1,i+Ncellx+2))*0.5d0/dz1)&
                    +bphir(j,k,i+Ncellx+2)*kpdiff
   bphigrdxr(j,k,i+Ncellx+2,3)= cg*( bphigrdxr(j,k,i+Ncellx+2,3) &
                    -(-bphir(j-1,k,i+Ncellx+2)+bphir(j+1,k,i+Ncellx+2))*0.5d0/dy1+(-bphir(j,k-1,i+Ncellx+2)+bphir(j,k+1,i+Ncellx+2))*0.5d0/dz1)&
                    +bphir(j,k,i+Ncellx+2)*kpdiff
   bphigrdxr(j,k,i+Ncellx+2,4)= cg*(-bphigrdxr(j,k,i+Ncellx+2,4) &
                    +(-bphir(j-1,k,i+Ncellx+2)+bphir(j+1,k,i+Ncellx+2))*0.5d0/dy1+(-bphir(j,k-1,i+Ncellx+2)+bphir(j,k+1,i+Ncellx+2))*0.5d0/dz1)&
                    +bphir(j,k,i+Ncellx+2)*kpdiff
   bphigrdxr(j,k,i+Ncellx+2,5)= cg*( bphigrdxr(j,k,i+Ncellx+2,5) &
                    +(-bphir(j-1,k,i+Ncellx+2)+bphir(j+1,k,i+Ncellx+2))*0.5d0/dy1-(-bphir(j,k-1,i+Ncellx+2)+bphir(j,k+1,i+Ncellx+2))*0.5d0/dz1)&
                    +bphir(j,k,i+Ncellx+2)*kpdiff
   bphigrdxr(j,k,i+Ncellx+2,6)= cg*(-bphigrdxr(j,k,i+Ncellx+2,6) &
                    -(-bphir(j-1,k,i+Ncellx+2)+bphir(j+1,k,i+Ncellx+2))*0.5d0/dy1-(-bphir(j,k-1,i+Ncellx+2)+bphir(j,k+1,i+Ncellx+2))*0.5d0/dz1)&
                    +bphir(j,k,i+Ncellx+2)*kpdiff
   bphigrdxr(j,k,i+Ncellx+2,7)= cg*( bphigrdxr(j,k,i+Ncellx+2,7) &
                    -(-bphir(j-1,k,i+Ncellx+2)+bphir(j+1,k,i+Ncellx+2))*0.5d0/dy1-(-bphir(j,k-1,i+Ncellx+2)+bphir(j,k+1,i+Ncellx+2))*0.5d0/dz1)&
                    +bphir(j,k,i+Ncellx+2)*kpdiff
   bphigrdxr(j,k,i+Ncellx+2,8)= cg*( -bphigrdxr(j,k,i+Ncellx+2,8) &
                    +(-bphir(j-1,k,i+Ncellx+2)+bphir(j+1,k,i+Ncellx+2))*0.5d0/dy1-(-bphir(j,k-1,i+Ncellx+2)+bphir(j,k+1,i+Ncellx+2))*0.5d0/dz1)&
                    +bphir(j,k,i+Ncellx+2)*kpdiff
