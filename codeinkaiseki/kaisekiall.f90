program computeraw
 ...
 character(3) NPE,time,cntc,cntctj
 DOUBLE PRECISION, parameter :: kb=8.63359d0
 character(46) :: dir='/glv0/maedarn/clst-form-HIcol/cnv100-wb-wg-sm/'
 !character(37) :: dir='/glv0/maedarn/dbug/debuggausssidelwb/'
                                         !debuggausssidel
 !character(54) :: dir='/glv0/maedarn/clst-form-HIcol/cnv100-wb-wg-sm-bnaname/'
...
!open(unit=250,file='NEW'//time//NPE//'.dat') !バイナリ
    ...
    !              if(u(i,j,k,1)>1.d3 ) then
                  ...
                  Tn(i+IST*(mesh-2),j+JST*(mesh-2),k+KST*(mesh-2)) = &
                       v(i,j,k,5)/(kb*(v(i,j,k,10)+v(i,j,k,9)+v(i,j,k,11)+v(i,j,k,12)+v(i,j,k,13)) )
    !              else
