...
write_hdf5_data()
{
    ...
    */
    ...
    //float *Phiexa = (float *) malloc((NX)*(NY)*(NZ) * sizeof(float));
    ...
    for(k=0;k<NZ;k++){
        for(j=0;j<NY;j++){
            ...
            for(n=0;n<NX;n++){
                ...
                fscanf(fp,"%f",&(Phiexa[tot])); //%f,%f,%f,%f,%f,%f,%f",\
                //&(rho[tot]),&(velocityx[tot]),&(velocityy[tot]),&(velocityz[tot]),&(pressure[tot]),&(Bfieldx[tot]),&(Bfieldy[tot]),&(Bfieldz[tot]));
            }
        }
    }
    ...
    float *dPhi = (float *) malloc((NX)*(NY)*(NZ) * sizeof(float));
    ...
    for(k=0;k<NZ;k++){
        for(j=0;j<NY;j++){
            ...
            for(n=0;n<NX;n++){
                ...
                fscanf(fp,"%f",&(dPhi[tot])); //%f,%f,%f,%f,%f,%f,%f",\
                //&(rho[tot]),&(velocityx[tot]),&(velocityy[tot]),&(velocityz[tot]),&(pressure[tot]),&(Bfieldx[tot]),&(Bfieldy[tot]),&(Bfieldz[tot]));
            }
        }
    }
    ...
    float *dprPhi = (float *) malloc((NX)*(NY)*(NZ) * sizeof(float));
    ...
    for(k=0;k<NZ;k++){
        for(j=0;j<NY;j++){
            ...
            for(n=0;n<NX;n++){
                ...
                fscanf(fp,"%f",&(dprPhi[tot])); //%f,%f,%f,%f,%f,%f,%f",\
                //&(rho[tot]),&(velocityx[tot]),&(velocityy[tot]),&(velocityz[tot]),&(pressure[tot]),&(Bfieldx[tot]),&(Bfieldy[tot]),&(Bfieldz[tot]));
            }
        }
    }
    ...
    float *rhoini = (float *) malloc((NX)*(NY)*(NZ) * sizeof(float));
    
    char filename3[50];
    sprintf(filename3, "INIT.DAT");
    ...
    for(k=0;k<NZ;k++){
        for(j=0;j<NY;j++){
            ...
            for(n=0;n<NX;n++){
                ...
                fscanf(fp,"%f",&(rhoini[tot])); //%f,%f,%f,%f,%f,%f,%f",\
                //&(rho[tot]),&(velocityx[tot]),&(velocityy[tot]),&(velocityz[tot]),&(pressure[tot]),&(Bfieldx[tot]),&(Bfieldy[tot]),&(Bfieldz[tot]));
            }
        }
    }
...
H5P_DEFAULT, x);
      ...
      //
      dataspace_id = H5Screate_simple(1, dims, NULL); //あってる？
...
/*
    ...
    status = H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, Phiexa);
    ...
    dataset_id = H5Dcreate(file_id, "/dPhi", H5T_NATIVE_FLOAT,dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    
    status = H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, dPhi);
    ...
    dataset_id = H5Dcreate(file_id, "/dprPhi", H5T_NATIVE_FLOAT,dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    
    status = H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, dprPhi);
    ...
    dataset_id = H5Dcreate(file_id, "/rhoini", H5T_NATIVE_FLOAT,dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    
    status = H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rhoini);
