...
open(8,file='/work/maedarn/3DMHD/INPUT3D.DAT')
  read(8,*)  Np1x,Np2x
  read(8,*)  Np1y,Np2y
  read(8,*)  Np1z,Np2z
  read(8,*)  ql1x,ql2x
...
IF(NRANK.EQ.0) THEN
  400 format(D25.17)
  open(4,file='/work/maedarn/3DMHD/cdnt.DAT')
...
open(2,file='/work/maedarn/3DMHD/tsave.DAT')
...
!**** read inhomogeneous density field ****!
  ...
  do MRANK = 0, NPE-1
    ...
    if((JS.eq.JST).and.(KS.eq.KST)) then
      ...
      open(unit=8,file='/work/maedarn/3DMHD/DTF/D'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
      do k = 1, Ncellz
      do j = 1, Ncelly
...
if(nunit.eq.1) goto 120
  ...
  open(unit=8,file='/work/maedarn/3DMHD/000'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  do k = 1, Ncellz+1
  do j = 1, Ncelly+1
...
open(2,file='/work/maedarn/3DMHD/tsave.DAT')
  ...
  read(2,*) nunit
close(2)
open(2,file='/work/maedarn/3DMHD/tsave2D.DAT')
  read(2,*) nunit2D
close(2)
open(3,file='/work/maedarn/3DMHD/time.DAT')
do i = 1, nunit
  read(3,'(1p1d25.17)') t(i)
end do
close(3)
!IF(NRANK.EQ.0) THEN
!  open(2,file='/work/maedarn/3DMHD/test.DAT')
!END IF

write(fnunit,'(I3.3)') nunit;  write(fnpe,'(I3.3)') NRANK
open(5,file='/work/maedarn/3DMHD/info'//fnunit//'.DAT')
!open(5,file='/work/maedarn/3DMHD/info'//fnunit//fnpe//'.DAT')
...
!open(10,file='/work/maedarn/3DMHD/'//filenm//NPENUM//'.dat')
open(10,FILE='/work/maedarn/3DMHD/'//filenm//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
...
!    close(5)
  ...
  open(8,file='/work/maedarn/3DMHD/000'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  do k = 1, Ncellz+1
  do j = 1, Ncelly+1
...
open(11,FILE='/work/maedarn/3DMHD/2D'//filenm//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  k=1; do j=1,Ncelly
...
IF(NRANK.EQ.0) THEN
  open(3,file='/work/maedarn/3DMHD/tsave2D.DAT')
    write(3,*) nunit2D
    write(3,*) time
  close(3)
END IF
