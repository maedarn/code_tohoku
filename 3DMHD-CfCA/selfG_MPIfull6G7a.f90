...
IF(NRANK.EQ.0) THEN
  ...
  open(4,file='/work/maedarn/3DMHD/test/cdnt.DAT')
    write(4,400) (  sngl(0.5d0 * ( x_i(i-1)+x_i(i) )), i=1, Ncellx*NSPLTx )
    write(4,400) (  sngl(0.5d0 * ( y_i(j-1)+y_i(j) )), j=1, Ncelly*NSPLTy )
    write(4,400) (  sngl(0.5d0 * ( z_i(k-1)+z_i(k) )), k=1, Ncellz*NSPLTz )
  close(4)
END IF
...
SUBROUTINE addintMPI(uf,uc,res,nx,ny,nz,NRANK)
...
isw=2 !for speed up  original
!isw=1 !for speed up  fordebug
!do jsw=2,1,-1
!   isw=jsw
do k=0,nz
  do j=0,ny
    do i=isw-1,nx,2
       ...
       !if(NRANK==40) then
       !   write(*,*) isw, i +(ny+1)*(j) + (k)*(ny+1)*(nz+1) ,'addintMPI'
       !end if
    end do
    isw=3-isw
  end do
...
!                 (1.0d0-dble(ifl)),rhs(i,j,k)
          ...
          !if(NRANK==40 .and. jsw==2) then
          !   write(*,*) isw, i,j,k, 'relaxMPImode1'
          !end if
       end do
       isw=3-isw
      end do
...
!                  (1.0d0-dble(ifl)),rhs(i,j,k),u(i+1,j,k)+u(i-1,j,k)+u(i,j+1,k)+u(i,j-1,k)+u(i,j,k+1)+u(i,j,k-1)!,u(i,j,k)-uold(i,j,k)
          ...
          !CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
          !if(NRANK==40 .and. jsw==2) then
          !   write(*,*) isw, i + (ny+1)*(j) + (k)*(ny+1)*(nz+1) ,'relaxMPI'
          !end if
       end do
       isw=3-isw
      end do
