...
double precision, PARAMETER :: xk_vis=40.d0,B_ex=5.d-4
!double precision, PARAMETER :: xk_vis=0.d0,B_ex=0.d0
END MODULE radtemp
...
pi=3.14159265358979d0,sigma_B=5.67d-5,xm_e=9.109534d-28
double precision :: yHe=8.333d-2
double precision :: T_ro=575.d0,c_light=2.99792458d10,h_Pl=6.62618d-27
integer, PARAMETER :: N_sp=50, maxit=100, N_p=100, N_react=675 !Species, itr in chemcool, , reaction
END MODULE
...
!-----INCLUDES------
!-------------------
include "subs2_n/chemistry.f90"
include "subs2_n/reaction.f90"  !full reactions
include "subs2_n/grain.f90"
include "subs2_n/xk_prm.f90"
include "subs2_n/H2.f90"
include "subs2_n/HD.f90"
include "subs2_n/lines.f90"
include "subs2_n/CR.f90"
include "subs2_n/math.f90"
include "subs2_n/CO.f90"
include "subs2_n/H2O.f90"
include "subs2_n/OH.f90"
...
double precision :: y(N_sp), t_ratio, dn_end

!---parameters---
Z_gas=1.0d-2 !gas metallicity
Z_dust=1.0d-2 !dust metallicity
!G_0=1.d0
!zeta=1.d-17
G_0=0.d0 !UV
zeta=0.d0 !ionization by CR
T_rad=10.d0
yD=3.d-5
!t_ratio=4.d-3 !for Z=1.d0
t_ratio=2.d-2 !timestep dt=t_ratio*t_cool
dn_end=1.d14 !final density
...
i_ev=1
itchem=0
t_chem=1.d-1 !timestep for chemistry
dt=1.d-1 !initial
!---parameters---
   
open(8,file='data2/nT_z2_nm1_GZ0v.dat',status='unknown')
open(11,file='data2/1_z2_nm1_GZ0v.dat',status='unknown')
open(12,file='data2/2_z2_nm1_GZ0v.dat',status='unknown')
!open(13,file='3.dat',status='unknown')    
open(14,file='data2/4_z2_nm1_GZ0v.dat',status='unknown')
open(15,file='data2/data_y_z2_nm1_GZ0v.dat',status='unknown')
...
do i=1,1000000
      ...
      call update(i_ev,rho,e,p,xnH,t_col,xLmbd_net,xmu,gamma,dt,T_K,t)
...
204   format(10e11.3)
      ...
      if(xLmbd_net.ne.0.d0) then
            t_cool=e/dabs(xLmbd_net) !cooling time
      else
            t_cool=1.d50
      endif
...
!----SUBROUTINES----
!-------------------
SUBROUTINE update(i_ev,rho,e,p,xnH,t_col,xLmbd_net,xmu,gamma,dt,T_K,t)
   USE PYSCONST
...
FUNCTION func(Tp,t,gamma,xmu,xLmbd,dt)
   USE PYSCONST
   IMPLICIT REAL*8(a-h,o-z)
func=Tp-t-(gamma-1.d0)*xmu*xm_p*xLmbd*dt/xk_B
return
END FUNCTION func
...
fL=func(Tp,Tp1,gamma,xmu,xLmbd,dt)

Tp2=1.01d0*Tp1
call cnt(Z_metal,xnH,Tp2,Tp_gr,&
     radius,A_v,esc_cnt,tau_cnt,xLmbd_cnt,xLmbd_gr)
xLmbd=xLmbd_ch+xLmbd_line+xLmbd_cnt+xLmbd_gr
f=func(Tp,Tp2,gamma,xmu,xLmbd,dt)
...
do it=1,maxit
   ...
   f=func(Tp,Tpsec,gamma,xmu,xLmbd1,dt)
   err=dabs(f/Tpsec)
...
xJ_ex=0.d0
call grtemp(xnH,T_K,esc_cnt,T_rad,xJ_ex,T_gr_K,xk_vis)
rho=(1.d0+4.d0*yHe)*xm_p*xnH
