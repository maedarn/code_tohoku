...
module grvvar
  ...
  DOUBLE PRECISION , dimension(-1-1:ndx2) :: x , Phicgm ,rho, Phi1step , Phi2step ,Phicgp
  DOUBLE PRECISION , dimension(-1-1:ndx2) :: Phidt,Phigrd,Phiexa,Phi
end module grvvar
...
subroutine timesource(Phiv,source,dt,mode)
  ...
  DOUBLE PRECISION, dimension(-1-1:ndx) :: Phiv,source
...
subroutine timesource2(Phiv,Phidt,source,dt)
  ...
  DOUBLE PRECISION, dimension(-1-1:ndx) :: Phiv,source,Phidt
  do i=1,ndx-2
     if((source(i) .ne. 0.0d0) .and. (Phiv(i) .ne. 0.0d0))then
...
subroutine muslcslv1D(Phiv,source,dt,mode)
  ...
  DOUBLE PRECISION, dimension(-1-1:ndx) :: Phigrad,Phipre,fluxphi,Phiv,source,Phi2dt,Phiu,sourcepre,sourcepri
  character(5) name
...
subroutine vanalbada(Phipre,Phigrad)
  ...
  integer :: i , ip , im
  DOUBLE PRECISION, dimension(-1-1:ndx) :: Phigrad,Phipre
...
subroutine fluxcal(preuse,pre,u,ep,kappa,mode)
  use comvar
  double precision :: ep , kappa
  DOUBLE PRECISION , dimension(-1-1:ndx) :: ul,ur,pre,slop,preuse,u
  integer :: i,mode
  ...
  DOUBLE PRECISION , dimension(-1-1:ndx) :: d3qp12
