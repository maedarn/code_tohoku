...
character(31) :: dir='/work/maedarn/3DMHD/samplecnv9/' !samplecnv2
END MODULE comvar
...
!**** read inhomogeneous density field ****!
 ...
 !-----thermal inst-----
  ...
  do k=1,Ncellz; do j=1,Ncelly; do i=1,Ncellx
    ...
    !ndHm(i,j,k)  = U(i,j,k,1)*BBRV_cm(9)  !Hmのreadとwrite
  end do;end do;end do
  119 continue
!--------------------------------
...
!************************************************************************
  ...
  !*************check part**************
  if(NRANK==292) then
     open(10,FILE=dir//'xbc292.dat',FORM='FORMATTED',position='append')
     write(10,*)  ccc , (U(-1,1,1,N_MPI(numloop)),U(0,1,1,N_MPI(numloop)),U(1,1,1,N_MPI(numloop)),U(2,1,1,N_MPI(numloop)),numloop=1,N_MPI(20))
     close(10)
  end if
  if(NRANK==291) then
     open(10,FILE=dir//'xbc291.dat',FORM='FORMATTED',position='append')
     write(10,*)   ccc , (U(64-1,1,1,N_MPI(numloop)),U(64+0,1,1,N_MPI(numloop)),U(64+1,1,1,N_MPI(numloop)),U(64+2,1,1,N_MPI(numloop)),numloop=1,N_MPI(20))
     close(10)
  end if
!  if(NRANK==219) then
  !     open(10,FILE=dir//'x'//chnum//bccnt//NPENUM//'.dat',FORM='UNFORMATTED',position='append')
!  write (10)  (U(i,j,k,numloop),numloop=1,N_MPI(20) )



!  WRITE(bccnt,'(I2.2)') N_MPI(20)
!  WRITE(chnum,'(I4.4)') cntnum
!  WRITE(NPENUM,'(I3.3)') NRANK
!  open(10,FILE=dir//'x'//chnum//bccnt//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
!  do k = -1, Ncellz+2
!     do j = -1, Ncelly+2
!        do i = -1, Ncellx+2
!           write(10,*) (U(i,j,k,numloop),numloop=1,N_MPI(20) )
...
!***************************************************************************
  ...
  if(NRANK==292) then
     open(10,FILE=dir//'ybc292.dat',FORM='FORMATTED',position='append')
     write(10,*) ccc ,   (U(1,-1,1,N_MPI(numloop)),U(1,0,1,N_MPI(numloop)),U(1,1,1,N_MPI(numloop)),U(1,2,1,N_MPI(numloop)),numloop=1,N_MPI(20))
     close(10)
  end if
  if(NRANK==284) then
     open(10,FILE=dir//'ybc284.dat',FORM='FORMATTED',position='append')
     write(10,*)  ccc ,  (U(1,64-1,1,N_MPI(numloop)),U(1,64+0,1,N_MPI(numloop)),U(1,64+1,1,N_MPI(numloop)),U(1,64+2,1,N_MPI(numloop)),numloop=1,N_MPI(20))
     close(10)
  end if
...
!  WRITE(chnum,'(I4.4)') cntnum
!  WRITE(NPENUM,'(I3.3)') NRANK
!  open(10,FILE=dir//'y'//chnum//bccnt//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
!  do k = -1, Ncellz+2
!     do j = -1, Ncelly+2
!        do i = -1, Ncellx+2
!           write(10,*) (U(i,j,k,numloop),numloop=1,N_MPI(20) )
...
!***************************************************************************
  ...
  if(NRANK==292) then
     open(10,FILE=dir//'zbc292.dat',FORM='FORMATTED',position='append')
     write(10,*)  ccc ,  (U(1,1,-1,N_MPI(numloop)),U(1,1,0,N_MPI(numloop)),U(1,1,1,N_MPI(numloop)),U(1,1,2,N_MPI(numloop)),numloop=1,N_MPI(20))
     close(10)
  end if
  if(NRANK==228) then
     open(10,FILE=dir//'zbc228.dat',FORM='FORMATTED',position='append')
     write(10,*)  ccc ,  (U(1,1,64-1,N_MPI(numloop)),U(1,1,64+0,N_MPI(numloop)),U(1,1,64+1,N_MPI(numloop)),U(1,1,64+2,N_MPI(numloop)),numloop=1,N_MPI(20))
     close(10)
  end if
...
!  WRITE(chnum,'(I4.4)') cntnum
!  WRITE(NPENUM,'(I3.3)') NRANK
!  open(10,FILE=dir//'z'//chnum//bccnt//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
!  do k = -1, Ncellz+2
!     do j = -1, Ncelly+2
!        do i = -1, Ncellx+2
!           write(10,*) (U(i,j,k,numloop),numloop=1,N_MPI(20) )
...
!***********************************  BC for Lagrange coordinate  **********
 ...
 !-----------ckbc--------------
 if(NRANK==292) then
     open(10,FILE=dir//'xlagbc292.dat',FORM='FORMATTED',position='append')
     write(10,*) ccc ,   dxlagM(-1,1,1),dxlagM(0,1,1),dxlagM(1,1,1),dxlagM(2,1,1)
     close(10)
  end if
  if(NRANK==291) then
     open(10,FILE=dir//'xlagbc291.dat',FORM='FORMATTED',position='append')
     write(10,*) ccc ,   dxlagM(64-1,1,1),dxlagM(64+0,1,1),dxlagM(64+1,1,1),dxlagM(64+2,1,1)
     close(10)
  end if
  !-----------ckbc--------------
...
!********************************************************  BC for Bcc ******
  IF(N_MPI(11).eq.1) THEN
   ...
   !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'xbccbc292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Bcc(-1,1,1,K),Bcc(0,1,1,K),Bcc(1,1,1,K),Bcc(2,1,1,K),K=1,3)
      close(10)
   end if
   if(NRANK==291) then
      open(10,FILE=dir//'xbccbc291.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Bcc(64-1,1,1,K),Bcc(64+0,1,1,K),Bcc(64+1,1,1,K),Bcc(64+2,1,1,K),K=1,3)
      close(10)
   end if
   !-----------ckbc--------------
...
!********************************************************  BC for Blg ******
  IF(N_MPI(12).eq.1) THEN
   ...
   !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'xblg292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Blg(-1,1,1,K),Blg(0,1,1,K),Blg(1,1,1,K),Blg(2,1,1,K),K=1,2)
      close(10)
   end if
   if(NRANK==291) then
      open(10,FILE=dir//'xblg291.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Blg(64-1,1,1,K),Blg(64+0,1,1,K),Blg(64+1,1,1,K),Blg(64+2,1,1,K),K=1,2)
      close(10)
   end if
   !-----------ckbc--------------
...
!********************************************************  BC for Vfc ******
  IF(N_MPI(13).eq.1) THEN
   ...
   !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'xvfcbc292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Vfc(-1,1,1,NV(K)),Vfc(0,1,1,NV(K)),Vfc(1,1,1,NV(K)),Vfc(2,1,1,NV(K)),K=1,2)
      close(10)
   end if
   if(NRANK==291) then
      open(10,FILE=dir//'xvfcbc291.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Vfc(64-1,1,1,NV(K)),Vfc(64+0,1,1,NV(K)),Vfc(64+1,1,1,NV(K)),Vfc(64+2,1,1,NV(K)),K=1,2)
      close(10)
   end if
   !-----------ckbc--------------
...
!        ndH(IX,JY,KZ)   = BBRV_cm(1); ndp(IX,JY,KZ) = BBRV_cm(2); ndH2(IX,JY,KZ) = BBRV_cm(3); ndHe(IX,JY,KZ) = BBRV_cm(4) !なぜ入れないのか？
!        ndHep(IX,JY,KZ) = BBRV_cm(5); ndC(IX,JY,KZ) = BBRV_cm(6); ndCO(IX,JY,KZ) = BBRV_cm(7); ndCp(IX,JY,KZ) = BBRV_cm(8)
        ndH(IX,JY,KZ)   = BBRV_cm(1)*U(IX,JY,KZ,1); ndp(IX,JY,KZ)  = BBRV_cm(2)*U(IX,JY,KZ,1)
        ndH2(IX,JY,KZ)  = BBRV_cm(3)*U(IX,JY,KZ,1); ndHe(IX,JY,KZ) = BBRV_cm(4)*U(IX,JY,KZ,1)
        ndHep(IX,JY,KZ) = BBRV_cm(5)*U(IX,JY,KZ,1); ndC(IX,JY,KZ)  = BBRV_cm(6)*U(IX,JY,KZ,1)
 ...
 !-----------ckbc--------------
 if(NRANK==292) then
    open(10,FILE=dir//'xcembc292.dat',FORM='FORMATTED',position='append')
    write(10,*)  ccc ,  ndH(-1,1,1),ndH(0,1,1),ndH(1,1,1 ),ndH(2,1,1 ),&
         ndp(-1,1,1),ndp(0,1,1),ndp(1,1,1 ),ndp(2,1,1 ),&
         ndH2(-1,1,1),ndH2(0,1,1),ndH2(1,1,1 ),ndH2(2,1,1 ),&
         ndHe(-1,1,1),ndHe(0,1,1),ndHe(1,1,1 ),ndHe(2,1,1 ),&
         ndHep(-1,1,1),ndHep(0,1,1),ndHep(1,1,1 ),ndHep(2,1,1 ),&
         ndC(-1,1,1),ndC(0,1,1),ndC(1,1,1 ),ndC(2,1,1 ),&
         ndCO(-1,1,1),ndCO(0,1,1),ndCO(1,1,1 ),ndCO(2,1,1 ),&
         ndCp(-1,1,1),ndCp(0,1,1),ndCp(1,1,1 ),ndCp(2,1,1 )
    close(10)
 end if
 if(NRANK==291) then
    open(10,FILE=dir//'xcembc291.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   ndH(64-1,1,1 ),ndH(64+0,1,1 ),ndH(64+1,1,1 ),ndH(64+2,1,1),&
         ndp(64-1,1,1 ),ndp(64+0,1,1 ),ndp(64+1,1,1 ),ndp(64+2,1,1),&
         ndH2(64-1,1,1 ),ndH2(64+0,1,1 ),ndH2(64+1,1,1 ),ndH2(64+2,1,1),&
         ndHe(64-1,1,1 ),ndHe(64+0,1,1 ),ndHe(64+1,1,1 ),ndHe(64+2,1,1),&
         ndHep(64-1,1,1 ),ndHep(64+0,1,1 ),ndHep(64+1,1,1 ),ndHep(64+2,1,1),&
         ndC(64-1,1,1 ),ndC(64+0,1,1 ),ndC(64+1,1,1 ),ndC(64+2,1,1),&
         ndCO(64-1,1,1 ),ndCO(64+0,1,1 ),ndCO(64+1,1,1 ),ndCO(64+2,1,1),&
         ndCp(64-1,1,1 ),ndCp(64+0,1,1 ),ndCp(64+1,1,1 ),ndCp(64+2,1,1)
    close(10)
 end if
end IF
 !-----------ckbc--------------
...
!***********************************  BC for Lagrange coordinate  **********
  IF(N_MPI(10).eq.1) THEN
    ...
    !-----------ckbc--------------
 if(NRANK==292) then
     open(10,FILE=dir//'ylagbc292.dat',FORM='FORMATTED',position='append')
     write(10,*)  ccc ,  dxlagM(1,-1,1),dxlagM(1,0,1),dxlagM(1,1,1),dxlagM(1,2,1)
     close(10)
  end if
  if(NRANK==284) then
     open(10,FILE=dir//'ylagbc284.dat',FORM='FORMATTED',position='append')
     write(10,*) ccc ,   dxlagM(1,64-1,1),dxlagM(1,64+0,1),dxlagM(1,64+1,1),dxlagM(1,64+2,1)
     close(10)
  end if
  !-----------ckbc--------------
...
!********************************************************  BC for Bcc ******
  IF(N_MPI(11).eq.1) THEN
    ...
    END DO
     ...
     !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'ybccbc292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Bcc(1,-1,1,K),Bcc(1,0,1,K),Bcc(1,1,1,K),Bcc(1,2,1,K),K=1,3)
      close(10)
   end if
   if(NRANK==284) then
      open(10,FILE=dir//'ybccbc284.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Bcc(1,64-1,1,K),Bcc(1,64+0,1,K),Bcc(1,64+1,1,K),Bcc(1,64+2,1,K),K=1,3)
      close(10)
   end if
   !-----------ckbc--------------
...
!********************************************************  BC for Blg ******
  IF(N_MPI(12).eq.1) THEN
   ...
   END DO
    ...
    !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'yblg292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Blg(1,-1,1,K),Blg(1,0,1,K),Blg(1,1,1,K),Blg(1,2,1,K),K=1,2)
      close(10)
   end if
   if(NRANK==284) then
      open(10,FILE=dir//'yblg284.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Blg(1,64-1,1,K),Blg(1,64+0,1,K),Blg(1,64+1,1,K),Blg(1,64+2,1,K),K=1,2)
      close(10)
   end if
   !-----------ckbc--------------
...
!********************************************************  BC for Vfc ******
  IF(N_MPI(13).eq.1) THEN
   ...
   !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'yvfcbc292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Vfc(1,-1,1,NV(K)),Vfc(1,0,1,NV(K)),Vfc(1,1,1,NV(K)),Vfc(1,2,1,NV(K)),K=1,2)
      close(10)
   end if
   if(NRANK==284) then
      open(10,FILE=dir//'yvfcbc284.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Vfc(1,64-1,1,NV(K)),Vfc(1,64+0,1,NV(K)),Vfc(1,64+1,1,NV(K)),Vfc(1,64+2,1,NV(K)),K=1,2)
      close(10)
   end if
   !-----------ckbc--------------
END IF
!***********************************  BC for chemical species  **********
  IF(N_MPI(10).eq.10) THEN
    ...
    CALL MPI_SENDRECV( ndCp(-1,1            ,klps),1,VECU,BOTM,1, &
      ...
      !-----------ckbc--------------
 if(NRANK==292) then
    open(10,FILE=dir//'ycembc292.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   ndH(1,-1,1),ndH(1,0,1),ndH(1,1,1 ),ndH(1,2,1 ),&
         ndp(1,-1,1),ndp(1,0,1),ndp(1,1,1 ),ndp(1,2,1 ),&
         ndH2(1,-1,1),ndH2(1,0,1),ndH2(1,1,1 ),ndH2(1,2,1 ),&
         ndHe(1,-1,1),ndHe(1,0,1),ndHe(1,1,1 ),ndHe(1,2,1 ),&
         ndHep(1,-1,1),ndHep(1,0,1),ndHep(1,1,1 ),ndHep(1,2,1 ),&
         ndC(1,-1,1),ndC(1,0,1),ndC(1,1,1 ),ndC(1,2,1 ),&
         ndCO(1,-1,1),ndCO(1,0,1),ndCO(1,1,1 ),ndCO(1,2,1 ),&
         ndCp(1,-1,1),ndCp(1,0,1),ndCp(1,1,1 ),ndCp(1,2,1 )
    close(10)
 end if
 if(NRANK==284) then
    open(10,FILE=dir//'ycembc284.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   ndH(1,64-1,1 ),ndH(1,64+0,1 ),ndH(1,64+1,1 ),ndH(1,64+2,1),&
         ndp(1,64-1,1 ),ndp(1,64+0,1 ),ndp(1,64+1,1 ),ndp(1,64+2,1),&
         ndH2(1,64-1,1 ),ndH2(1,64+0,1 ),ndH2(1,64+1,1 ),ndH2(1,64+2,1),&
         ndHe(1,64-1,1 ),ndHe(1,64+0,1 ),ndHe(1,64+1,1 ),ndHe(1,64+2,1),&
         ndHep(1,64-1,1 ),ndHep(1,64+0,1 ),ndHep(1,64+1,1 ),ndHep(1,64+2,1),&
         ndC(1,64-1,1 ),ndC(1,64+0,1 ),ndC(1,64+1,1 ),ndC(1,64+2,1),&
         ndCO(1,64-1,1 ),ndCO(1,64+0,1 ),ndCO(1,64+1,1 ),ndCO(1,64+2,1),&
         ndCp(1,64-1,1 ),ndCp(1,64+0,1 ),ndCp(1,64+1,1 ),ndCp(1,64+2,1)
    close(10)
 end if
 !-----------ckbc--------------
 END IF
...
!***********************************  BC for Lagrange coordinate  **********
  IF(N_MPI(10).eq.1) THEN
    ...
    CALL MPI_SENDRECV(dxlagM(-1,-1,1            ),1,VECU,DOWN,1, &
     ...
     !-----------ckbc--------------
 if(NRANK==292) then
     open(10,FILE=dir//'zlagbc292.dat',FORM='FORMATTED',position='append')
     write(10,*)  ccc ,  dxlagM(1,1,-1),dxlagM(1,1,0),dxlagM(1,1,1),dxlagM(1,1,2)
     close(10)
  end if
  if(NRANK==228) then
     open(10,FILE=dir//'zlagbc228.dat',FORM='FORMATTED',position='append')
     write(10,*)  ccc ,  dxlagM(1,1,64-1),dxlagM(1,1,64+0),dxlagM(1,1,64+1),dxlagM(1,1,64+2)
     close(10)
  end if
  !-----------ckbc--------------
...
!********************************************************  BC for Bcc ******
  IF(N_MPI(11).eq.1) THEN
   ...
   END DO
    ...
    !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'zbccbc292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Bcc(1,1,-1,K),Bcc(1,1,0,K),Bcc(1,1,1,K),Bcc(1,1,2,K),K=1,3)
      close(10)
   end if
   if(NRANK==228) then
      open(10,FILE=dir//'zbccbc228.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Bcc(1,1,64-1,K),Bcc(1,1,64+0,K),Bcc(1,1,64+1,K),Bcc(1,1,64+2,K),K=1,3)
      close(10)
   end if
   !-----------ckbc--------------
END IF
!********************************************************  BC for Blg ******
  IF(N_MPI(12).eq.1) THEN
   ...
   !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'zblg292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Blg(1,1,-1,K),Blg(1,1,0,K),Blg(1,1,1,K),Blg(1,1,2,K),K=1,2)
      close(10)
   end if
   if(NRANK==228) then
      open(10,FILE=dir//'zblg228.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Blg(1,1,64-1,K),Blg(1,1,64+0,K),Blg(1,1,64+1,K),Blg(1,1,64+2,K),K=1,2)
      close(10)
   end if
   !-----------ckbc--------------
...
!********************************************************  BC for Vfc ******
  IF(N_MPI(13).eq.1) THEN
   ...
   END DO
    ...
    !-----------ckbc--------------
   if(NRANK==292) then
      open(10,FILE=dir//'zvfcbc292.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Vfc(1,1,-1,NV(K)),Vfc(1,1,0,NV(K)),Vfc(1,1,1,NV(K)),Vfc(1,1,2,NV(K)),K=1,2)
      close(10)
   end if
   if(NRANK==228) then
      open(10,FILE=dir//'zvfcbc228.dat',FORM='FORMATTED',position='append')
      write(10,*) ccc ,   (Vfc(1,1,64-1,NV(K)),Vfc(1,1,64+0,NV(K)),Vfc(1,1,64+1,NV(K)),Vfc(1,1,64+2,NV(K)),K=1,2)
      close(10)
   end if
   !-----------ckbc--------------
...
!***********************************  BC for chemical species  **********
 ...
 !-----------ckbc--------------
 if(NRANK==292) then
    open(10,FILE=dir//'zcembc292.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   ndH(1,1,-1),ndH(1,1,0),ndH(1,1,1),ndH(1,1,2),&
         ndp(1,1,-1),ndp(1,1,0),ndp(1,1,1),ndp(1,1,2),&
         ndH2(1,1,-1),ndH2(1,1,0),ndH2(1,1,1),ndH2(1,1,2),&
         ndHe(1,1,-1),ndHe(1,1,0),ndHe(1,1,1),ndHe(1,1,2),&
         ndHep(1,1,-1),ndHep(1,1,0),ndHep(1,1,1),ndHep(1,1,2),&
         ndC(1,1,-1),ndC(1,1,0),ndC(1,1,1),ndC(1,1,2),&
         ndCO(1,1,-1),ndCO(1,1,0),ndCO(1,1,1),ndCO(1,1,2),&
         ndCp(1,1,-1),ndCp(1,1,0),ndCp(1,1,1),ndCp(1,1,2)
    close(10)
 end if
 if(NRANK==228) then
    open(10,FILE=dir//'zcembc228.dat',FORM='FORMATTED',position='append')
    write(10,*) ccc ,   ndH(1,1,64-1),ndH(1,1,64+0),ndH(1,1,64+1),ndH(1,1,64+2),&
         ndp(1,1,64-1),ndp(1,1,64+0),ndp(1,1,64+1),ndp(1,1,64+2),&
         ndH2(1,1,64-1),ndH2(1,1,64+0),ndH2(1,1,64+1),ndH2(1,1,64+2),&
         ndHe(1,1,64-1),ndHe(1,1,64+0),ndHe(1,1,64+1),ndHe(1,1,64+2),&
         ndHep(1,1,64-1),ndHep(1,1,64+0),ndHep(1,1,64+1),ndHep(1,1,64+2),&
         ndC(1,1,64-1),ndC(1,1,64+0),ndC(1,1,64+1),ndC(1,1,64+2),&
         ndCO(1,1,64-1),ndCO(1,1,64+0),ndCO(1,1,64+1),ndCO(1,1,64+2),&
         ndCp(1,1,64-1),ndCp(1,1,64+0),ndCp(1,1,64+1),ndCp(1,1,64+2)
    close(10)
 end if
 !-----------ckbc--------------
 END IF
