...
if(mode==1) then
      ...
      open(unit=18,file='/work/maedarn/3DMHD/test/PHIDT2INI/INIPHIDT2'//NPENUM//'.DAT',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
      do k = -1, Ncellz+2
         do j = -1, Ncelly+2
            !do i = -1, Ncellx+2
      ...
      do k = -1, Ncellz+2
         do j = -1, Ncelly+2
            !do i = -1, Ncellx+2
            read(28) (Phidt2(i,j,k),i=-1,Ncellx+2)
            !enddo
         end do
      end do
      close(28)
...
if(mode==4) then
   ...
   open(unit=48,file='/work/maedarn/3DMHD/test/PHIDT2INI/INIPHIDT2'//NPENUM//countcha//'.DAT',FORM='UNFORMATTED')!,FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
   !write(*,*) 'save?????'
...
!      do j = -1, Ncelly+2
!         !do i = -1, Ncellx+2
!         write(48) (sngl(Phidt2(i,j,k)),i=-1,Ncellx+2)
!         !enddo
!      end do
!   end do
!   close(48)
...
subroutine slvg(dt)
  ...
  DOUBLE PRECISION, dimension(:,:,:), allocatable :: f,g , fnew , gnew,gradPhidt,phipre

  ALLOCATE(f(-1:ndx,-1:ndy,-1:ndz))
  ALLOCATE(g(-1:ndx,-1:ndy,-1:ndz))
  ALLOCATE(fnew(-1:ndx,-1:ndy,-1:ndz))
  ALLOCATE(gnew(-1:ndx,-1:ndy,-1:ndz))
  ALLOCATE(phipre(-1:ndx,-1:ndy,-1:ndz))
  phipre(:,:,:)=Phi(:,:,:)
  ...
  !   end do
  !end do
  call dtcal(gradPhidt)
  !------------- dPhi/dt ----------------

  call exact(dt*0.5d0,gradPhidt(-1,-1,-1))

  call MUSCL1D(f(-1,-1,-1),g(-1,-1,-1),1,0,0,gradPhidt(-1,-1,-1),dt,phipre)
  call muslcslv1D(dt,f(-1,-1,-1),g(-1,-1,-1),1,0,0)
  fnew(:,:,:)=f(:,:,:)
  gnew(:,:,:)=g(:,:,:)
  call MUSCL1D(f(-1,-1,-1),g(-1,-1,-1),0,1,0,gradPhidt(-1,-1,-1),dt,phipre)
  call muslcslv1D(dt,f(-1,-1,-1),g(-1,-1,-1),0,1,0)
  fnew(:,:,:)=fnew(:,:,:)+f(:,:,:)
  gnew(:,:,:)=gnew(:,:,:)+g(:,:,:)
  call MUSCL1D(f(-1,-1,-1),g(-1,-1,-1),0,0,1,gradPhidt(-1,-1,-1),dt,phipre)
  call muslcslv1D(dt,f(-1,-1,-1),g(-1,-1,-1),0,0,1)
  fnew(:,:,:)=fnew(:,:,:)+f(:,:,:)
  gnew(:,:,:)=gnew(:,:,:)+g(:,:,:)
  ...
  deallocate(fnew)
  deallocate(fnew)
  deallocate(gradPhidt)
  deallocate(phipre)
end subroutine slvg

subroutine exact(dt,gradPhidt)
  USE comvar
  USE mpivar
  USE slfgrv
  INCLUDE 'mpif.h'
  ...
  double precision :: gradPhidt(-1:Ncellx+2,-1:Ncelly+2,-1:Ncellz+2),Phipre(-1:Ncellx+2,-1:Ncelly+2,-1:Ncellz+2),dt
  Phipre(:,:,:) = Phi(:,:,:)
  do k=-1,Ncellz+2
     do j=-1,Ncelly+2
        do i=-1,Ncellx+2
           Phi(i,j,k) = 0.5d0 * 4piG * U(i,j,k,1) * (dt**2) + gradPhidt(i,j,k) * dt + Phipre(i,j,k)
        end do
     end do
  end do
end subroutine exact

subroutine MUSCL1D(f,g,ix,iy,iz,dt,phipre)
  USE comvar
  USE mpivar
  USE slfgrv
  INCLUDE 'mpif.h'
  integer :: ix,iy,iz
  !integer :: Impix,Jmpiy,Kmpiz
  double precision f(-1:ndx,-1:ndy,-1:ndz),g(-1:ndx,-1:ndy,-1:ndz),gradPhidt(-1:ndx,-1:ndy,-1:ndz)
  double precision gradPhidtdx(-1:ndx,-1:ndy,-1:ndz),dt,phipre(-1:ndx,-1:ndy,-1:ndz)
  DOUBLE PRECISION, dimension(:,:), allocatable :: intC , Phisurface
  double precision dimension(:,:,:), allocatable :: intgral

  do k=-1,Ncellz+2
     do j=-1,Ncelly+2
        do i=-1,Ncellx+2
           gradPhidtdx(i,j,k) =  gradPhidt(i,j,k) * deltalength ! dphi/dt = a(x,y,z) , a(x,y,z) * dx   !constant mesh
        end do
     end do
  end do
  ...
  if(iz.eq.1) then; Ncell=Ncellz; Ncm=Ncellx; Ncl=Ncelly;  end if
     ...
     ALLOCATE(integral(1:Ncm,1:Ncl,0:NPE-1))
     allocate(intC(1:Ncm,1:Ncl))
     do k = 1 , Ncl
        do j = 1 , Ncm
           do i = 1 , Ncell
         !Phi1D(i) = gradPhidt(i,j,k) * deltalength ! dphi/dt = a(x,y,z) , a(x,y,z) * dx
              Phisurface(j,k)=gradPhidtdx(i,j,k) + Phisurface(j,k)
           end do
        end do
     end do
     ...
     if(ix==1) then
        ...
        do k=1,Ncl
           do j=1,Ncm
              do i=1,Ncell
                 gradPhidtdx(i,j,k) =  gradPhidtdx(i,j,k) + intC(j,k)
              end do
           end do
        end do
        call splitfg(gradPhidtdx(-1,-1,-1),f(-1,-1,-1),g(-1,-1,-1),ix,iy,iz,phipre)
     end if
     ...
     if(iy==1) then
        ...
        do i=1,Ncl
           do k=1,Ncm
              do j=1,Ncell
                 gradPhidtdx(i,j,k) =  gradPhidtdx(i,j,k) + intC(k,i)
              end do
           end do
        end do
        call splitfg(gradPhidtdx(-1,-1,-1),f(-1,-1,-1),g(-1,-1,-1),ix,iy,iz,phipre)
     ...
     if(iz==1) then
        ...
        do j=1,Ncl
           do i=1,Ncm
              do k=1,Ncell
                 gradPhidtdx(i,j,k) =  gradPhidtdx(i,j,k) + intC(i,j)
              end do
           end do
        end do
        call splitfg(gradPhidtdx(-1,-1,-1),f(-1,-1,-1),g(-1,-1,-1),ix,iy,iz,phipre)
     ...
     deallocate(Phisurface)
     deALLOCATE(integral)
     deallocate(intC)
end subroutine MUSCL1D

subroutine splitfg(gradPhidtdx,f,g,ix,iy,iz,phipre)
  USE comvar
  USE mpivar
  USE slfgrv
  INCLUDE 'mpif.h'
  integer ix,iy,iz,N_ol
  double precision f(-1:ndx,-1:ndy,-1:ndz),g(-1:ndx,-1:ndy,-1:ndz)
  double precision gradPhidtdx(-1:ndx,-1:ndy,-1:ndz),phipre(-1:ndx,-1:ndy,-1:ndz)
  ...
  do k=1,Ncellz
     do j=1,Ncelly
        do i=1,Ncellx
           ...
           !f(i,j,k) = 0.5d0 * Phi(i,j,k) + gradPhidtdx(i,j,k) !x+cg*t
           !g(i,j,k) = 0.5d0 * Phi(i,j,k) - gradPhidtdx(i,j,k) !x-cg*t
           f(i,j,k) = 0.5d0 * phipre(i,j,k) + gradPhidtdx(i,j,k) !x+cg*t
           g(i,j,k) = 0.5d0 * phipre(i,j,k) - gradPhidtdx(i,j,k) !x-cg*t
        end do
     end do
  end do

  call fgcom(f(-1,-1,-1),1,0,0)
  call fgcom(g(-1,-1,-1),1,0,0)
  call fgcom(f(-1,-1,-1),0,1,0)
  call fgcom(g(-1,-1,-1),0,1,0)
  call fgcom(f(-1,-1,-1),0,0,1)
  call fgcom(g(-1,-1,-1),0,0,1)

end subroutine splitfg

subroutine fgcom(fg,ix,iy,iz)
  USE comvar
  USE mpivar
  USE slfgrv
  INCLUDE 'mpif.h'
  double precision fg(-1:ndx,-1:ndy,-1:ndz)
  integer ix,iy,iz,N_ol
  ...
  IF(iwx.EQ.1) THEN
     ...
     RIGTt = RIGT!; IF(IST.eq.NSPLTx-1) RIGT = MPI_PROC_NULL 周期
     !*****  BC for the leftsides of domains  *****
     CALL MPI_SENDRECV(f(Ncellx+1-N_ol,-1,-1),1,VECU,RIGT,1, &
          f(       1-N_ol,-1,-1),1,VECU,LEFT,1, MPI_COMM_WORLD,MSTATUS,IERR)
     !*****  BC for the rightsides of domains *****
     CALL MPI_SENDRECV(f(1            ,-1,-1),1,VECU,LEFT,1, &
          f(Ncellx+1     ,-1,-1),1,VECU,RIGT,1, MPI_COMM_WORLD,MSTATUS,IERR)
     CALL MPI_TYPE_FREE(VECU,IERR)
     LEFT = LEFTt; RIGT = RIGTt
  ...
  if(iwy==1) then
     ...
     TOPt  = TOP  !; IF(JST.eq.NSPLTy-1) TOP  = MPI_PROC_NULL
     !*****  BC for the downsides of domains  ****
     CALL MPI_SENDRECV(f(-1,Ncelly+1-N_ol,-1),1,VECU,TOP ,1, &
          f(-1,       1-N_ol,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
     !*****  BC for the upsides of domains  ****
     CALL MPI_SENDRECV(f(-1,1            ,-1),1,VECU,BOTM,1, &
          f(-1,Ncelly+1     ,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
     CALL MPI_TYPE_FREE(VECU,IERR)
     TOP = TOPt; BOTM = BOTMt
  ...
  if(iwz==1) then
     ...
     UPt   = UP   !; IF(KST.eq.NSPLTz-1) UP   = MPI_PROC_NULL
     !*****  BC for the downsides of domains  ****
     CALL MPI_SENDRECV(f(-1,-1,Ncellz+1-N_ol),1,VECU,UP  ,1, &
          f(-1,-1,       1-N_ol),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
     !*****  BC for the upsides of domains  ****
     CALL MPI_SENDRECV(f(-1,-1,1            ),1,VECU,DOWN,1, &
          f(-1,-1,Ncellz+1     ),1,VECU,UP  ,1, MPI_COMM_WORLD,MSTATUS,IERR)
     CALL MPI_TYPE_FREE(VECU,IERR)
     UP = UPt; DOWN = DOWNt
  end if
end subroutine fgcom

subroutine muslcslv1D(dt,f,g,ix,iy,iz)
  USE comvar
  USE mpivar
  USE slfgrv
  INCLUDE 'mpif.h'
  !DOUBLE PRECISION, dimension(:,:,:), allocatable :: Phidummy , gradPhidt !, Phidtdummy
  double precision :: nu2 , w=6.0d0 , dt2 , dt, kappa=1.0d0/3.0d0 , deltap,deltam !kappa -> comver  better?
  !character(3) rn
  !DOUBLE PRECISION, dimension(:), allocatable :: grad1DPhi , Phi1D
  !DOUBLE PRECISION, dimension(:), allocatable :: grad1DPhix , Phi1Dx , grad1DPhiy , Phi1Dy , grad1DPhiz , Phi1Dz
  !double precision dimension(:,:,:), allocatable :: intgral !intgrl(0:NPE-1)
  integer direction , mode , invdt , loopmode , dloop , Ncell , Ncell1 , Ncell2,ix,iy,iz
  !DOUBLE PRECISION, dimension(:,:), allocatable :: intC , Phisurface
  DOUBLE PRECISION :: f(-1:ndx,-1:ndy,-1:ndz),g(-1:ndx,-1:ndy,-1:ndz)
  DOUBLE PRECISION :: fpre(-1:ndx,-1:ndy,-1:ndz),gpre(-1:ndx,-1:ndy,-1:ndz)
  DOUBLE PRECISION :: gradf(-1:ndx,-1:ndy,-1:ndz),gradg(-1:ndx,-1:ndy,-1:ndz)
  ...
  call vanalbada(fpre(-1,-1,-1),gradf(-1,-1,-1),ix,iy,iz) !-------等間隔メッシュ
  call vanalbada(gpre(-1,-1,-1),gradg(-1,-1,-1),ix,iy,iz)
  !------2nd step-----
  ...
  do k = 1 , Ncl
     do j = 1 , Ncm
        do i = 1 , Ncell
           ...
           fluxf(i,j,k) = f(i+1,j,k) - gradf(i,j,k) * 0.25d0 *( (1.0d0 - kappa * gradf(i,j,k)) * deltap + (1.0d0 + kappa * gradf(i,j,k)) * deltam)  !ur_{j+1/2}
           ...
           fluxg(i,j,k) = g(i  ,j,k) + gradg(i,j,k) * 0.25d0 *( (1.0d0 - kappa * gradg(i,j,k)) * deltam + (1.0d0 + kappa * gradg(i,j,k)) * deltap)  !ul_{j+1/2}
...
subroutine vanalbada(fg,gradfg,iwx,iwy,iwz)
  ...
  INCLUDE 'mpif.h'
  double precision :: nu2 , w=6.0d0 , dt2 , dt, kappa=1.0d0/3.0d0 , deltap,deltam !kappa -> comver  better?
  integer i_sta , i_end ,iwx,iwy,iwz
  DOUBLE PRECISION :: fg(-1:ndx,-1:ndy,-1:ndz),gradfg(-1:ndx,-1:ndy,-1:ndz)

  if(iwx.eq.1) then; Ncell = Ncellx; Ncm = Ncelly; Ncl = Ncellz;  end if
  if(iwy.eq.1) then; Ncell = Ncelly; Ncm = Ncellz; Ncl = Ncellx;  end if
  if(iwz.eq.1) then; Ncell = Ncellz; Ncm = Ncellx; Ncl = Ncelly;  end if
     DO Lnum = 0, Ncl+1
        DO Mnum = 0, Ncm+1
           ...
           do i = 1 , Ncell
              ...
              grdfg(i,k) = flmt*( fg(ixp,jyp,kzp)-fg(ixm,jym,kzm) )/( 2.0d0 * deltalength )
           end do
        end do
     end do
...
do j=0,ncy; n = j+kk
  ...
  !IF(IST==0) then
     Phi(0,j,k)= bphi2(pointb2(NGL)+n,1)
  !end IF
