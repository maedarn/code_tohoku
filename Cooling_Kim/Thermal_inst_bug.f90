...
use chmvar
!implicit none
integer, parameter :: ix = 50,ixt = 50,ixp = 50,cnt=10!,itrcool=1000
integer :: i,j,k,jm,j_lin

double precision, parameter :: Tmin=1.d-2,Tmax=1.d2,Tmp1p=1.d-2,Tmp2p=1.d4
double precision :: Tmp1 = 1.0d-2,Tmp2 = 1.0d5
double precision, parameter :: Rhomin=1.d0,Rhomax=1.d3,CooLth=1.d-3,dRho1=1.d-6,dTmp1=1.d-6
double precision :: Prsmin=1.d-1*kb*Rhomin,Prsmax=1.d0*kb*Rhomax

double precision :: dlogT,dlogT1,dlogRho,Rho1,dlogRho1,CooLh,CooLl,CooLmid1,CooLmid2,dlogP,dlogP1
double precision, dimension(:), allocatable :: Tmp,Rho,Prs
double precision, dimension(:,:,:), allocatable :: CooLR
...
DOUBLE PRECISION :: ndpmd1,ndHmd1,ndH2md1,ndHmmd1,ndHemd1,ndHepmd1,ndCmd1,ndCpmd1,ndCOmd1,ndemd1,ndtotmd1
DOUBLE PRECISION, dimension(2) :: Ntotmd1,NH2md1,NnCmd1,NCOmd1,tCIImd1
DOUBLE PRECISION :: ndpmd2,ndHmd2,ndH2md2,ndHmmd2,ndHemd2,ndHepmd2,ndCmd2,ndCpmd2,ndCOmd2,ndemd2,ndtotmd2
DOUBLE PRECISION, dimension(2) :: Ntotmd2,NH2md2,NnCmd2,NCOmd2,tCIImd2

DOUBLE PRECISION :: dt,Tmpmid,CooL1,CooL2,CooLmid,nechl,nechh,nechmd,neoldl,neoldh,neoldmd,dinit1,Rhopre,gmm
...
double precision :: Lamlmd1,Lamcmd1,Lamomd1,Lamdmd1,LCOrmd1,LCOHmd1,LCOH2md1,LamH2md1,Lffmd1,Lrrmd1,LCIEmd1,LCIEHemd1
double precision :: Gampemd1,Gamcrmd1,Gampdmd1,Lnebmd1
double precision :: Lamlmd2,Lamcmd2,Lamomd2,Lamdmd2,LCOrmd2,LCOHmd2,LCOH2md2,LamH2md2,Lffmd2,Lrrmd2,LCIEmd2,LCIEHemd2
double precision :: Gampemd2,Gamcrmd2,Gampdmd2,Lnebmd2
double precision :: AbsL,LamcKI,LamlKI,CooLhKI,CooLlKI,CooLmid1KI,CooLmid2KI,CooLmidKI

allocate(Tmp(0:ixt),Rho(0:ix),Prs(0:ixp))
allocate(CooLR(0:ix,0:ixt,7))
...
dlogT = (-dlog10(Tmin) + dlog10(Tmax))/dble(ixt)
dlogRho = (-dlog10(Rhomin) + dlog10(Rhomax))/dble(ix)
dlogP = (-dlog10(Tmin*Rhomin) + dlog10(Tmax*Rhomax))/dble(ixp)
Tmp(0) = Tmin
Rho(0) = Rhomin
Prs(0) = Tmin*Rhomin
dlogT1=dlog10(Tmin)
dlogRho1=dlog10(Rhomin)
dlogP1=dlog10(Tmin*Rhomin)
...
do j=0,ix
!write(*,*) (ndHmd*mH+ndpmd*mH+2.d0*ndH2md*mH2+ndHemd*mHe+ndHepmd*mHe)/(ndHmd+ndpmd+2.d0*ndH2md+ndHemd+ndHepmd) !Rho(j)/ndtotmd * Rho1 / Rhopre
Rho1 = Rho(j)/1.27d0

ndHmd   = ndHmd * Rho1 / Rhopre
ndpmd   = ndpmd * Rho1 / Rhopre
ndH2md  = ndH2md * Rho1 / Rhopre
ndHmmd  = ndHmmd * Rho1 / Rhopre
ndHemd  = ndHemd * Rho1 / Rhopre
ndHepmd = ndHepmd * Rho1 / Rhopre
ndCmd   = ndCmd * Rho1 / Rhopre
ndCOmd  = ndCOmd * Rho1 / Rhopre
ndCpmd  = ndCpmd * Rho1 / Rhopre
ndemd   = ndpmd+ndHepmd+ndCpmd
ndtotmd = ndHmd+ndpmd+2.d0*ndH2md+ndHemd+ndHepmd
Ntotmd(1)=0.d0; NH2md(1)=0.d0; NnCmd(1)=0.d0; tCIImd(1)=0.d0
Ntotmd(2)=0.d0; NH2md(2)=0.d0; NnCmd(2)=0.d0; tCIImd(2)=0.d0

ndHl   = ndHl * (Rho1-dRho1) / Rhopre
ndpl   = ndpl * (Rho1-dRho1) / Rhopre
ndH2l  = ndH2l * (Rho1-dRho1) / Rhopre
ndHml  = ndHml * (Rho1-dRho1) / Rhopre
ndHel  = ndHel * (Rho1-dRho1) / Rhopre
ndHepl = ndHepl * (Rho1-dRho1) / Rhopre
ndCl   = ndCl * (Rho1-dRho1) / Rhopre
ndCOl  = ndCOl * (Rho1-dRho1) / Rhopre
ndCpl  = ndCpl * (Rho1-dRho1) / Rhopre
ndel   = ndpl+ndHepl+ndCpl
ndtotl = ndHl+ndpl+2.d0*ndH2l+ndHel+ndHepl
Ntotl(1)=0.d0; NH2l(1)=0.d0; NnCl(1)=0.d0; tCIIl(1)=0.d0
Ntotl(2)=0.d0; NH2l(2)=0.d0; NnCl(2)=0.d0; tCIIl(2)=0.d0

ndHh   = ndHh * (Rho1+dRho1) / Rhopre
ndph   = ndph * (Rho1+dRho1) / Rhopre
ndH2h  = ndH2h * (Rho1+dRho1) / Rhopre
ndHmh  = ndHmh * (Rho1+dRho1) / Rhopre
ndHeh  = ndHeh * (Rho1+dRho1) / Rhopre
ndHeph = ndHeph * (Rho1+dRho1) / Rhopre
ndCh   = ndCh * (Rho1+dRho1) / Rhopre
ndCOh  = ndCOh * (Rho1+dRho1) / Rhopre
ndCph  = ndCph * (Rho1+dRho1) / Rhopre
ndeh   = ndph+ndHeph+ndCph
ndtoth = ndHh+ndph+2.d0*ndH2h+ndHeh+ndHeph
Ntoth(1)=0.d0; NH2h(1)=0.d0; NnCh(1)=0.d0; tCIIh(1)=0.d0
Ntoth(2)=0.d0; NH2h(2)=0.d0; NnCh(2)=0.d0; tCIIh(2)=0.d0

!Tmin=Prsmin/Rho(j)/kb
!Tmax=Prsmax/Rho(j)/kb
dlogT = (-dlog10(Prsmin/Rho(j)/kb) + dlog10(Prsmax/Rho(j)/kb))/dble(ixt)
!Tmp(0) = Tmin
dlogT1=dlog10(Prsmin/Rho(j)/kb)
write(*,*) 'Loop',j,Prsmin/Rho(j)/kb,Prsmax/Rho(j)/kb
do i=0,ixt
!do j=0,ixp
 Tmpmid=10.d0**dlogT1
 Tmp1 = Tmpmid-dTmp1
 ...
 !Tmpmid = Prs(j)/Rho1
 !Tmp1 = Prs(j)/(Rho1-dRho1)
 !Tmp2 = Prs(j)/Rho1+dRho1)
 ...
 ndpmd1=ndpmd;ndHmd1=ndHmd;ndH2md1=ndH2md;ndHmmd1=ndHmmd;ndHemd1=ndHemd;ndHepmd1=ndHepmd;ndCmd1=ndCmd
 ndCpmd1=ndCpmd;ndCOmd1=ndCOmd;ndemd1=ndemd;ndtotmd1=ndtotmd;Ntotmd1(:)=Ntotmd(:);NH2md1(:)=NH2md(:)
 NnCmd1(:)=NnCmd(:);NCOmd1(:)=NCOmd(:);tCIImd1(:)=tCIImd(:)

 ndpmd2=ndpmd;ndHmd2=ndHmd;ndH2md2=ndH2md;ndHmmd2=ndHmmd;ndHemd2=ndHemd;ndHepmd2=ndHepmd;ndCmd2=ndCmd
 ndCpmd2=ndCpmd;ndCOmd2=ndCOmd;ndemd2=ndemd;ndtotmd2=ndtotmd;Ntotmd2(:)=Ntotmd(:);NH2md2(:)=NH2md(:)
 NnCmd2(:)=NnCmd(:);NCOmd2(:)=NCOmd(:);tCIImd2(:)=tCIImd(:)

 ndph=ndpmd*(Rho1+dRho1)/Rho1;ndHh=ndHmd*(Rho1+dRho1)/Rho1;ndH2h=ndH2md*(Rho1+dRho1)/Rho1;ndHmh=ndHmmd*(Rho1+dRho1)/Rho1
 ndHeh=ndHemd*(Rho1+dRho1)/Rho1;ndHeph=ndHepmd*(Rho1+dRho1)/Rho1;ndCh=ndCmd*(Rho1+dRho1)/Rho1
 ndCph=ndCpmd*(Rho1+dRho1)/Rho1;ndCOh=ndCOmd*(Rho1+dRho1)/Rho1;ndeh=ndemd*(Rho1+dRho1)/Rho1
 ndtoth=ndtotmd*(Rho1+dRho1)/Rho1;Ntoth(:)=Ntotmd(:)*(Rho1+dRho1)/Rho1;NH2h(:)=NH2md(:)*(Rho1+dRho1)/Rho1
 NnCh(:)=NnCmd(:)*(Rho1+dRho1)/Rho1;NCOh(:)=NCOmd(:)*(Rho1+dRho1)/Rho1;tCIIh(:)=tCIImd(:)*(Rho1+dRho1)/Rho1

 ndpl=ndpmd*(Rho1-dRho1)/Rho1;ndHl=ndHmd*(Rho1-dRho1)/Rho1;ndH2l=ndH2md*(Rho1-dRho1)/Rho1;ndHml=ndHmmd*(Rho1-dRho1)/Rho1
 ndHel=ndHemd*(Rho1-dRho1)/Rho1;ndHepl=ndHepmd*(Rho1-dRho1)/Rho1;ndCl=ndCmd*(Rho1-dRho1)/Rho1
 ndCpl=ndCpmd*(Rho1-dRho1)/Rho1;ndCOl=ndCOmd*(Rho1-dRho1)/Rho1;ndel=ndemd*(Rho1-dRho1)/Rho1
 ndtotl=ndtotmd*(Rho1-dRho1)/Rho1;Ntotl(:)=Ntotmd(:)*(Rho1-dRho1)/Rho1;NH2l(:)=NH2md(:)*(Rho1-dRho1)/Rho1
 NnCl(:)=NnCmd(:)*(Rho1-dRho1)/Rho1;NCOl(:)=NCOmd(:)*(Rho1-dRho1)/Rho1;tCIIl(:)=tCIImd(:)*(Rho1-dRho1)/Rho1
 ...
 CooLmid=CooLmid!/Rho1

 call Fcool(CooLmid1,ndpmd1,ndHmd1,ndH2md1,ndHmmd1,ndHemd1,ndHepmd1,ndCmd1,&
 ndCpmd1,ndCOmd1,ndemd1,ndtotmd1,Ntotmd1,NH2md1,NnCmd1,NCOmd1,tCIImd1,Tmp1,Lamlmd1,Lamcmd1,Lamomd1,&
 Lamdmd1,LCOrmd1,LCOHmd1,LCOH2md1,LamH2md1,Lffmd1,Lrrmd1,LCIEmd1,LCIEHemd1,Gampemd1,Gamcrmd1,Gampdmd1,Lnebmd1)
 CooLmid1=CooLmid1!/Rho1

 call Fcool(CooLmid2,ndpmd2,ndHmd2,ndH2md2,ndHmmd2,ndHemd2,ndHepmd2,ndCmd2,&
 ndCpmd2,ndCOmd2,ndemd2,ndtotmd2,Ntotmd2,NH2md2,NnCmd2,NCOmd2,tCIImd2,Tmp2,Lamlmd2,Lamcmd2,Lamomd2,&
 Lamdmd2,LCOrmd2,LCOHmd2,LCOH2md2,LamH2md2,Lffmd2,Lrrmd2,LCIEmd2,LCIEHemd2,Gampemd2,Gamcrmd2,Gampdmd2,Lnebmd2)
 CooLmid2=CooLmid2!/Rho1

 call Fcool(CooLh,ndph,ndHh,ndH2h,ndHmh,ndHeh,ndHeph,ndCh,ndCph,&
 ndCOh,ndeh,ndtoth,Ntoth,NH2h,NnCh,NCOh,tCIIh,Tmpmid,Lamlh,Lamch,Lamoh,Lamdh,LCOrh,LCOHh,&
 LCOH2h,LamH2h,Lffh,Lrrh,LCIEh,LCIEHeh,Gampeh,Gamcrh,Gampdh,Lnebh)
 CooLh=CooLh!/(Rho1+dRho1)

 call Fcool(CooLl,ndpl,ndHl,ndH2l,ndHml,ndHel,ndHepl,ndCl,ndCpl,&
 ndCOl,ndel,ndtotl,Ntotl,NH2l,NnCl,NCOl,tCIIl,Tmpmid,Lamll,Lamcl,Lamol,Lamdl,LCOrl,LCOHl,&
 LCOH2l,LamH2l,Lffl,Lrrl,LCIEl,LCIEHel,Gampel,Gamcrl,Gampdl,Lnebl)
 CooLl=CooLl!/(Rho1-dRho1)



 CooLR(i,j,1)=CooLmid
 CooLR(i,j,2)=ndtotmd !Rho1 !ndtotmd
 CooLR(i,j,3)=Tmpmid*1.d3*ndtotmd!*Rho1 !*ndtotmd
 !CooLR(j,i,2)=ndtotmd
 !CooLR(j,i,3)=Tmpmid*1.d3*ndtotmd
 !CooLR(i,j,4)=(CooLh-CooLl)/(dRho1*2.d0)
 !CooLR(i,j,5)=(CooLmid2-CooLmid1)/(Tmp2-Tmp1)-(ndtotmd)/Tmpmid * (CooLh-CooLl)/(dRho1*2.d0) !(CooLmid2-CooLmid1)/(Tmp2-Tmp1)
 CooLR(i,j,4)=(CooLmid2-CooLmid1)/(Tmp2-Tmp1) * Tmpmid - CooLmid
 CooLR(i,j,5)=(CooLmid2-CooLmid1)/(Tmp2-Tmp1) * Tmpmid - (CooLh-CooLl)/(dRho1*2.d0) * Rho1 - CooLmid  !(CooLmid2-CooLmid1)/(Tmp2-Tmp1)
 !CooLR(j,i,5)=(CooLmid2-CooLmid1)/(Tmp2-Tmp1)-(ndtotmd)/Tmpmid * (CooLh-CooLl)/(dRho1*2.d0)
 gmm=( 5.d0*(ndHmd+ndpmd+ndHemd+ndHepmd+ndHmmd)+7.d0*ndH2md )/( 3.d0*(ndHmd+ndpmd+ndHemd+ndHepmd+ndHmmd)+5.d0*ndH2md )
 CooLR(i,j,6)  = (CooLmid2-CooLmid1)/(Tmp2-Tmp1)+(ndtotmd)/Tmpmid * (CooLh-CooLl)/(dRho1*2.d0)/(gmm-1.d0)
 ...
 !N=ndtotmd
 !T=Tmpmid
 LamlKI = 1.0d7 * dexp( -1.184d5/(Tmpmid*1.0d3+1.0d3) )
 LamlKI = dmin1(LamlKI,5.d3)
 LamcKI = 1.4d-2 * dsqrt(Tmpmid*1.0d3) * dexp( -9.2d1/(Tmpmid*1.0d3) )
 CooLmidKI = ( ndtotmd * ( LamlKI + LamcKI ) - 1.d0 ) * AbsL * ndtotmd
 !CooLmidKI = CooLmidKI
 ...
 LamlKI = dmin1(LamlKI,5.d3)
 LamcKI = 1.4d-2 * dsqrt(Tmpmid*1.0d3) * dexp( -9.2d1/(Tmpmid*1.0d3) )
 CooLmid1KI = ( ndtoth * ( LamlKI + LamcKI ) - 1.d0 ) * AbsL * ndtoth
 !CooLmid1 = CooLmid
 ...
 LamlKI = dmin1(LamlKI,5.d3)
 LamcKI = 1.4d-2 * dsqrt(Tmpmid*1.0d3) * dexp( -9.2d1/(Tmpmid*1.0d3) )
 CooLmid2KI = ( ndtotl* ( LamlKI + LamcKI ) - 1.d0 ) * AbsL * ndtotl
 !CooLmid2 = CooLmid2

 !N=ndtotmd
 !T=Tmp1
 LamlKI = 1.0d7 * dexp( -1.184d5/(Tmp1*1.0d3+1.0d3) )
 LamlKI = dmin1(LamlKI,5.d3)
 LamcKI = 1.4d-2 * dsqrt(Tmp1*1.0d3) * dexp( -9.2d1/(Tmp1*1.0d3) )
 CooLlKI = ( ndtotmd * ( LamlKI + LamcKI ) - 1.d0 ) * AbsL * ndtotmd
 !CooLl = CooLl

 !N=ndtotmd
 !T=Tmp2
 LamlKI = 1.0d7 * dexp( -1.184d5/(Tmp2*1.0d3+1.0d3) )
 LamlKI = dmin1(LamlKI,5.d3)
 LamcKI = 1.4d-2 * dsqrt(Tmp2*1.0d3) * dexp( -9.2d1/(Tmp2*1.0d3) )
 CooLhKI = ( ndtotmd * ( LamlKI + LamcKI ) - 1.d0 ) * AbsL * ndtotmd
 !CooLh = CooLh


 CooLR(i,j,7)=(CooLhKI-CooLlKI)/(Tmp2-Tmp1) * Tmpmid - (CooLmid1KI-CooLmid2KI)/(dRho1*2.d0) * Rho1 - CooLmidKI 
 !CooLR(i,j,5)=(CooLh-CooLl)/(Tmp2-Tmp1) * Tmpmid - CooLmid 
...
open(120,file=dir//'_br.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(130,file=dir//'.dat' ,access='stream',FORM='FORMATTED')
do j=1,ixt
do i=1,ix
write(120) CooLR(i,j,2),CooLR(i,j,3),&
CooLR(i,j,1),CooLR(i,j,4),CooLR(i,j,5),CooLR(i,j,6)
write(130,*) CooLR(i,j,2),CooLR(i,j,3),&
CooLR(i,j,1),CooLR(i,j,4),CooLR(i,j,5),CooLR(i,j,6)
enddo
enddo
close(120)
...
open(220,file=dir//'_br_Tmp.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(230,file=dir//'_br_Rho.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
do i=1,ix
write(220) Rho(i)
enddo
do j=1,ixt
...
open(320,file=dir//'_br_dLdR.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(330,file=dir//'_br_dLdT.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(340,file=dir//'_br_R.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(350,file=dir//'_br_T.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(360,file=dir//'_br_gmm.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(370,file=dir//'_br_KI.dat' ,access='stream',FORM='UNFORMATTED')
do j=1,ixt
do i=1,ix
write(320) CooLR(i,j,4)
write(330) CooLR(i,j,5)
write(340) CooLR(i,j,2)
write(350) CooLR(i,j,3)
write(360) CooLR(i,j,6)
write(370) CooLR(i,j,7)
enddo
enddo
close(320)
close(330)
...
do k=1,ifile
write(nm,'(I2.2)') k
open(14,file='/Users/maedarn/Dropbox/code/code/Cooling_Kim/Mtl'//nm//'.dat')
imaxlp=2
do j=0,imtrx
!read(14,'E8.2E2') (Mtl,i=1,imaxlp)
read(14,*) (Mtl(k,i,j),i=1,imaxlp)
