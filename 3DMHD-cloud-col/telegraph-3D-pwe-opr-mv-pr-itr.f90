RECURSIVE subroutine SELFGRAVWAVE(dt,mode)
  ...
  USE slfgrv
  INCLUDE 'mpif.h'
  integer :: mode,MRANK,count=0,rdnum,rddmy,ndt1=0,svc1=0!,svci=160
  ...
  integer nt
  double precision :: tdm,err1,err2,err3,terr1,terr2
...
do k = 1, Ncellz
   do j = 1, Ncelly
      ...
      err1=((Phiwv(i-1,j,k,1)+Phiwv(i+1,j,k,1)-2.d0*Phiwv(i,j,k,1))/dx1/dx1+(Phiwv(i,j-1,k,1)+Phiwv(i,j+1,k,1)-2.d0*Phiwv(i,j,k,1))/dy1/dy1 &
          +(Phiwv(i,j,k-1,1)+Phiwv(i,j,k+1,1)-2.d0*Phiwv(i,j,k,1))/dz1/dz1)!/Phi_cr!-G4pi*(U(i,j,k,1)-rhomean))!*dx1*dx1
      err2=((Phiwv(i-1,j,k,1)+Phiwv(i+1,j,k,1)-2.d0*Phiwv(i,j,k,1))/dx1/dx1+(Phiwv(i,j-1,k,1)+Phiwv(i,j+1,k,1)-2.d0*Phiwv(i,j,k,1))/dy1/dy1 &
          +(Phiwv(i,j,k-1,1)+Phiwv(i,j,k+1,1)-2.d0*Phiwv(i,j,k,1))/dz1/dz1-G4pi*(U(i,j,k,1)))*dx1*dx1/Phi_cr
      err3=((Phiexa(i-1,j,k)+Phiexa(i+1,j,k)-2.d0*Phiexa(i,j,k))/dx1/dx1+(Phiexa(i,j-1,k)+Phiexa(i,j+1,k)-2.d0*Phiexa(i,j,k))/dy1/dy1 &
          +(Phiexa(i,j,k-1)+Phiexa(i,j,k+1)-2.d0*Phiexa(i,j,k))/dz1/dz1-G4pi*(U(i,j,k,1)-rhomean))/Phi_cr!*dx1*dx1
      terr1=(Phiwv(i,j,k,1)-4.d0*Phidt(i,j,k)+3.d0*Phidt2(i,j,k))*0.5d0/Phi_cr
      terr2=(2.d0*Phiwv(i,j,k,1)-5.d0*Phidt(i,j,k)+4.d0*Phidt2(i,j,k)-Phidt3(i,j,k))/Phi_cr


            write(28) Phiwv(i,j,k,1),Phiexa(i,j,k),U(i,j,k,1),err1,err2,err3,terr1,terr2,Phiwv(i,j,k,8)
          enddo
        end do
  ...
  if(mode==40) then
    ...
    if(mode==41) then
     ...
     do k = -1, Ncellz+2
        do j = -1, Ncelly+2
           do i = -1, Ncellx+2
           read(28) Phiwv(i,j,k,8),Phigrdwv(i,j,k,8),Phiexa(i,j,k),Phigrd(i,j,k,1),U(i,j,k,1)
          enddo
        end do
        !write(*,*) sngl(Phiwv(8,8,k,1)),sngl(Phigrdwv(8,8,k,1))
     end do
...
subroutine slvmuscle(dt)
  ...
  double precision :: adiff2=0.375d0
...
enddo
   ...
   Phiwv(i,j,k,1) = 0.5d0*Phiwvpre(i,j,k,1)*(1.d0+dexp(-1.d0/Tdiff * 0.5d0 * dt))+0.5d0*Phigrdwvpre(i,j,k,1)*(1.d0-dexp(-1.0d0/Tdiff * 0.5d0 * dt))+&
   (-cg*cg*Tdiff*Tdiff*G4pi*rho(i,j,k))*(-1.d0+dexp(-1.0d0/Tdiff * 0.5d0 * dt))-cg*cg*2.d0*Tdiff*G4pi*rho(i,j,k)*0.5d0*(0.5d0 * dt)
   !Phiwv(i,j,k,2) = 0.5d0*Phiwvpre(i,j,k,2)*(1.d0+dexp(-1.d0/Tdiff * 0.5d0 * dtt2))+0.5d0*Phigrdwvpre(i,j,k,2)*(1.d0-dexp(-1.0d0/Tdiff * 0.5d0 * dtt2))+&
   !(-cg*cg*Tdiff*Tdiff*G4pi*rho(i,j,k))*(-1.d0+dexp(-1.0d0/Tdiff * 0.5d0 * dtt2))-cg*cg*2.d0*Tdiff*G4pi*rho(i,j,k)*0.5d0*(0.5d0 * dtt2)

   Phigrdwv(i,j,k,1) = 0.5d0*Phiwvpre(i,j,k,1)*(1.d0-dexp(-1.d0/Tdiff *0.5d0* dt))+0.5d0*Phigrdwvpre(i,j,k,1)*(1.d0+dexp(-1.0d0/Tdiff *0.5d0* dt))+&
   (-cg*cg*Tdiff*Tdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-1.0d0/Tdiff *0.5d0* dt))-cg*cg*2.d0*Tdiff*G4pi*rho(i,j,k)*0.5d0* (0.5d0*dt)
   !Phigrdwv(i,j,k,2) = 0.5d0*Phiwvpre(i,j,k,2)*(1.d0-dexp(-1.d0/Tdiff *0.5d0* dtt2))+0.5d0*Phigrdwvpre(i,j,k,2)*(1.d0+dexp(-1.0d0/Tdiff *0.5d0* dtt2))+&
   !(-cg*cg*Tdiff*Tdiff*G4pi*rho(i,j,k))*(1.d0-dexp(-1.0d0/Tdiff *0.5d0* dtt2))-cg*cg*2.d0*Tdiff*G4pi*rho(i,j,k)*0.5d0* (0.5d0*dtt2)
     ...
     (-2.d0*2.d0*Tdiff*cg*cg*grdxy1/dx1/dy1 &
      -2.d0*2.d0*Tdiff*cg*cg*grdyz1/dy1/dz1 &
      -2.d0*2.d0*Tdiff*cg*cg*grdzx1/dz1/dx1) *dt
...
DO k = -1, Ncellz+2; DO j = -1, Ncelly+2; DO i = 1-N_ol, 0
(-Phiexa(i-1,j,k)+Phiexa(i+1,j,k))*0.5d0/dx1 &
...
do Nroot=0,NPE-1
CALL MPI_BCAST(xg_mpi(Nroot),1,MPI_REAL8,Nroot,MPI_COMM_WORLD,IERR)
CALL MPI_BCAST(yg_mpi(Nroot),1,MPI_REAL8,Nroot,MPI_COMM_WORLD,IERR)
CALL MPI_BCAST(zg_mpi(Nroot),1,MPI_REAL8,Nroot,MPI_COMM_WORLD,IERR)
end do

xg=0.d0;yg=0.d0;zg=0.d0
do Nroot=0,NPE-1
xg = xg_mpi(Nroot)+xg
yg = yg_mpi(Nroot)+yg
zg = zg_mpi(Nroot)+zg
...
do lsph = 0, lsphmax
  do msph = -lsph, lsph
    ...
    !phig=datan((y(j)-yg)/(x(i)-xg))
    thetag=dacos((z(k)-zg)/(rg+eps_Q))
    phig=dsign(1.d0,(y(j)-yg))*dacos((x(i)-xg)/dsqrt((x(i)-xg)**2.d0+(y(j)-yg)**2.d0+eps_Q))
...
do lsph = 0, lsphmax
  do msph = -lsph, lsph
    ...
    Phiexa(i,j,k)=-G*Qlmall(lsph,msph)*dsqrt(4.d0*pi/(2.d0*dble(lsph)+1.d0))*Ylmreal/((rg+eps_Q)**dble(lsph+1))+Phiwv(i,j,k,1)
...
FUNCTION Ylm_xyz(l,m,xr,yr,zr)
USE comvar
USE mpivar
USE slfgrv
INCLUDE 'mpif.h'
INTEGER l,m
double precision xr,yr,zr,Ylm_xyz,rr
INTEGER i,ll
double precision Ylmxyz_coeff
double precision :: pi=3.14159265358979d0
double precision :: eps_Q

eps_Q=dx1*1.d-3

rr=dsqrt(xr**2.d0+yr**2.d0+zr**2.d0)

if(l==0 .and. m==0) then; Ylm_xyz=dsqrt(1.d0/4.d0/pi) ;endif

if(l==1 .and. m==-1) then; Ylm_xyz=dsqrt(3.d0/4.d0/pi)*(yr/(rr+eps_Q)) ;endif
if(l==1 .and. m== 0) then; Ylm_xyz=dsqrt(3.d0/4.d0/pi)*(zr/(rr+eps_Q)) ;endif
if(l==1 .and. m== 1) then; Ylm_xyz=dsqrt(3.d0/4.d0/pi)*(xr/(rr+eps_Q)) ;endif

if(l==2 .and. m== -2) then; Ylm_xyz=dsqrt(15.d0/4.d0/pi)*(xr*yr/(rr+eps_Q)**2.d0) ;endif
if(l==2 .and. m== -1) then; Ylm_xyz=dsqrt(15.d0/4.d0/pi)*(yr*zr/(rr+eps_Q)**2.d0) ;endif
if(l==2 .and. m==  0) then; Ylm_xyz=dsqrt(5.d0/16.d0/pi)*((3.d0*zr**2.d0-rr**2)/(rr+eps_Q)**2.d0) ;endif
if(l==2 .and. m==  1) then; Ylm_xyz=dsqrt(15.d0/4.d0/pi)*(xr*zr/(rr+eps_Q)**2.d0) ;endif
if(l==2 .and. m==  2) then; Ylm_xyz=dsqrt(5.d0/16.d0/pi)*((xr**2.d0-yr**2.d0)/(2.d0*(rr+eps_Q)**2.d0)) ;endif

if(l==3 .and. m== -3) then; Ylm_xyz=dsqrt(35.d0/32.d0/pi)*(yr*(3.d0*xr**2.d0-yr**2.d0)/((rr+eps_Q)**3.d0)) ;endif
if(l==3 .and. m== -2) then; Ylm_xyz=dsqrt(105.d0/4.d0/pi)*(xr*yr*zr/((rr+eps_Q)**3.d0)) ;endif
if(l==3 .and. m== -1) then; Ylm_xyz=dsqrt(21.d0/32.d0/pi)*(yr*(5.d0*zr**2.d0-rr**2.d0)/((rr+eps_Q)**3.d0)) ;endif
if(l==3 .and. m==  0) then; Ylm_xyz=dsqrt(7.d0/16.d0/pi)*(zr*(5.d0*zr**2.d0-3.d0*rr**2.d0)/((rr+eps_Q)**3.d0)) ;endif
if(l==3 .and. m==  1) then; Ylm_xyz=dsqrt(21.d0/32.d0/pi)*(xr*(5.d0*zr**2.d0-rr**2.d0)/((rr+eps_Q)**3.d0)) ;endif
if(l==3 .and. m==  2) then; Ylm_xyz=dsqrt(105.d0/4.d0/pi)*(zr*(xr**2.d0-yr**2.d0)/(2.d0*(rr+eps_Q)**3.d0)) ;endif
if(l==3 .and. m==  3) then; Ylm_xyz=dsqrt(35.d0/32.d0/pi)*(xr*(xr**2.d0-3.d0*yr**2.d0)/((rr+eps_Q)**3.d0)) ;endif

if(l==4 .and. m== -4) then; Ylm_xyz=dsqrt(315.d0/4.d0/pi)*(xr*yr*(xr**2.d0-yr**2.d0)/(2.d0*(rr+eps_Q)**4.d0)) ;endif
if(l==4 .and. m== -3) then; Ylm_xyz=dsqrt(315.d0/32.d0/pi)*(yr*zr*(3.d0*xr**2.d0-yr**2.d0)/((rr+eps_Q)**4.d0)) ;endif
if(l==4 .and. m== -2) then; Ylm_xyz=dsqrt(45.d0/16.d0/pi)*(xr*yr*(7.d0*zr**2.d0-rr**2.d0)/((rr+eps_Q)**4.d0)) ;endif
if(l==4 .and. m== -1) then; Ylm_xyz=dsqrt(45.d0/32.d0/pi)*(yr*zr*(7.d0*zr**2.d0-3.d0*rr**2.d0)/((rr+eps_Q)**4.d0)) ;endif
if(l==4 .and. m==  0) then; Ylm_xyz=dsqrt(9.d0/256.d0/pi)*((35.d0*zr**4.d0-30.d0*zr**2.d0*rr**2.d0+3.d0*rr**4.d0)/((rr+eps_Q)**4.d0)) ;endif
if(l==4 .and. m==  1) then; Ylm_xyz=dsqrt(45.d0/32.d0/pi)*(xr*zr*(7.d0*zr**2.d0-3.d0*rr**2.d0)/((rr+eps_Q)**4.d0)) ;endif
if(l==4 .and. m==  2) then; Ylm_xyz=dsqrt(45.d0/16.d0/pi)*((xr**2.d0-yr**2.d0)*(7.d0*zr**2.d0-rr**2.d0)/(2.d0*(rr+eps_Q)**4.d0)) ;endif
if(l==4 .and. m==  3) then; Ylm_xyz=dsqrt(315.d0/32.d0/pi)*(xr*zr*(xr**2.d0-3.d0*yr**2.d0)/((rr+eps_Q)**4.d0)) ;endif
if(l==4 .and. m==  4) then; Ylm_xyz=dsqrt(315.d0/4.d0/pi)*((xr**2.d0*(xr**2.d0-3.d0*yr**2.d0)-yr**2.d0*(3.d0*xr**2.d0-yr**2.d0))/(8.d0*(rr+eps_Q)**4.d0)) ;endif

return
END FUNCTION Ylm_xyz
