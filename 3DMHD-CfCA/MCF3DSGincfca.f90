...
open(8,file='/work/maedarn/3DMHD/samplecnv/INPUT3D.DAT')
  read(8,*)  Np1x,Np2x
  read(8,*)  Np1y,Np2y
  read(8,*)  Np1z,Np2z
  read(8,*)  ql1x,ql2x
...
open(2,file='/work/maedarn/3DMHD/samplecnv/tsave.DAT')
  ...
  !********purtube yz plane***********!
  !goto 1333
  ALLOCATE (plane(1:Ncelly*NSPLTy,1:Ncellz*NSPLTz))
  open(unit=28,file='/work/maedarn/3DMHD/samplecnv/delta.dat',FORM='UNFORMATTED')
  do c=1,Ncellz*NSPLTz
     do b=1,Ncelly*NSPLTy
        read(28) plane(b,c)
     end do
  end do
  close(28)
...
do k = 1, Ncellz; do j = 1, Ncelly; do i = -1, Ncellx+2
   i2 = IST*Ncellx+i
   i3 = JST*Ncelly+j
   i4 = KST*Ncellz+k
...
!CALL MPI_SENDRECV(U(-1,Ncelly-1,-1,i),1,VECU,TOP ,1, &
!     U(-1,      -1,-1,i),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
!CALL MPI_SENDRECV(U(-1,1       ,-1,i),1,VECU,BOTM,1, &
!     U(-1,Ncelly+1,-1,i),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
!CALL MPI_TYPE_COMMIT(VECU,IERR)
!CALL MPI_SENDRECV(U(-1,-1,Ncellz-1,i),1,VECU,UP  ,1, &
!     U(-1,-1,      -1,i),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
!CALL MPI_SENDRECV(U(-1,-1,1       ,i),1,VECU,DOWN,1, &
!     U(-1,-1,Ncellz+1,i),1,VECU,UP  ,1, MPI_COMM_WORLD,MSTATUS,IERR)
!CALL MPI_TYPE_FREE(VECU,IERR)
!end do

!CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
!CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndH(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndH(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndH(-1,1       ,-1),1,VECU,BOTM,1, &
     ndH(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndH(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndH(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndp(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndp(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndp(-1,1       ,-1),1,VECU,BOTM,1, &
     ndp(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndp(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndp(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndH2(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndH2(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndH2(-1,1       ,-1),1,VECU,BOTM,1, &
     ndH2(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndH2(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndH2(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndHe(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndHe(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndHe(-1,1       ,-1),1,VECU,BOTM,1, &
     ndHe(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndHe(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndHe(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndHep(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndHep(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndHep(-1,1       ,-1),1,VECU,BOTM,1, &
     ndHep(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndHep(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndHep(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndC(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndC(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndC(-1,1       ,-1),1,VECU,BOTM,1, &
     ndC(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndC(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndC(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndCO(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndCO(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndCO(-1,1       ,-1),1,VECU,BOTM,1, &
     ndCO(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndCO(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndCO(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndCp(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndCp(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndCp(-1,1       ,-1),1,VECU,BOTM,1, &
     ndCp(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndCp(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndCp(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
...
CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(nde(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     nde(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(nde(-1,1       ,-1),1,VECU,BOTM,1, &
     nde(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(nde(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     nde(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(nde(-1,-1,1       ),1,VECU,DOWN,1, &
     nde(-1,-1,Ncellz+1),1,VECU,UP  ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)

CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndtot(-1,Ncelly-1,-1),1,VECU,TOP ,1, &
     ndtot(-1,      -1,-1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndtot(-1,1       ,-1),1,VECU,BOTM,1, &
     ndtot(-1,Ncelly+1,-1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(ndtot(-1,-1,Ncellz-1),1,VECU,UP  ,1, &
     ndtot(-1,-1,      -1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(ndtot(-1,-1,1       ),1,VECU,DOWN,1, &
     ndtot(-1,-1,Ncellz+1),1,VECU,UP  ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)

CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(Ntot(-1,Ncelly-1,-1,1),1,VECU,TOP ,1, &
     Ntot(-1,      -1,-1,1),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(Ntot(-1,1       ,-1,1),1,VECU,BOTM,1, &
     Ntot(-1,Ncelly+1,-1,1),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(Ntot(-1,-1,Ncellz-1,1),1,VECU,UP  ,1, &
     Ntot(-1,-1,      -1,1),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(Ntot(-1,-1,1       ,1),1,VECU,DOWN,1, &
     Ntot(-1,-1,Ncellz+1,1),1,VECU,UP  ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)

CALL MPI_TYPE_VECTOR(Ncellz+4,2*(Ncellx*NSPLTx+4),(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(Ntot(-1,Ncelly-1,-1,2),1,VECU,TOP ,1, &
     Ntot(-1,      -1,-1,2),1,VECU,BOTM,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(Ntot(-1,1       ,-1,2),1,VECU,BOTM,1, &
     Ntot(-1,Ncelly+1,-1,2),1,VECU,TOP ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
CALL MPI_TYPE_VECTOR(1,2*(Ncellx*NSPLTx+4)*(Ncelly+4),2*(Ncellx*NSPLTx+4)*(Ncelly+4),MPI_REAL8,VECU,IERR)
CALL MPI_TYPE_COMMIT(VECU,IERR)
CALL MPI_SENDRECV(Ntot(-1,-1,Ncellz-1,2),1,VECU,UP  ,1, &
     Ntot(-1,-1,      -1,2),1,VECU,DOWN,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_SENDRECV(Ntot(-1,-1,1       ,2),1,VECU,DOWN,1, &
     Ntot(-1,-1,Ncellz+1,2),1,VECU,UP  ,1, MPI_COMM_WORLD,MSTATUS,IERR)
CALL MPI_TYPE_FREE(VECU,IERR)
!1333 continue
  !********purtube yz plane***********!


!/work/maedarn/3DMHD/samplecnv/
...
!**** read inhomogeneous density field ****!
  ...
  do MRANK = 0, NPE-1
    ...
    if((JS.eq.JST).and.(KS.eq.KST)) then
      ...
      open(unit=8,file='/work/maedarn/3DMHD/samplecnv/DTF/D'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
      do k = 1, Ncellz
      do j = 1, Ncelly
...
if(nunit.eq.1) goto 120
  ...
  open(unit=8,file='/work/maedarn/3DMHD/samplecnv/000'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  do k = 1, Ncellz+1
  do j = 1, Ncelly+1
...
open(2,file='/work/maedarn/3DMHD/samplecnv/tsave.DAT')
  ...
  read(2,*) nunit
close(2)
open(2,file='/work/maedarn/3DMHD/samplecnv/tsave2D.DAT')
  read(2,*) nunit2D
close(2)
open(3,file='/work/maedarn/3DMHD/samplecnv/time.DAT')
do i = 1, nunit
  read(3,'(1p1d25.17)') t(i)
end do
close(3)
!IF(NRANK.EQ.0) THEN
!  open(2,file='/work/maedarn/3DMHD/samplecnv/test.DAT')
!END IF

write(fnunit,'(I3.3)') nunit;  write(fnpe,'(I3.3)') NRANK
open(5,file='/work/maedarn/3DMHD/samplecnv/info'//fnunit//'.DAT')
!open(5,file='/work/maedarn/3DMHD/samplecnv/info'//fnunit//fnpe//'.DAT')
...
!open(10,file='/work/maedarn/3DMHD/samplecnv/'//filenm//NPENUM//'.dat')
open(10,FILE='/work/maedarn/3DMHD/samplecnv/'//filenm//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
...
!    close(5)
  ...
  open(8,file='/work/maedarn/3DMHD/samplecnv/000'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  do k = 1, Ncellz+1
  do j = 1, Ncelly+1
...
open(11,FILE='/work/maedarn/3DMHD/samplecnv/2D'//filenm//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  k=1; do j=1,Ncelly
...
IF(NRANK.EQ.0) THEN
  open(3,file='/work/maedarn/3DMHD/samplecnv/tsave2D.DAT')
    write(3,*) nunit2D
    write(3,*) time
  close(3)
END IF
