...
integer :: rsix,rsjy,rskz,nstloop,NRANKdm,nstdm,lran1=0,lran2=0,idenref,nidck,icn1,jcn1,kcn1
integer :: nlpnm1,nlpnm6,nlpnm4,nlpnm44,nmIST,nmJST,nmKST,nlpnm7,nlpnm3,nlpnm,nmIST1,nmJST1,nmKST1,Frank
DOUBLE PRECISION :: nlpnm2,nlpnm5,nlpnm8,nlpnm9,nlpnm10,nlpnm11,nwnid2,nlpnm21,nlpnm22,nlpnm23,nlpnm25,nlpnm26,nlpnm27,&
nlpnm88,nlpnm888,nlpnm99,nlpmassive
DOUBLE PRECISION  :: rsixc,rsjyc,rskzc,ndcore,nlpnmpre14,nlpnmpre16
double precision :: div(-1:ndx,-1:ndy,-1:ndz,1:4)
double precision :: SFE,SFratio=0.5d0,Mmassivetot=0.d0,rdumy,pre16,pre14,lQtot=0.d0,Massivenum=0.d0
...
DOUBLE PRECISION :: nmeanrdmax,nmcellradmax,nmeanrddenmax,nmeanrdmin,nmcellradmin,nmeanrddenmin,nrmv1,rdenst1,&
radint1,radint2,rhoimp,rhopre1
...
double precision :: dnMPI(0:NPE-1,3),rdnumMPI(0:NPE-1,3),rdcellMPI(0:NPE-1,3),nrmv(0:NPE-1),nwnid(0:NPE-1),nwnidex(0:NPE-1),&
stprc(0:NPE-1,3),submas(0:NPE-1,2)
...
double precision :: vv1,vv2,vv3,xx1,xx2,xx3,ffgas=1.d0,clsmass=10.d0,Rstdmy,masstoflux,SFE2,Mtotdt,SFE2pre,&
Fstarnum,Fran,Frandmy,Frandmyi
DOUBLE PRECISION, dimension(:,:)  , allocatable :: STF1
DOUBLE PRECISION, dimension(:,:,:)  , allocatable :: STF2
double precision:: U1pre,ndHpre,ndppre,ndH2pre,ndHepre,ndHeppre,ndCpre,ndCOpre,ndCppre,ndepre,ndtotpre,Phistarpre,U1ms,rmin1=1.5d0
...
ndH2pre = ndH2(i,j,k)
ndHepre = ndHe(i,j,k)
ndHeppre = ndHep(i,j,k)
ndCpre = ndC(i,j,k)
ndCOpre = ndCO(i,j,k)
ndCppre = ndCp(i,j,k)
ndepre = nde(i,j,k)
ndtotpre = ndtot(i,j,k)
Phistarpre = Phistar(i,j,k,1)
...
emag1 = 0.d0 !0.5d0 * ( U(i,j,k,6)**2 + U(i,j,k,7)**2 + U(i,j,k,8)**2 )
...
grlocp=(0.5d0-dsign(0.5d0,Phi(i,j,k)-Phi(i+1,j,k)))*(0.5d0-dsign(0.5d0,Phi(i,j,k)-Phi(i,j+1,k)))&
*(0.5d0-dsign(0.5d0,Phi(i,j,k)-Phi(i,j,k+1)))&
...
do nstdm=1,(1)*idint(nlpnm2)*nlpnm3
!---Mass reduction---
U(i,j,k,1)   = U(i,j,k,1) *rmvratio
ndH(i,j,k)   = ndH(i,j,k) *rmvratio
ndp(i,j,k)   = ndp(i,j,k) *rmvratio
ndH2(i,j,k)  = ndH2(i,j,k) *rmvratio
ndHe(i,j,k)  = ndHe(i,j,k) *rmvratio
ndHep(i,j,k) = ndHep(i,j,k) *rmvratio
ndC(i,j,k)   = ndC(i,j,k) *rmvratio
ndCO(i,j,k)  = ndCO(i,j,k) *rmvratio
ndCp(i,j,k)  = ndCp(i,j,k) *rmvratio
nde(i,j,k)   = nde(i,j,k) *rmvratio
ndtot(i,j,k) = ndtot(i,j,k) *rmvratio
!---Mass reduction---
...
Ustar(7,(nid+nidnw)*nlpnm)=0.d0!ranout
...
Ustar(16,(nid+nidnw)*nlpnm)=0.d0!2.2d0*(U(i,j,k,1)*ffgas)**(-1.d0/3.d0)*(ranout*clsmass)**(1.d0/3.d0) !10 -> cluster mass
!Ustar(16,(nid+nidnw)*nlpnm)=2.1d0*(U(i,j,k,1)*ffgas)**(-1.d0/3.d0)*(Ustar(7,(nid+nidnw)*nlpnm)*10.d0)**(1.d0/3.d0)
Ustar(14,(nid+nidnw)*nlpnm)=0.d0!Rst+Ustar(16,(nid+nidnw)*nlpnm)
...
Ustar(18,(nid+nidnw)*nlpnm)=SFE2
Ustar(19,(nid+nidnw)*nlpnm)=Mstar1
Ustar(20,(nid+nidnw)*nlpnm)=dble(nummsv)
Ustar(21,(nid+nidnw)*nlpnm)=0.d0!Mstar1-
...
!Ustar(5,(nid+nidnw)*nlpnm)=U(i,j,k,3)
!Ustar(6,(nid+nidnw)*nlpnm)=U(i,j,k,4)
Ustar(7,(nid+nidnw)*nlpnm)=Mmassivetot !U(i,j,k,1)*dx1*dy1*dz1*SFratio
!Ustar(8,(nid+nidnw)*nlpnm)=intt
!Ustar(9,(nid+nidnw)*nlpnm)=dble(nid)
lQ=aq+bq*dlog10(ranout)+cq*dlog10(ranout)**2+dq*dlog10(ranout)**3+eq*dlog10(ranout)**4+fq*dlog10(ranout)**5
lQtot=dlog10(10.d0**lQtot+10.d0**lQ * dble(nlpnm))
Ustar(10,(nid+nidnw)*nlpnm)=lQtot
Massivenum=Massivenum+dble(nlpnm)
Ustar(21,(nid+nidnw)*nlpnm)=Massivenum
enddo

nlpnm99=0.d0
do nstdm=1,(1)*idint(nlpnm2)*nlpnm3
Rst=10.d0*(10.d0**(Ustar(10,(nid+nidnw)*nlpnm)-49.d0))**(1.d0/3.d0)*(U(i,j,k,1)/1.27d0/10.d0)**(-2.d0/3.d0) !loop
...
Ustar(19,(nid+nidnw)*nlpnm)=Mstar1-Ustar(7,(nid+nidnw)*nlpnm)!Mstarlp
nlpnm99=0.5d0-dsign(0.5d0,Ustar(19,(nid+nidnw)*nlpnm))
Ustar(19,(nid+nidnw)*nlpnm)=Ustar(19,(nid+nidnw)*nlpnm)*(1.d0-nlpnm99)

!---Mass reduction---
do kk1=-1,1,1; do jj1=-1,1,1; do ii1=-1,1,1
if((ii1 .ne. 0).or.(jj1 .ne. 0).or.(kk1 .ne. 0)) then
U(ii1,jj1,kk1,1)   = U(ii1,jj1,kk1,1) *rmvratio * nlpnm99  + U(ii1,jj1,kk1,1) *(1.d0-nlpnm99)
ndH(ii1,jj1,kk1)   = ndH(ii1,jj1,kk1) *rmvratio * nlpnm99  + ndH(ii1,jj1,kk1) *(1.d0-nlpnm99)
ndp(ii1,jj1,kk1)   = ndp(ii1,jj1,kk1) *rmvratio * nlpnm99  + ndp(ii1,jj1,kk1) *(1.d0-nlpnm99)
ndH2(ii1,jj1,kk1)  = ndH2(ii1,jj1,kk1) *rmvratio * nlpnm99 + ndH2(ii1,jj1,kk1) *(1.d0-nlpnm99)
ndHe(ii1,jj1,kk1)  = ndHe(ii1,jj1,kk1) *rmvratio * nlpnm99 + ndHe(ii1,jj1,kk1) *(1.d0-nlpnm99)
ndHep(ii1,jj1,kk1) = ndHep(ii1,jj1,kk1) *rmvratio * nlpnm99+ ndHep(ii1,jj1,kk1) *(1.d0-nlpnm99)
ndC(ii1,jj1,kk1)   = ndC(ii1,jj1,kk1) *rmvratio * nlpnm99  + ndC(ii1,jj1,kk1) *(1.d0-nlpnm99)
ndCO(ii1,jj1,kk1)  = ndCO(ii1,jj1,kk1) *rmvratio * nlpnm99 + ndCO(ii1,jj1,kk1) *(1.d0-nlpnm99)
ndCp(ii1,jj1,kk1)  = ndCp(ii1,jj1,kk1) *rmvratio * nlpnm99 + ndCp(ii1,jj1,kk1) *(1.d0-nlpnm99)
nde(ii1,jj1,kk1)   = nde(ii1,jj1,kk1) *rmvratio * nlpnm99  + nde(ii1,jj1,kk1) *(1.d0-nlpnm99)
ndtot(ii1,jj1,kk1) = ndtot(ii1,jj1,kk1) *rmvratio * nlpnm99+ ndtot(ii1,jj1,kk1) *(1.d0-nlpnm99)
Ustar(22,(nid+nidnw)*nlpnm)=Ustar(22,(nid+nidnw)*nlpnm)+U(ii1,jj1,kk1,1) *rmvratio * nlpnm99 *dx1*dy1*dz1*Msun
endif
...
STF2(istf,jstf,kstf)=10.d0**Ustar(10,nstloop1)+STF2(istf,jstf,kstf)
enddo
...
nlpnm44=int(0.5d0+dsign(0.5d0,Ustar(21,nstloop)-0.5d0))
...
icn1=idint((dmod(Ustar(1,nstloop)-0.5d0*dx1,dble(Ncellx)*dx1)/dx1))+1
jcn1=idint((dmod(Ustar(2,nstloop)-0.5d0*dy1,dble(Ncelly)*dy1)/dy1))+1
kcn1=idint((dmod(Ustar(3,nstloop)-0.5d0*dz1,dble(Ncellz)*dz1)/dz1))+1
...
nlpmassive=Ustar(14,nstloop)

do irdn= 1,iradloop*nlpnm44
...
!nmeanrd=nmeanrd-(U1ms/1.27d0)*(U1ms/1.27d0)*alpharcm*dx1*dy1*dz1*(3.d0*1.d18)**3.d0


dnMPI(NRANK,1)=nmeanrd
call BC_ST_rad(nmeanrd,dnMPI(0,1),1)
if(nmeanrd < 0.d0) nmeanrd =0.d0

if(dabs(nmeanrd-10.d0**Ustar(10,nstloop)) < 10.d0**Ustar(10,nstloop) * 1.d-1) write(*,*)'conv',irdn; exit
...
!Ustar(14,nstloop)=dmax1(Ustar(14,nstloop), dx1*rmin1) * ndcore
call BC_ST_RS_one(nstloop,1,14)
...
nlpnmpre14=(0.5d0-dsign(0.5d0,-Ustar(14,nstloop)+rmin1*dx1*2.d0))
...
Ustar(14,nstloop)=(Ustar(14,nstloop)*nlpnmpre14+rmin1*dx1*2.d0*(1.d0-nlpnmpre14)) * ndcore
...
do k = 1, Ncellz * nlpnm44; do j = 1, Ncelly * nlpnm44 ; do i = 1, Ncellx * nlpnm44
!do k = 1*nkst1, Ncellz*nkst1; do j = 1*njst1, Ncelly*njst1; do i = 1*nist1, Ncellx*nist1
...
rdrgn(i,j,k,1)=rdrgn(i,j,k,1)*rdrgn(i,j,k,2)
U(i,j,k,5)=(3.d0*1.d4/T0)*kb*(ndp(i,j,k)+ndH(i,j,k)+ndH2(i,j,k)+ndHe(i,j,k)+ndHep(i,j,k))*nlpnm11*nlpnm888&
+U(i,j,k,5)*(1.d0-nlpnm11*nlpnm888)
end do; end do; end do

!U(icn1,jcn1,kcn1,5)=U(icn1,jcn1,kcn1,5)*2.5d0*ndcore
...
call runge(Ustar(1,nstloop),Ustar(2,nstloop),Ustar(3,nstloop),Ustar(4,nstloop),dt,nstloop,rsixc,rsjyc,rskzc,1,&
(Ustar(7,nstloop)+Ustar(19,nstloop))/Msun)
call runge(Ustar(2,nstloop),Ustar(3,nstloop),Ustar(1,nstloop),Ustar(5,nstloop),dt,nstloop,rsixc,rsjyc,rskzc,2,&
(Ustar(7,nstloop)+Ustar(19,nstloop))/Msun)
call runge(Ustar(3,nstloop),Ustar(1,nstloop),Ustar(2,nstloop),Ustar(6,nstloop),dt,nstloop,rsixc,rsjyc,rskzc,3,&
(Ustar(7,nstloop)+Ustar(19,nstloop))/Msun)
