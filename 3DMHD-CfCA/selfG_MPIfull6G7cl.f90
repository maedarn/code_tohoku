...
!ALLOCATE(Phiexa(1:ndx-2,1:ndy-2,1:ndz-2))
...
!    CALL MPI_ATHER(dt_mpi(NRANK),1,MPI_REAL8,   &
!                    dt_gat       ,1,MPI_REAL8,   &
...
if(mode.eq.2) then
   ...
   call PB()
   write(*,*) 'aftPB'
   call mglin(Nmem1,Nmem2,4,10,10)
...
character(3) Nfinal
character(3) nr
double precision sa,sa1

!Pre-BC for rho is necessary
write(nr,'(I3.3)') NRANK
open(421+NRANK,file='INIT'//nr//'.dat')
do k=1,Ncellz+1
   do j=1,Ncelly+1
      do i=1,Ncellx+1
         write(421+NRANK,*) sngl( U(i,j,k,1)*G4pi )
      end do
   end do
end do
CLOSE(421+NRANK)
...
!ncx = Ncellx+1
!if(NRANK==40) then
!write(*,*) NRANK,saexact1,bphi1(1,1),saexact1 -bphi1(1,1),'saexact1pre'
sa1 = - saexact1 + bphi1(1,1)
!write(*,*)
!end if
!write(*,*)
...
!CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
!********debug**********
write(*,*) NRANK,sa1,bphi1(1,1),saexact1 -bphi1(1,1),'saexact1'
...
do i=1,ncx; ii = i+jj
   Phi(i,j,k) = cphi2(ii)
    write(521+NRANK,*) sngl(Phi(i,j,k))
end do; end do; end do
!if(NRANK==40) then
close(521+NRANK)
!end if
...
do j=1,ncy
do i=1,ncx
    write(621+NRANK,*) sngl(Phiexa(i,j,k))
end do; end do; end do
close(621+NRANK)

!CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
!if(IST==NSPLTx/2 .and. KST==NSPLTz/2 .and. JST==NSPLTy/2) then
!   sa=Phi(1,1,1) - Phiexa(1,1,1)
!   CALL MPI_BCAST(sa,1,MPI_REAL8,0,MPI_COMM_WORLD,IERR)
!end if
!CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
...
do j=1,ncy
do i=1,ncx
    write(721+NRANK,*) sngl(Phi(i,j,k) - Phiexa(i,j,k) - sa1)
end do; end do; end do
close(721+NRANK)
...
do j=1,ncy
do i=1,ncx
    write(721+NRANK,*) sngl((Phi(i,j,k) - Phiexa(i,j,k) -(sa1))/Phi(i,j,k) )
end do; end do; end do
close(721+NRANK)
...
jjst=0; IF(JST.eq.0) jjst=1    !for debug
kkst=0; IF(KST.eq.0) kkst=1    !for debug
...
IF(IST.eq.0) ixst = 2; IF(JST.eq.0) iyst = 2; IF(KST.eq.0) izst = 2 !for debug
!write(*,*) 'rMPI' , uf(0,0,0)
