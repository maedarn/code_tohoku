program main
  ...
  integer nx,ny,nz,rgn,zero,sloop,smesh,nx1,ny1,nz1,nx2,ny2,nz2,lev,nsm
  integer,allocatable,dimension(:,:,:,:) ::tag
  integer,allocatable,dimension(:,:,:,:,:) ::tag1
  real(4),allocatable,dimension(:,:,:,:) ::U
  real(4),allocatable,dimension(:,:,:,:,:) ::U1!,U2
  ...
  allocate(U1(-1:nx+2,-1:ny+2,-1:nz+2,val,0:lev))
  ...
  U1(:,:,:,:,0)=U(:,:,:,:)
  do i=1,lev
     !allocate(U1(-1:nx1+2,-1:ny1+2,-1:nz1+2,val))
     !allocate(U2(-1:nx1/2+2,-1:ny1/2+2,-1:nz1/2+2,val))
     !U1(:,:,:)=
     !U2(:,:,:)=0.e0
     ! call smooth(U1(-1,-1,-1,1,i-1),U2(-1,-1,-1,1,i),nx1,ny1,nz1,nx1/2,ny1/2,nz1/2,val)
     call smooth(U1(-1,-1,-1,1,i-1),U1(-1,-1,-1,1,i),nx1,ny1,nz1,nx1,ny1,nz1,val)
    ! nx1=nx1/2
    ! ny1=ny1/2
    ! nz1=nz1/2
  end do
        ...
        do j=1,ny
           do i=1,nz
              ...
              !end if
                 ...
                 if(nsm.ne.0) then
                 !MM(nsm)=MM(nsm)+dble(U1(i,j,k,1,))*(dl**3)
                 !vx(n)=dble(U(i,j,k,2))+vx(n)
                 !vy(n)=dble(U(i,j,k,3))+vy(n)
                 !vz(n)=dble(U(i,j,k,4))+vz(n)
                 !vxg(n)=dble(U(i,j,k,2))*dble(U(i,j,k,1))*(dl**3)+vxg(n)
              ...
              !        +tag(i,j,k,m)*tag(i,j-1,k,m)+tag(i,j,k,m)*tag(i,j+1,k,m)+tag(i,j,k,m)*tag(i,j,k-1,m)&
                 !        +tag(i,j,k,m)*tag(i,j,k+1,m)))/n/n ,'countS 6'
                  ...
                  !Mass1(n)=dble(v(i,j,k,1))*dl*dl*dl+Mass1
                  !mxrho1=amax1(mxrho1,v(i,j,k,1))
                 if(dble(U1(i,j,k,9,lev))<phi(nsm))then
                    phi(nsm)=dble(U1(i,j,k,9,lev))
                    ig(nsm)=i
                    jg(nsm)=j
                    kg(nsm)=k
                 end if
              !end if
                 xg(nsm)=dble(U1(i,j,k,1,lev))*x(i)*(dl**3)+xg(i)
                 yg(nsm)=dble(U1(i,j,k,1,lev))*y(i)*(dl**3)+yg(i)
                 zg(nsm)=dble(U1(i,j,k,1,lev))*z(i)*(dl**3)+zg(i)
              end if
           end do
        end do
        write(*,*) k,n,'mass'
...
close(100)
  ...
  deallocate(vdotf,vdotfg,vdotfp,vdotfb,U1,tag1)

end program main

subroutine smooth(Uin,Uout,nxin,nyin,nzin,nxout,nyout,nzout,val)
  !double precision,allocatable,dimension(:,:,:) :: Uin,Uout
  real(4),allocatable,dimension(-1:nxin+2,-1:nyin+2,-1:nzin+2,val) :: Uin
  real(4),allocatable,dimension(-1:nxout+2,-1:nyout+2,-1:nzout+2,val) :: Uout
  integer nxin,nyin,nzin,nxout,nyout,nzout,i,j,k,ii,jj,kk,val

  !allocate(Uin(nxin,nyin,nzin),Uout(nxout,nyout,nzout))
  ...
  do k=1,nzout
     do j=1,nyout
        do i=1,nxout
           Uout(i,j,k) = (Uin(i*2-1,j*2-1,k*2-1,val)+Uin(i*2,j*2,k*2,val)+Uin(i*2,j*2-1,k*2-1,val)+Uin(i*2-1,j*2,k*2-1,val)&
                +Uin(i*2-1,j*2-1,k*2,val)+Uin(i*2,j*2,k*2-1,val)+Uin(i*2,j*2-1,k*2,val)+Uin(i*2-1,j*2,k*2,val))/8.d0
        end do
     end do
  end do
  Uout(-1,:,:,:)=Uout(nx2-1,:,:,:)
...
subroutine smoothtag(Uin,Uout,rho,nxin,nyin,nzin,nxout,nyout,nzout,shd)
  !double precision,allocatable,dimension(:,:,:) :: Uin,Uout
  integer,allocatable,dimension(0:nxin+1,0:nyin+1,0:nzin+1) :: Uin
  integer,allocatable,dimension(0:nxout+1,0:nyout+1,0:nzout+1) :: Uout
  real(4),allocatable,dimension(0:nxout+1,0:nyout+1,0:nzout+1) :: rho
  real(4) shd
  ...
  do k=1,nzout
     do j=1,nyout
        do i=1,nxout
           maxint=0
           maxint=max0(maxint,Uin(i*2-1,j*2-1,k*2-1,val),Uin(i*2,j*2,k*2,val),Uin(i*2,j*2-1,k*2-1,val),Uin(i*2-1,j*2,k*2-1,val)&
                ,Uin(i*2-1,j*2-1,k*2,val),Uin(i*2,j*2,k*2-1,val),Uin(i*2,j*2-1,k*2,val),Uin(i*2-1,j*2,k*2,val))
           Uout(i,j,k) = (Uin(i*2-1,j*2-1,k*2-1,val)+Uin(i*2,j*2,k*2,val)+Uin(i*2,j*2-1,k*2-1,val)+Uin(i*2-1,j*2,k*2-1,val)&
                +Uin(i*2-1,j*2-1,k*2,val)+Uin(i*2,j*2,k*2-1,val)+Uin(i*2,j*2-1,k*2,val)+Uin(i*2-1,j*2,k*2,val))/8
