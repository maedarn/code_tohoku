...
real*8 :: ql1x,ql1y,ql1z,ql2x,ql2y,ql2z,dinit1
character*3 :: NPENUM
...
Ncellx = 512; Ncelly = 512; Ncellz = 512
...
ALLOCATE(U(Ncellx+1,Ncelly+1,Ncellz+1))
...
IF(NRANK.EQ.0) THEN
  400 format(D25.17)
  open(4,file='/work/inouety/CMF1/cdnt.DAT')
...
END IF
  ...
  if(NRANK.eq.0) then
    do NN=0,NPE-1
      amp = amp + ampn(NN)
    end do; amp = amp/NPE/Ncellx/Ncelly/Ncellz
    amp = 0.1d0/dsqrt(amp)
  end if
...
!***** WRITE Initial Conditions *****!
  ...
  open(unit=8,file='/work/inouety/CMF1/D'//NPENUM//'.dat',FORM='UNFORMATTED')
  do k = 1, Ncellz
  do j = 1, Ncelly
