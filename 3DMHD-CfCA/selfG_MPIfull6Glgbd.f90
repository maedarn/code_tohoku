...
open(8,file='/home/maedarn/code/sample/INPUT3D.DAT')
  read(8,*)  Np1x,Np2x
  read(8,*)  Np1y,Np2y
  read(8,*)  Np1z,Np2z
  read(8,*)  ql1x,ql2x
...
IF(NRANK.EQ.0) THEN
  400 format(D25.17)
  open(4,file='/home/maedarn/code/sample/cdnt.DAT')
...
open(2,file='/home/maedarn/code/sample/tsave.DAT')
  ...
  open(unit=28,file='/home/maedarn/code/sample/delta2.dat',FORM='UNFORMATTED')
...
!**** read inhomogeneous density field ****!
  ...
  do MRANK = 0, NPE-1
    ...
    if((JS.eq.JST).and.(KS.eq.KST)) then
      ...
      open(unit=8,file='/home/maedarn/code/sample/DTF/D'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
      do k = 1, Ncellz
      do j = 1, Ncelly
...
if(nunit.eq.1) goto 120
  ...
  open(unit=8,file='/home/maedarn/code/sample/000'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  do k = 1, Ncellz+1
  do j = 1, Ncelly+1
...
open(2,file='/home/maedarn/code/sample/tsave.DAT')
  ...
  read(2,*) nunit
close(2)
open(2,file='/home/maedarn/code/sample/tsave2D.DAT')
  read(2,*) nunit2D
close(2)
open(3,file='/home/maedarn/code/sample/time.DAT')
do i = 1, nunit
  read(3,'(1p1d25.17)') t(i)
end do
close(3)
!IF(NRANK.EQ.0) THEN
!  open(2,file='/home/maedarn/code/sample/test.DAT')
!END IF

write(fnunit,'(I3.3)') nunit;  write(fnpe,'(I3.3)') NRANK
open(5,file='/home/maedarn/code/sample/info'//fnunit//'.DAT')
!open(5,file='/home/maedarn/code/sample/info'//fnunit//fnpe//'.DAT')
...
do i=0,Ncellx+1; nc = i+jj
   write(250+NRANK,*)  U(i,j,k,1)*G4pi
end do
end do
end do
close(250+NRANK)
...
write (name,'(i4.4)') countin
write (nR,'(i3.3)') NRANK
open(51+NRANK,file='rMPIpre'//name//nR//'.dat')
do kkkk=0,nz
   do jjjj=0,ny
      do iiii=0,nx
         write(51+NRANK,*) iiii,jjjj,kkkk,nx,u(iiii,jjjj,kkkk),rhs(iiii,jjjj,kkkk)
      end do
   end do
end do
close(51+NRANK)
...
write (name,'(i4.4)') countin
write (nR,'(i3.3)') a
open(51+NRANK,file='rpre'//name//nR//'.dat')
do kkkk=0,nz
   do jjjj=0,ny
      do iiii=0,nx
         write(51+NRANK,*) iiii,jjjj,kkkk,nx,u(iiii,jjjj,kkkk),rhs(iiii,jjjj,kkkk)
...
if(klr.le.Ncellx) then
  ...
  do m=1,nn2/2+1; do l=1,nn1/2
    kap = 4.d0*( dsin(pi*dble((l-1)/nn1))**2/dxx**2 + dsin(pi*dble((m-1)/nn2))**2/dyy**2 )
    kap = dsqrt(kap)
...
WRITE(NPENUM,'(I3.3)') NRANK
write(filenm,'(I3.3)') nunit
!open(10,file='/home/maedarn/code/sample/'//filenm//NPENUM//'.dat')
...
!    close(5)
  ...
  open(8,file='/home/maedarn/code/sample/000'//NPENUM//'.dat',FORM='UNFORMATTED') !,CONVERT='LITTLE_ENDIAN')
  do k = 1, Ncellz+1
  do j = 1, Ncelly+1
