...
double precision :: dm,lgmmax,lgmmin,ddN,Ntot,cefflw,ceffhi,stcen,diff
...
write(*,*) cnc
do i= 0,cnc
dN(i)=cefflw * dexp(-0.5d0*diff*diff*(dlog10(M(i)/stcen))**2.d0)!/M(i)/dlog(10.d0)
enddo

do i= cnc,nmesh
dN(i)=ceffhi*(M(i))**(-1.3d0)!+ddN
!dN(i)=ceffhi*(lgM(i))**(-1.3d0)
