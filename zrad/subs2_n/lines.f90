...
double precision :: esc(N_p),func1(N_p),desc(N_p),&
esc_f(N_p),func1_f(N_p),A(N_p,N_p),&
esc_min(N_p)

err_max_min=1.d10
do itr=1,1000
   call pop(esc,func1,xLd)
   do j=1,N_line
      ...
      call pop(esc_f,func1_f,xLd)
! ------ set the matrix A
      do i=1,N_line
         A(i,j)=(func1_f(i)-func1(i))/(esc_f(j)-esc(j)+1.d-100)
      enddo
   enddo
! ------- set the vector desc
   do i=1,N_line
...
SUBROUTINE pop_rot(esc,func1,xLd_rot)
   ...
   DDD(0:N_p),xnn(0:N_p),esc(N_p),func1(N_p),Q(N_p),S(N_p)
        ...
        do j=1,J_max
           ...
           func1(j)=esc(j)-beta_esc(tau,tau_cnt)
           xLd_rot=xLd_rot+f(j)*A(j)*DE*beta_esc(tau,tau_cnt)*(1.d0-Q(j)/S(j))/xnH
        enddo
END SUBROUTINE pop_rot

SUBROUTINE pop_CII(esc,func1,xLd_CII)
   USE lines
   USE PYSCONST
   IMPLICIT REAL*8(a-h,o-z)
   double precision :: esc(N_p),func1(N_p)
...
func1(1)=esc_10-beta_esc(tau_10,tau_cnt)
...
SUBROUTINE pop_CI(esc,func1,xLd_CI)
   !use COMVAR
   USE lines  
   USE PYSCONST
   IMPLICIT REAL*8(a-h,o-z)
...
func1(1)=esc_10-beta_esc(tau_10,tau_cnt)
func1(2)=esc_20-beta_esc(tau_20,tau_cnt)
func1(3)=esc_21-beta_esc(tau_21,tau_cnt)
...
SUBROUTINE pop_OI(esc,func1,xLd_OI)
...
SUBROUTINE pop_CIImeta(esc,func1,xLd_CIImeta)
   USE lines
   USE PYSCONST
   IMPLICIT REAL*8(a-h,o-z)
   double precision :: esc(N_p),func1(N_p)
...
SUBROUTINE pop_CImeta(esc,func1,xLd_CImeta)
    USE lines
    USE PYSCONST
    IMPLICIT REAL*8(a-h,o-z)
    double precision :: esc(N_p),func1(N_p)
...
SUBROUTINE pop_OImeta(esc,func1,xLd_OImeta)
    !     xnH*rn(OI)*xLd_OI : cooling rate owing to OI per unit volume
    USE lines
    USE PYSCONST
    IMPLICIT REAL*8(a-h,o-z)
