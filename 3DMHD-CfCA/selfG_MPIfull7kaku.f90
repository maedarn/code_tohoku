...
if(mode.eq.2) then
  ...
  !end do; end do; end if
  if(IST.eq.0       ) then; do k=-1,Ncellz+2; do j=-1,Ncelly+2
    Phi(0       ,j,k) = Phi(1     ,j,k); Phi(-1       ,j,k) = Phi(1     ,j,k) !grad=0
  end do; end do; end if
  if(IST.eq.NSPLTx-1) then; do k=-1,Ncellz+2; do j=-1,Ncelly+2
    Phi(Ncellx+1,j,k) = Phi(Ncellx,j,k); Phi(Ncellx+2,j,k) = Phi(Ncellx,j,k)
 end do; end do; end if
...
open(12,file='inPHI_MPIxr'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(17,file='inPHI_MPIxl'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(13,file='inPHI_MPIyr'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(14,file='inPHI_MPIyl'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(15,file='inPHI_MPIzr'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(16,file='inPHI_MPIzl'//frank//sav//'.DAT',FORM='UNFORMATTED')

open(22,file='bcPHI_MPIxr'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(27,file='bcPHI_MPIxl'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(23,file='bcPHI_MPIyr'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(24,file='bcPHI_MPIyl'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(25,file='bcPHI_MPIzr'//frank//sav//'.DAT',FORM='UNFORMATTED')
open(26,file='bcPHI_MPIzl'//frank//sav//'.DAT',FORM='UNFORMATTED')
...
!   write(nr1,'(I3.3)') NRANK
!   open(4,file=dir//'fg'//nr1//'.DAT')
   dxi = 1.d0/(12.d0*dx(0))
...
!      write(4,*) i,j,k,( -Phi(i+2,j,k)+8.d0*Phi(i+1,j,k)-8.d0*Phi(i-1,j,k)+Phi(i-2,j,k) ) * dxi *0.5d0,&
!           ( -Phi(i,j+2,k)+8.d0*Phi(i,j+1,k)-8.d0*Phi(i,j-1,k)+Phi(i,j-2,k) ) * dxi *0.5d0,&
!           ( -Phi(i,j,k+2)+8.d0*Phi(i,j,k+1)-8.d0*Phi(i,j,k-1)+Phi(i,j,k-2) ) * dxi *0.5d0,&
!       Phi(i,j+2,k),Phi(i,j+1,k),Phi(i,j-1,k),Phi(i,j-2,k)
...
!   U(:,1,:,3)=U(:,2,:,3)
!   U(:,:,1,4)=U(:,:,2,4)
!   U(:,Ncelly,:,3)=U(:,Ncelly-1,:,3)
!   U(:,:,Ncellz,4)=U(:,:,Ncellz-1,4)
...
CALL BCsgr_MPI(uf,nx,ny,nz,1,1,1,1,1,1)
...
integer :: fromx=0,fromy=0,fromz=0
integer nxst,nxen,nxjm,nyst,nyen,nyjm,nzst,nzen,nzjm
if(mod(fromx,2)==0) then
   nxst=1
   nxen=nx-1
   nxjm=1
else
