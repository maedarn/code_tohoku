...
MODULE chmvar
integer :: fct=1
double precision :: Zsolar=1.d0,xHe=0.1d0, fgr=1.d0,av1para=0.d0,av2para=0.d0,zeta_cr=1.d0
...
open(100,file=dir//'Thermal_bialy9_Z10m0_l_mid.dat' ,access='stream',FORM='UNFORMATTED')!, position='append')
open(110,file=dir//'Thermal_bialy9_Z10m0_md_mid.dat',access='stream',FORM='UNFORMATTED')!, position='append')
open(120,file=dir//'Thermal_bialy9_Z10m0_h_mid.dat' ,access='stream',FORM='FORMATTED')!, position='append')
...
integer :: idlogT
DOUBLE PRECISION :: Ecr,phis, phi_pah=0.5d0,Td
...
!---WMHT+03
!Lamc=3.15d-27*1.9733d27*dexp(-0.092d0/T)*fgr*(dmax1(ndCp,0.d0)/xc)*(ndH+0.5d0*ndH2)
!Lamc=Lamc+1.4d-24*1.9733d27*(T*1.d0)**(-0.5d0)*dexp(-0.092d0/T)*fgr*(dmax1(ndCp,0.d0)/xc)*nde
!***------------------------- OI Cooling
...
Lamd = fgr*nde*ndtot*phi_pah * 4.65d-30 *  1.9733d27 * ((T*1.d3)**0.94d0) * ( (1.7d0*pha)**( 0.74d0/(T**6.8d-2) ) )
!***------------------------- Dust-gas-coll. Cooling
!---Bialy+19; BT+94, WMHT+03---
Td=1.64d1*G0**(1.d0/6.0)
...
!Gampe = fgr*ndtot * 2.2d-24 * 1.9733d27 * G0*ATN1 * &
!       ( 3.7d-2*((T/1.d1)**0.7d0)/(1.d0+3.4d-4*pha) + 4.9d-2/(1.d0+5.9d-13*(pha**0.73d0)) )
!------------------------- CR Heating
!Gamcr = (ndH+ndHe+ndH2) * 1.89435d0
!Gamcr = (ndH+ndHe+ndH2) * 12.62d0/2.d0 * zeta_cr !zeta=1.d-16
Ecr=6.43d0*(1.d0+4.06d0*(nde/(nde+7.d-2))**0.5d0)*1.6022d-12
phis=(1.d0-nde/1.2d0)*0.67d0/(1.d0+nde/5.d-2)
