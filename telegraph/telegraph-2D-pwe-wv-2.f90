...
module grvvar
  ...
  DOUBLE PRECISION , dimension(-1:ndx2,-1:ndy2,dim2) :: fp1 ,fp2, fppre1 ,fp2pre !fp->wave
...
program muscl1D
  ...
  do i=1,laststep
    ...
    call muslcslv1D(fp2(:,:,1),rho,dt*0.5d0,1,2)
    call muslcslv1D(fp2(:,:,2),rho,dt*0.5d0,2,2)
    call muslcslv1D(fp2(:,:,3),rho,dt*0.5d0,1,2)
    call muslcslv1D(fp2(:,:,4),rho,dt*0.5d0,2,2)
    ...
    call muslcslv1D(fp2(:,:,1),rho,dt*0.5d0,1,2)
    call muslcslv1D(fp2(:,:,2),rho,dt*0.5d0,2,2)
    call muslcslv1D(fp2(:,:,3),rho,dt*0.5d0,2,2)
    call muslcslv1D(fp2(:,:,4),rho,dt*0.5d0,1,2)
...
iwx=1;iwy=1
call BC(fp2(:,:,1),13,13,13,13,1)
call BC(fp2(:,:,2),13,13,13,13,2)
call BC(fp2(:,:,3),13,13,13,13,3)
call BC(fp2(:,:,4),13,13,13,13,4)

call BC(fp1(:,:,1),14,14,14,14,1)
call BC(fp1(:,:,2),14,14,14,14,2)
call BC(fp1(:,:,3),14,14,14,14,3)
call BC(fp1(:,:,4),14,14,14,14,4)
...
do k=1,ndy-2
    do j=1,ndx-2
fp2(j,k,1) = fp2(j,k,1)+cg*dt*fppre1(j,k,1)!*0.5d0
fp2(j,k,2) = fp2(j,k,2)+cg*dt*fppre1(j,k,2)!*0.5d0
fp2(j,k,3) = fp2(j,k,3)+cg*dt*fppre1(j,k,3)!*0.5d0
fp2(j,k,4) = fp2(j,k,4)+cg*dt*fppre1(j,k,4)!*0.5d0
    enddo
enddo
...
-0.5d0*dt*cg*(fp2pre(j+1,k+1,1)-fp2pre(j+1,k-1,1)-fp2pre(j-1,k+1,1)+fp2pre(j-1,k-1,1))/dx/dy
fp1(j,k,2) = fp1(j,k,2)-cg*G4pi*rho(j,k)*dt &
-0.5d0*dt*cg*(fp2pre(j+1,k+1,2)-fp2pre(j+1,k-1,2)-fp2pre(j-1,k+1,2)+fp2pre(j-1,k-1,2))/dx/dy
fp1(j,k,3) = fp1(j,k,3)-cg*G4pi*rho(j,k)*dt &
+0.5d0*dt*cg*(fp2pre(j+1,k+1,3)-fp2pre(j+1,k-1,3)-fp2pre(j-1,k+1,3)+fp2pre(j-1,k-1,3))/dx/dy
fp1(j,k,4) = fp1(j,k,4)-cg*G4pi*rho(j,k)*dt &
+0.5d0*dt*cg*(fp2pre(j+1,k+1,4)-fp2pre(j+1,k-1,4)-fp2pre(j-1,k+1,4)+fp2pre(j-1,k-1,4))/dx/dy
      enddo
    enddo
...
subroutine saveu(in1)
  ...
  do j=-1,ndy
     do i=-1,ndx
        write(21,*) x(i),',',y(j),',',fp1(i,j,1),',',fp1(i,j,2),',',fp1(i,j,3),',',fp1(i,j,4) &
      ,',',fp2(i,j,1),',',fp2(i,j,2),',',fp2(i,j,3),',',fp2(i,j,4)&
      ,',',0.25d0*(fp2(i,j,1)+fp2(i,j,2)+fp2(i,j,3)+fp2(i,j,4)),',',rho(i,j),',',Phiexa(i,j),',',grdch(i,j),',',&
Phigrd(i,j,1),',',Phigrd(i,j,3),',',Phigrd(i,j,4),',',fp2(i,j,1)-Phiexa(i,j),',' &
,Phiexa2(i,j)+cg*Tdiff*((Phiexa2(i+1,j)-Phiexa2(i-1,j))/dx+(Phiexa2(i,j+1)-Phiexa2(i,j-1))/dy)!,',',&
  ...
  j=ndy/2
  do i=-1,ndx
        write(22,*) x(i),fp1(i,j,1),fp2(i,j,1),rho(i,j),Phiexa(i,j),Phigrd(i,j,1)&
