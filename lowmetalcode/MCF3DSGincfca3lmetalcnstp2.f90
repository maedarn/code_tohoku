...
! dinit1=mH*Hini+mH*pini+mH2*H2ini+mHe*Heini+mHe*Hepini; dinit2=dinit1
 ...
 Cini=0.d0*totn1; COini=0.d0*totn1; Cpini=0.14d-3*totn1
...
double precision  :: mmean,rtTx,rtTy,rtTz,tcd,CooL,rhoshd,G30,T30,timeIO=0.d0,cs30,coolkakunin,l1,tcool1,l2
...
!----- Cooling ---------------------------------------------------------
  ...
  gammi1 = ( 2.d0*(ndH(i,j,k)+ndp(i,j,k)+ndHe(i,j,k)+ndHep(i,j,k))+2.d0*ndH2(i,j,k) )/gammi1
  rhoold=U(i,j,k,1)
  U(i,j,k,1)=U(i,j,k,1)+gammi1*U(i,j,k,1)/(gammi1+1.d0)/U(i,j,k,5)*CooL*dt
  ndH(i,j,k)=U(i,j,k,1)/rhoold*ndH(i,j,k)
  ndp(i,j,k)=U(i,j,k,1)/rhoold*ndp(i,j,k)
  ndH2(i,j,k)=U(i,j,k,1)/rhoold*ndH2(i,j,k)
  ndHe(i,j,k)=U(i,j,k,1)/rhoold*ndHe(i,j,k)
  ndHep(i,j,k)=U(i,j,k,1)/rhoold*ndHep(i,j,k)
  ndC(i,j,k)=U(i,j,k,1)/rhoold*ndC(i,j,k)
  ndCp(i,j,k)=U(i,j,k,1)/rhoold*ndCp(i,j,k)
  ndCO(i,j,k)=U(i,j,k,1)/rhoold*ndCO(i,j,k)
  !coolkakunin=CooL
...
!CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
!if(NRANK==40) write(*,*) NRANK,U(33,33,33,1),U(33,33,33,5),sngl(U(33,33,33,1)),CooL,rhoold,'point101'
...
if(NRANK==40) then
   ...
   tcool1=U(30,30,30,5)/dabs(CooL)/gammi1
   l1=cs30*tcool1
   !l2=dsqrt(Kcond*T30/CooL)
   T30 = U(30,30,30,5)/( kb*(ndp(30,30,30)+ndH(30,30,30)+ndH2(30,30,30)+ndHe(30,30,30)+ndHep(30,30,30)) )
   l2=dsqrt(Kcond*T30/dabs(CooL))
   write(128,*) U(30,30,30,1),U(30,30,30,5),U(30,30,30,1)/1.27d0,T30,G30,cs30,tcool1,l1,l2,timeIO
end if
!CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
!write(*,*)'sourecout'
timeIO=timeIO+dt
...
if(NRANK==40) then
   ...
   G30=gammi1+1.d0
   cs30=dsqrt(U(30,30,30,5)/U(30,30,30,1)*G30)
   write(128) U(30,30,30,1),U(30,30,30,5),U(30,30,30,1)/1.27d0,Tn(30,30,30),G30,cs30,timeIO
end if
do k = 1, Ncellz; do j = 1, Ncelly; do i = 1, Ncellx
!----- Cooling ---------------------------------------------------------
  Call Fcool( CooL,Tn(i,j,k),i,j,k)
