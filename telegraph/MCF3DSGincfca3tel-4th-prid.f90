...
DOUBLE PRECISION, dimension(-1-1-1:ndx+1+1) :: x,dx
DOUBLE PRECISION, dimension(-1-1-1:ndy+1+1) :: y,dy
DOUBLE PRECISION, dimension(-1-1-1:ndz+1+1) :: z,dz
...
DOUBLE PRECISION, parameter :: cg=1.0d0,sourratio=0.5d0,Tdiff=20.d0,adiff=0.375d0,rratio=0.2d0,tratio=0.05d0
double precision :: dx1,dy1,dz1
!integer ifevogrv,ifevogrv2
!character(25) :: dir='/work/maedarn/3DMHD/test/' !samplecnv2
character(68) :: dir='/work/maedarn/3DMHD/tel/cg1-T20-mash128-L100-3pwe-4thmscl-test-t005/'
END MODULE comvar
...
!*********grvwave*********
ALLOCATE(Phiexa(-2-1:ndx+1+1,-2-1:ndy+1+1,-2-1:ndz+1+1))
...
ALLOCATE(Phigrd(-2:ndx+1,-2:ndy+1,-2:ndz+1,1:wvnum))

!ALLOCATE(Phiwv(-1:ndx,-1:ndy,-1:ndz,1:wvnum))
!ALLOCATE(Phigrdwv(-1:ndx,-1:ndy,-1:ndz,1:wvnum))
ALLOCATE(Phiwv(-2:ndx+1,-2:ndy+1,-2:ndz+1,1:wvnum))
ALLOCATE(Phigrdwv(-2:ndx+1,-2:ndy+1,-2:ndz+1,1:wvnum))
...
double precision cenx,ceny,cenz,rsph,rrsph,Hsheet,censh,minexa,rsph3,rrsph3,rhomean1
...
write(*,*) NRANK,'INIT'
ALLOCATE(dx_i(-2-1:Ncellx*NSPLTx+2+1+1)); ALLOCATE(dy_i(-2-1:Ncelly*NSPLTy+2+1+1)); ALLOCATE(dz_i(-2-1:Ncellz*NSPLTz+2+1+1))
ALLOCATE( x_i(-2-1:Ncellx*NSPLTx+2+1+1)); ALLOCATE( y_i(-2-1:Ncelly*NSPLTy+2+1+1)); ALLOCATE( z_i(-2-1:Ncellz*NSPLTz+2+1+1))
...
do i = Np1x+1, Ncellx*NSPLTx+2+1+1
  dx_i(i) = ql2x/dble(Np2x)
end do
do j = -2-1, Ncelly*NSPLTy+2+1+1
  dy_i(j) = ql1y/dble(Np1y)
end do
do k = -2-1, Ncellz*NSPLTz+2+1+1
  dz_i(k) = ql1z/dble(Np1z)
end do
...
do i = -2-1, Ncellx+2+1+1
  ix    =  IST*Ncellx + i
  x(i)  =  x_i(ix)
  dx(i) =  dx_i(ix)
end do
do j = -2-1, Ncelly+2+1+1
  jy    =  JST*Ncelly + j
  y(j)  =  y_i(jy)
  dy(j) =  dy_i(jy)
end do
do k = -2-1, Ncellz+2+1+1
  kz    =  KST*Ncellz + k
  z(k)  =  z_i(kz)
  dz(k) =  dz_i(kz)
end do
...
rhomean1=rhomean1/(Np1x+Np2x)/(Np1y+Np2y)/(Np1z+Np2z)
  
do k = -1, Ncellz+2; do j = -1, Ncelly+2; do i = -1, Ncellx+2
  U(i,j,k,1)=U(i,j,k,1)-rhomean1
end do
end do
end do
...
do k = -2-1, Ncellz+2+1+1; do j = -2-1, Ncelly+2+1+1; do i = -2-1, Ncellx+2+1+1
   i2 = IST*Ncellx+i
   i2y = JST*Ncelly+j
   i2z = KST*Ncellz+k
   cenx=dble(Np1x)+0.5d0
