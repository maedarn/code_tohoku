...
character(31)::dir='/work/maedarn/3DMHD/sampletstC/'
...
INTEGER, parameter :: numstar=5000,valstar=22,nlpmass=50000,nran1=1000000
...
DOUBLE PRECISION  :: mass1,rhomean,rrsph3x,rrsph3y,rrsph3z,alpharcm=3.d0*1.d-13
DOUBLE PRECISION, dimension(:,:,:,:), allocatable  :: rdrgn,denrgn,radint
...
ALLOCATE(Ustar(1:valstar,0:numstar),radintst(0:numstar))
...
!               sngl(Phi(i,j,k)),i=1,Ncellx+1 )
  ...
  !             sngl(Phi(i,j,k)),i=-1,Ncellx+2 )
   ...
   sngl(ndHep(i,j,k)),sngl(ndC(i,j,k)),sngl(ndCO(i,j,k)),sngl(ndCp(i,j,k)), &
   !rdrgn(i,j,k,2),Phi(i,j,k), i=1,Ncellx+1 )
   sngl(rdrgn(i,j,k,2)),sngl(Phi(i,j,k)), i=-1,Ncellx+2 )
