...
1 if (nn.gt.3) then   !Similarly allocate storage and ll r.h.s. on all
  ...
  do j=2,ngrid! Nested iteration loop.
     ...
     if (j.ne.ngrid) then !.ne. = not equal
        call copy(z(irhs(j)),z(irho(j)),nn) !Set up r.h.s.  nn=3の時２など？ zを使う(rstrctで用いた値を使う)
     else
        call copy(z(irhs(j)),u,nn) !元の行列を使う
     endif
     ...
     do jcycle=1,ncycle !V-cycle loop.
        ...
        do jj=j,2,-1 !Downward stoke of the V.
           do jpre=1,NPRE !Pre-smoothing.ガウスサイデル法の回数
              call relax(z(iu(jj)),z(irhs(jj)),nf) !前のあらさの値を使う、そして収束させる。
           enddo
