module comvar
  implicit none
  integer :: ndx=130,laststep=1000,ist=2,ien=3 !preiodic:ist=1,ien=2 , kotei:ist=2,ien=3
...
subroutine timesource(Phiv,source,dt,mode)
  ...
  if(mode==1) then
     ...
     if( (maxdt < dt) .and. (maxde .ne. 0.0d0)) then
        dt = sdt
     end if
  end if
...
subroutine muslcslv1D(Phiv,source,dt,mode)
  ...
  DOUBLE PRECISION, dimension(-1:ndx) :: Phigrad,Phipre,fluxphi,Phiv,source,Phi2dt,Phiu
