SUBROUTINE grtemp(xnH,T,esc_cnt,T_rad,xJ_ex,T_gr,xk_vis)
   ...
   IMPLICIT REAL*8(a-h,o-z)
    ...
    x_l=T_gr
    x_u=T_gr
    if(funcD(xnH,T_gr,T,esc_cnt,T_rad).gt.0.d0) then
       do i=1,1000
          x_l=0.99d0*x_l
          if(funcD(xnH,x_l,T,esc_cnt,T_rad).lt.0.d0) go to 10
       enddo
    elseif(funcD(xnH,T_gr,T,esc_cnt,T_rad).lt.0.d0) then
       do i=1,1000
          x_u=1.01d0*x_u
          if(funcD(xnH,x_u,T,esc_cnt,T_rad).gt.0.d0) go to 10
       enddo
    else
       T_gr=T
       return
...
10  continue
    ...
    fmid=funcD(xnH,x2,T,esc_cnt,T_rad)
    f=funcD(xnH,x1,T,esc_cnt,T_rad)
    if(f.eq.0.d0) then
       T_gr=x1
       go to 13
    ...
    do j=1,100
      dx=dx*.5d0
      xmid=T_gr+dx
      fmid=funcD(xnH,xmid,T,esc_cnt,T_rad)
...
FUNCTION funcL(xnH,T_d,T)
   USE PYSCONST
   IMPLICIT REAL*8(a-h,o-z)

   funcL=4.9d-2*vol_gr((1.d0+4.d0*yHe)*xnH*xm_p,T_d)&
...
FUNCTION funcD(xnH,T_d,T,esc_cnt,T_rad)
   USE PYSCONST
   IMPLICIT REAL*8(a-h,o-z)

   funcD=xkp_gr((1.d0+4.d0*yHe)*xnH*xm_p,T_d)&
   *(T_d**4)*esc_cnt &
