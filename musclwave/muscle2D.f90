module comvar
  implicit none
  integer, parameter :: ndx=66,ndy=66,laststep=100000,istx=1,ienx=2,isty=1,ieny=2,svnum=1000 !preiodic:ist=1,ien=2 , kotei:ist=2,ien=3 : ndx=130
  ...
  character(58) :: dir='/Users/maeda/Desktop/Dropbox/kaiseki-desktpo/testmuscledf/'
end module comvar
...
program muscl1D
  ...
  do i=1,laststep
     ...
     do k=-1,ndy
        do j=-1,ndx
           wp1(j,k)=(Phidtn(j,k)+cg*fx(j,k))/dsqrt(2.d0*cg)
           wp2(j,k)=(Phidtn(j,k)-cg*fx(j,k))/dsqrt(2.d0*cg)
        end do
     end do
     ...
     do k=1,ndy-2
        do j=1,ndx-2
           Phidtn(j,k)=0.5d0*((wp1(j,k)+wp2(j,k))*dsqrt(2.d0*cg))
           fx(j,k)=0.5d0*((-wp1(j,k)+wp2(j,k))*dsqrt(2.d0*cg))/cg
        end do
     end do
     call BC(1)
     ...
     do k=-1,ndy
        do j=-1,ndx
           wp1(j,k)=(Phidtn(j,k)+cg*fy(j,k))/dsqrt(2.d0*cg)
           wp2(j,k)=(Phidtn(j,k)-cg*fy(j,k))/dsqrt(2.d0*cg)
        end do
     end do
     call muslcslv1D(wp1,rho,dt,2,2)
     call muslcslv1D(wp2,rho,dt,1,2)
     do k=1,ndy-2
        do j=1,ndx-2
           Phidtn(j,k)=0.5d0*((wp1(j,k)+wp2(j,k))*dsqrt(2.d0*cg))
           fy(j,k)=0.5d0*((-wp1(j,k)+wp2(j,k))*dsqrt(2.d0*cg))/cg
        end do
     end do
     call BC(2)
     ...
     do k=1,ndy-2
        do j=1,ndx-2
           Phidtn(j,k) = Phidtn(j,k) -  G4pi * cg * cg * dt * rho(j,k)
        end do
     end do
     call BC(1)
     call BC(2)
...
subroutine BC(mode)
  ...
  if(mode==2) then
     Phidtn(:,   0)=Phidtn(:,ndy-2)
     Phidtn(:,  -1)=Phidtn(:,ndy-3)
     Phidtn(:,ndy-1)=Phidtn(:,   1)
     Phidtn(:,ndy  )=Phidtn(:,   2)
...
subroutine saveu(in1)
  ...
  do j=1,ndy-2
     do i=1,ndx-2
        write(21,*) Phidtn(i,j),fx(i,j),fy(i,j),wp1(i,j),wp2(i,j)!Phi(i), v(i),wp1(i),wp2(i),xi1(i),xi2(i)!Phicgp(i) , Phicgm(i) ,rho(i)!,&
     end do
     write(21,*)
  end do
  close(21)
  ...
  j=ndy/2
  do i=1,ndx-2
        write(22,*)x(i), Phidtn(i,j),fx(i,j),fy(i,j),wp1(i,j),wp2(i,j)
  end do
  close(22)
  write(*,*) 'save step : ',in1
  in1=in1+1
